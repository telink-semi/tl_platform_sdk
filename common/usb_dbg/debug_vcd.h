...
#define log_tick_general(en,id)     if(VCD_EN && en) {my_irq_disable();log_uart(0x20|(id&31));int t=clock_time();log_uart(t);log_uart(t>>8);log_uart(t>>16);my_irq_restore();}
...
#define log_event_general(en,id)    if(VCD_EN && en) {my_irq_disable();log_uart(0x00|(id&31));my_irq_restore();}
...
#define log_task_general(en,id,b)    if(VCD_EN && en) {my_irq_disable();log_uart(((b)?0x60:0x40)|(id&31));int t=clock_time();log_uart(t);log_uart(t>>8);log_uart(t>>16);my_irq_restore();}
...
#define log_b8_general(en,id,d)              if(VCD_EN && en) {my_irq_disable();log_uart(0x80|(id&63));log_uart(d);my_irq_restore();}
...
#define log_b16_general(en,id,d)                   if(VCD_EN && en) {my_irq_disable();log_uart(0xc0|(id&63));log_uart(d);log_uart((d)>>8);my_irq_restore();}
#define log_b16_byte_general(en,id,high_b,low_b)   if(VCD_EN && en) {my_irq_disable();log_uart(0xc0|(id));log_uart(low_b);log_uart(high_b);my_irq_restore();}
 ...
 * log_xxx_irq:     can be only used in IRQ, not execute IRQ protect(disable & restore) to save running timing
 */
#define log_tick_general(en,id)     if(VCD_EN && en) {my_irq_disable();int t=clock_time();log_uart(3 | (id<<24) | (t&0xfffff8));my_irq_restore();}

#define log_tick_irq(en,id)         if(VCD_EN && en) {int t=clock_time();log_uart(3 | (id<<24) | (t&0xfffff8));}
...
/* log_task: 1-byte (01x_id-5bits) 1-bit data: id0 & id1 reserved for hardware */
#define log_task_general(en,id,level)    if(VCD_EN && en) {my_irq_disable();int t = clock_time();log_uart((level ? 7 : 5) | (id<<24) | (t&0xfffff8));my_irq_restore();}

#define log_task_begin_irq(en,id)    if(VCD_EN && en) {int t = clock_time(); log_uart(7 | (id<<24) | (t&0xfffff8));}
#define log_task_end_irq(en,id)  if(VCD_EN && en) {int t = clock_time(); log_uart(5 | (id<<24) | (t&0xfffff8));}

/* log_b16:  2-byte (11-id-6bits) 16-bit data */
#define log_b16_general(en,id,d)                   if(VCD_EN && en) {my_irq_disable();log_uart(1 | (id<<24) | ((d&0xffff)<<8));my_irq_restore();}

#define log_b16_irq(en,id,d)         if(VCD_EN && en) {log_uart((1) | (((id)<<24) & 0xFFFFFFFF) | ((((d)&0xffff)<<8) & 0xFFFFFFFF));}
#define log_b16_byte_general(en,id,high_b,low_b)
#define log_b16_byte_irq(en,id,high_b,low_b)       

/* log_b16: 24-bit data */
#define log_b24_general(en,id,d)                    if(VCD_EN && en) {my_irq_disable();log_uart(((id&127)<<1) | (d<<8));my_irq_restore();}
#define log_b24_irq(en,id,d)                        if(VCD_EN && en) {log_uart(((id&127)<<1) | (d<<8));}
#endif /* USB_CNT */
#else
