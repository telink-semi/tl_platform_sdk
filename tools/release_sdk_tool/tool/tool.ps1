...
function get_path_list{
    ...
    if(Test-Path $path){
        ...
        {
            $path_tmp = $path + '\' + $file
            if(Test-Path -Path $path_tmp -PathType Container)
            {
                get_path_list $path_tmp
            }
            else
            {
                remove_code_operation $path_tmp
            }
        }
    }
}

function remove_code_operation{
    param (
        [string]$path
    ) 
    $pattern_begin = "<remove when release sdk>"
    $pattern_end = "</remove when release sdk>"
    $pattern_para_begin = "<remove para when release sdk>"
    $pattern_para_end = "</remove para when release sdk>"
    $str_para_begin = "/*<remove para when release sdk>*/"
    $str_para_end = "/*</remove para when release sdk>*/"
    if(Test-Path $path){
        if($path.EndsWith('.c') -or $path.EndsWith('.h'))
        {
            ...
            $data_temp = @()
            $remove_flag = $false
            $remove_did_flag = $false
			
			if($data.Count -gt 5000)
			{
				return
			}
            ...
            for($i = 0; $i -lt $data.Count; $i++)
            {
                if($data[$i] -match $pattern_begin)
                {
                    $remove_flag = $true
                    $remove_did_flag = $true
                }
                ...
                if(-not $remove_flag)
                {
                    ...
                    if(($line_data -match $pattern_para_begin) -and ($line_data -match $pattern_para_end))
                    {
                        $line_data = $line_data.Replace($str_para_begin, '@')
                        $line_data = $line_data.Replace($str_para_end, '@')
                        $line_data_split = $line_data.Split('@')
                        if ($line_data_split.Length -eq 3)
                        {
                            $line_data = ($line_data_split[0] + $line_data_split[2])
                            $remove_did_flag = $true
                        }
                        else
                        {
            ...
            if($remove_did_flag)
            {
                show_info ("Remove code in '" + $path + "' successfully.")
            }         
        }
    }   
}
...
function show_help {
    ...
    show_info "rm_code:     para[1] path includeing files to remove code"
    show_info "help:        None."
}
...
switch($cmd){
    ...
    st_node{
        if([string]::IsNullOrEmpty($argv4)){
            ...
            try {
				...
				for($i = 0; $i -lt $format_list.Length; $i++)
				{
					$node_temp = $node_temp.Replace("{" + [string]$i + "}", $format_list[$i])
				}
            }
            catch {
    ...
    rm_code{
        if([string]::IsNullOrEmpty($argv1)){
            show_info "Incomplete parameters."
        }
        $folder_path = $argv1
        remove_code $folder_path
    }
    help{
        show_help
    }
        ...
        for($i = 0; $i -lt $data.Count; $i++){
            ...
            if($match_flag ){
                ...
                if($temp.Count -ge 1){
					...
					$line_temp1 = $line_temp.Replace($value_temp, $node_value)
                    if($line_temp -ne $line_temp1){
                        $line_temp = $line_temp1
                        $exec_flag = $true
...
#set_node 'D:\xupengcheng_workspace\2022\1025_jaguar_test\prj\telink_b91m_driver_src\tools\emi_bin_generate_tool\tool\.cproject' 'cproject::storageModule::cconfiguration::storageModule::configuration name="UART_Demo"::folderInfo::toolChain::tool::option::listOptionValue value="&quot;${workspace_loc:/${ProjName}/link/flash_boot.link}&quot;"' 'value="&quot;${workspace_loc:/${ProjName}/link/ram_boot.link}&quot;"'
...
#remove_code 'D:\xupengcheng_workspace\2022\1208_b92_release_sdk_tool\test\test_file\ADC_Demo'