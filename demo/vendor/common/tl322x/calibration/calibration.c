...
#if (MCU_CALIB_USE_NVM)
    unsigned char default_val = 0x00;
    nvm_ahb0_read_byte(addr, 1, &frequency_offset_value);
#else
    unsigned char default_val = 0xff;
    flash_read_page(addr, 1, &frequency_offset_value);
#endif

    if (default_val != frequency_offset_value) {
...
void calibration_func(void)
{
    ...
    user_calib_freq_offset(NVM_CAP_VALUE_ADDR_512K);
    user_get_ieee_addr(NVM_IEEE_ADDR_LOCATION_512K, ieee_addr);
...
{
    //Read IEEE address priority: FLASH/NVM > EFUSE
    if (addr == 0) {
        return user_get_efuse_ieee_addr(buf);
    } else {
#if (MCU_CALIB_USE_NVM)
        nvm_ahb0_read_byte(addr, 8, buf);
        if (user_check_ieee_addr(buf))
        {
            return IEEE_ADDR_FROM_NVM;
        }
#else
