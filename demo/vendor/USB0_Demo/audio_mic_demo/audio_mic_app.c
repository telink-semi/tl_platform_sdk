...
__attribute__((aligned(8))) unsigned char write_buffer[AUDIO_IN_PACKET];
...
void usbd_audio_open(unsigned char bus, unsigned char intf)
{
    ...
    usbd_ep_write(0, AUDIO_MIC_IN_ENDPOINT_ADDRESS, write_buffer, sizeof(write_buffer));
}
...
void usbd_audio_epin_callback(unsigned char bus, unsigned char ep_addr, unsigned int len)
{
    ...
    send_data++;
    memset(write_buffer, send_data, AUDIO_IN_PACKET);
