...
static unsigned char  chn, pkt_type, freq, uart_tx_index, uart_rx_index;
static unsigned char  bb_mode = 1;
static unsigned char  para, ctrl;
...
unsigned char                             bqbtest_buffer[290] __attribute__((aligned(4)));
...
{
#if (RF_FAST_SETTLE)&&(defined(MCU_CORE_B91) || defined(MCU_CORE_B92))
...
#elif (RF_FAST_SETTLE)&&(defined(MCU_CORE_TL721X) || defined(MCU_CORE_TL321X))
    rf_set_ble_500K_mode();
...
#elif (RF_FAST_SETTLE)&&(defined(MCU_CORE_TL322X) || defined(MCU_CORE_TL323X) || defined(MCU_CORE_TL521X))
    rf_set_ble_500K_mode();
...
#else
        pa_setting_init(g_bqb_setting.pa_en, g_bqb_setting.pa_bypass_en);
#endif
    }
}
...
void bqbtest_init()
{
    ...
    rf_set_ble_1M_NO_PN_mode();
#if (RF_FAST_SETTLE)
    rf_set_power_level(g_bqb_setting.power);
    #if defined(MCU_CORE_B91) || defined(MCU_CORE_B92)
    fs_get_value();
    ...
    rf_set_tx_settle_time(30);
    rf_set_rx_settle_time(30);
#endif
    rf_access_code_comm(g_bqb_setting.access_code);
    rf_pn_disable();
    bb_mode       = 1;
    uart_tx_index = 0;
    uart_rx_index = 0;
...
{
    unsigned int  total_len, bit_time, byte_time = 8;
    unsigned char preamble_len;
    unsigned int  total_time = 12500;
...
void bqb_serviceloop(void)
{
    ...
    rf_rx_performance_mode(RF_RX_LOW_POWER);
        ...
        {
#if defined(MCU_CORE_TL323X)
            rf_rx_performance_mode(RF_RX_HIGH_PERFORMANCE);//Must be RF_RX_HIGH_PERFORMANCE under tx mode or max power error.
#if 0 //This operation is designed to provide the modulation indicator with greater margin.
    rf_set_tx_modulation_index(525);
            ...
            if ((uPrivateStatus.tStatus.test_flag == 1) && (uPrivateStatus.tStatus.test_mode == PRIVATE_TX_CARRIER_MODE)) {
                ...
                extern void          rf_set_power_level_singletone(rf_power_level_e level);
...
#elif defined(MCU_CORE_TL721X)|| defined(MCU_CORE_TL322X) || defined(MCU_CORE_TL323X) || defined(MCU_CORE_TL521X) || defined(MCU_CORE_TL321X)
    rf_set_tx_rx_off(); //STOP_RF_STATE_MACHINE;
    rf_clr_irq_status(FLD_RF_IRQ_ALL);
    rf_set_tx_settle_time(113);        //adjust TX settle time
...
#elif  defined(MCU_CORE_TL323X) || defined(MCU_CORE_TL521X) || defined(MCU_CORE_TL321X)||defined(MCU_CORE_TL322X) || defined(MCU_CORE_TL721X)
    rf_set_rx_settle_time(93); //adjust RX settle time
    for (unsigned char f_chn = 0; f_chn <= 80; f_chn++) {
        rf_set_chn(f_chn);
        rf_set_rxmode();
