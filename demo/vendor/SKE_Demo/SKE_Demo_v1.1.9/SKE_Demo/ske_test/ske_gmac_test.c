...
unsigned int ske_gmac_test(SKE_ALG alg, unsigned char wordAlign, unsigned char *std_msg, unsigned int byteLen, unsigned char *key, unsigned short sp_key_idx, unsigned char *iv, unsigned char ivByteLen, unsigned char *aad, unsigned char aadByteLen, unsigned char *std_mac, unsigned int macByteLen)
{
    ...
    */
    ...
    //generate mac
    ret = ske_lp_gmac_crypto(alg, SKE_GENERATE_MAC, std_key_, sp_key_idx, std_iv_, ivByteLen, std_aad_, aadByteLen, std_msg_, byteLen, mac_, macByteLen);
    if ((SKE_SUCCESS != ret) || (memcmp_(mac_, std_mac_, macByteLen))) {
    ...
    //verify mac
    ret = ske_lp_gmac_crypto(alg, SKE_VERIFY_MAC, std_key_, sp_key_idx, std_iv_, ivByteLen, std_aad_, aadByteLen, std_msg_, byteLen, mac_, macByteLen);
    if (SKE_SUCCESS != ret) {
...
unsigned int ske_gmac_test_internal(SKE_GMAC_TEST_VECTOR *vector, unsigned int num)
{
    ...
    for (i = 0; i < num; i++) {
        ret = ske_gmac_test(vector[i].alg, 1, (unsigned char *)vector[i].std_msg, vector[i].c_bytes, (unsigned char *)vector[i].std_key, 0, (unsigned char *)vector[i].std_iv, vector[i].iv_bytes, (unsigned char *)vector[i].std_aad, vector[i].aad_bytes, (unsigned char *)vector[i].std_mac, 16);
        if (ret) {
            return 1;
        }

        ret = ske_gmac_test(vector[i].alg, 0, (unsigned char *)vector[i].std_msg, vector[i].c_bytes, (unsigned char *)vector[i].std_key, 0, (unsigned char *)vector[i].std_iv, vector[i].iv_bytes, (unsigned char *)vector[i].std_aad, vector[i].aad_bytes, (unsigned char *)vector[i].std_mac, vector[i].mac_bytes);
        if (ret) {
            return 1;
        }
...
unsigned char std_gmac_msg[256] = {
    0xd9,
    0x31,
    0x32,
    0x25,
...
unsigned char std_gmac_key[32] = {0xfe, 0xff, 0xe9, 0x92, 0x86, 0x65, 0x73, 0x1c, 0x6d, 0x6a, 0x8f, 0x94, 0x67, 0x30, 0x83, 0x08, 0xfe, 0xff, 0xe9, 0x92, 0x86, 0x65, 0x73, 0x1c, 0x6d, 0x6a, 0x8f, 0x94, 0x67, 0x30, 0x83, 0x08};
unsigned char std_gmac_iv[12]  = {0xca, 0xfe, 0xba, 0xbe, 0xfa, 0xce, 0xdb, 0xad, 0xde, 0xca, 0xf8, 0x88};
unsigned char std_gmac_aad[33] = {
    0xfe,
    0xed,
    0xfa,
    0xce,
