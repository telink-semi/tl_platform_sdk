...
unsigned int ske_xts_test(SKE_ALG alg, unsigned char wordAlign,  unsigned char *std_plain, unsigned int byteLen,  unsigned char *key,
        unsigned short sp_key_idx,  unsigned char *iv,  unsigned char *std_cipher)
{
    ...
    printf("\r\n %s XTS test(wordAlign is %u, P/C=%x bytes)", name[alg], wordAlign, byteLen);
    ...
    if(SKE_SUCCESS != ret)
    {
        printf("\r\n ske xts init 0 error, ret=%x", ret);
        return 1;
    }
    ...
    if(SKE_SUCCESS != ret)
    {
        printf("\r\n ske xts encrypt 0 error, ret=%x", ret);
    ...
    if(SKE_SUCCESS != ret)
    {
        printf("\r\n ske xts init 1 error, ret=%x", ret);
        return 1;
    }
    ...
    if(SKE_SUCCESS != ret)
    {
        printf("\r\n ske xts decrypt 1 error, ret=%x", ret);
        print_buf_U8(std_plain_, byteLen, "std_plain");
        print_buf_U8(cipher_, byteLen, "cipher");
    ...
    if(SKE_SUCCESS != ret)
    {
        printf("\r\n ske xts init 3 error, ret=%x", ret);
        return 1;
    }
...
unsigned int ske_xts_test_internal(SKE_XTS_TEST_VECTOR *vector, unsigned int num)
{
    ...
    for(i=0; i<num; i++)
    {
        ret = ske_xts_test(vector[i].alg, 1, vector[i].std_plain, vector[i].c_bytes, vector[i].key,
                vector[i].keyid, vector[i].i, vector[i].std_cipher);
        if(ret)
            return 1;

        ret = ske_xts_test(vector[i].alg, 0, vector[i].std_plain, vector[i].c_bytes, vector[i].key,
                vector[i].keyid, vector[i].i, vector[i].std_cipher);
        if(ret)
            return 1;
    }
 ...
 unsigned char std_aes128_xts_256_cipher[256] =  {
            ...
            ,};
 ...
 unsigned char std_aes128_xts_120_cipher[120] =  {
            ...
            0x01,0xB6,0xF4,0xA2,0x03,0x38,0xF3,0xCE,0xDB,0xF8,0x90,0x41,0x1C,0x7B,0xEA,0xC8,
            0x22,0x87,0x2D,0x15,0x40,0x59,0xFE,0x2C,0x39,0x4E,0x99,0x78,0x3C,0x8B,0x17,0x52,
            0x78,0x94,0x58,0xC2,0x15,0x65,0xD1,0xFC,0x47,0xB9,0xA9,0x6A,0x27,0x29,0xB9,0x45,
            0x1F,0xE7,0x2E,0x17,0xF4,0xD8,0xC1,0x5F,};

 unsigned char std_aes128_xts_65_cipher[65] =  {
            0x15,0xB9,0x63,0x88,0x26,0x0B,0x79,0x7A,0xEE,0xBD,0xD8,0xDC,0x30,0xA3,0x9B,0x49,
            0xCC,0xA5,0xF7,0x21,0x2D,0x33,0x70,0xA1,0x92,0xD2,0x5C,0x62,0x36,0x51,0xD9,0xB5,
            0x52,0x49,0xD7,0x36,0x6C,0x18,0x62,0x1C,0x3D,0x76,0x1A,0x85,0x2E,0xA3,0x5D,0x73,
            0xB5,0xB2,0x93,0x0C,0x00,0xD9,0xCF,0xC8,0x29,0x39,0xF7,0xC1,0x79,0xD1,0x18,0xD6,
            0xF3,};

 unsigned char std_aes128_xts_17_cipher[17] =  {
            0x8E,0x9B,0x62,0xF1,0xB7,0x0E,0x17,0xFE,0x7F,0xE1,0x76,0x53,0x23,0x85,0x0C,0xB8,
            0x15,};
...
unsigned int aes_128_xts_test(void)
{
 ...
 unsigned char std_aes192_xts_256_cipher[256] =  {
            ...
            ,};
 ...
 unsigned char std_aes192_xts_120_cipher[120] =  {
            ...
            0x57,0x74,0x7E,0x1D,0x0A,0x60,0x5F,0x5B,0xE0,0x4D,0x64,0x88,0xD4,0x81,0x0E,0x7A,
            0xBF,0x4B,0x9C,0x69,0x0B,0x7E,0x80,0xA2,0x2E,0xEB,0x19,0x81,0x68,0xCC,0xF6,0x28,
            0x9F,0x19,0x4D,0x4D,0xC1,0xFF,0xEF,0x40,0x6E,0xB4,0xAE,0xFF,0x74,0x5D,0x16,0x21,
            0x7C,0xCA,0x09,0x33,0xFD,0xEC,0x69,0x95,};

 unsigned char std_aes192_xts_65_cipher[65] =  {
            0xEA,0xCC,0x76,0xA5,0x82,0xDF,0x86,0x99,0x27,0xE1,0x1C,0x48,0x6B,0x7C,0xA1,0xFB,
            0x40,0x5E,0x76,0x75,0xBA,0x40,0xC3,0xF4,0x59,0xEC,0x45,0xD4,0x13,0x10,0x2B,0xE0,
            0xB8,0x63,0x39,0x45,0xC8,0xD3,0x80,0x7D,0x9D,0x06,0xEC,0xC6,0xEA,0xDF,0x6F,0xCC,
            0x00,0x6C,0xB9,0xCC,0x10,0xF4,0xB1,0x2D,0x39,0x78,0x54,0xB2,0xFF,0xE1,0x8B,0xCC,
            0x2A,};

 unsigned char std_aes192_xts_17_cipher[17] =  {
            0xDE,0x9B,0xE6,0xCA,0xC1,0xC9,0xBC,0xBB,0x5D,0x0A,0x37,0x81,0x6C,0x43,0xAC,0x0C,
            0xEA,};
...
unsigned int aes_192_xts_test(void)
{
 ...
 unsigned char std_aes256_xts_256_cipher[256] =  {
            ...
            ,};
 ...
 unsigned char std_aes256_xts_120_cipher[120] =  {
            ...
            0x5A,0x04,0xE1,0x99,0x02,0x3C,0x15,0x56,0x86,0x42,0x2E,0x92,0x66,0x1E,0xB9,0x90,
            0x5C,0xBF,0x21,0x4B,0x56,0x8D,0x34,0x71,0x5C,0x9E,0x16,0xDE,0xE2,0x99,0x52,0xC9,
            0xED,0x1F,0x54,0xA9,0x5C,0xE5,0x99,0x41,0x80,0x14,0x47,0x02,0x4F,0x68,0xB4,0x97,
            0x52,0xEA,0xF7,0x5B,0x90,0x6D,0x20,0xD8,};

 unsigned char std_aes256_xts_65_cipher[65] =  {
            0x4E,0x2B,0xA3,0xAD,0xA1,0xBC,0x79,0xAF,0x26,0x04,0xFC,0x2B,0x7D,0x71,0xC8,0xCB,
            0x71,0xFB,0x82,0x3E,0x0A,0x6B,0x34,0x31,0x77,0x37,0x05,0xD9,0xAD,0x86,0x0F,0x29,
            0xFE,0x7A,0x65,0x5A,0xCD,0x43,0x14,0xDD,0x46,0x74,0x6C,0xEF,0x79,0xFA,0xAE,0x71,
            0x96,0xB0,0xA7,0x62,0xB8,0x87,0x0D,0x12,0x13,0xCE,0x29,0xA3,0x85,0xC3,0x6D,0xEC,
            0x38,};

 unsigned char std_aes256_xts_17_cipher[17] =  {
            0x3D,0xAD,0x26,0x30,0xF4,0x29,0x49,0xF6,0x64,0x61,0x1E,0x8A,0xAD,0xB2,0xB0,0xF3,
            0x4E,};
...
unsigned int aes_256_xts_test(void)
{
 ...
 unsigned char std_sm4_xts_256_cipher[256] =  {
            ...
            ,};
 ...
 unsigned char std_sm4_xts_120_cipher[120] =  {
            ...
            0x74,0x7D,0x11,0xC3,0xFA,0x3B,0xBC,0x46,0x40,0xCA,0xDB,0x1A,0xD5,0xA7,0xAF,0xC6,
            0x6A,0x9F,0xD1,0xDD,0x0E,0xE9,0x08,0xD4,0xBA,0x10,0x5D,0xCE,0x43,0x2D,0x1B,0x92,
            0x69,0xB7,0xA3,0x54,0x1D,0x7A,0x66,0x01,0x9E,0x18,0x40,0xD7,0x54,0x76,0x27,0x5A,
            0x21,0xAC,0x0C,0x86,0x30,0x7D,0x8D,0x47,};

 unsigned char std_sm4_xts_65_cipher[65] =  {
            0x94,0x83,0xE9,0x1F,0x12,0xEE,0x81,0x81,0x1A,0x3C,0x4C,0xAB,0xAC,0xF4,0x01,0xA3,
            0x9D,0xBC,0x35,0xC2,0xE5,0x37,0x4D,0x69,0x73,0xDB,0x4D,0x79,0x32,0x10,0xC4,0x27,
            0x2A,0x54,0x17,0xE0,0x6D,0x86,0xCD,0xED,0xF6,0xEC,0x86,0x8E,0xDA,0x93,0xFE,0x67,
            0x0F,0x53,0xC0,0x9E,0x12,0x37,0xE9,0xBA,0x10,0x02,0xFB,0x3D,0x0E,0x7B,0x0A,0xA4,
            0xBC,};

 unsigned char std_sm4_xts_17_cipher[17] =  {
            0x83,0x29,0xD4,0xDF,0x72,0x77,0x63,0xDC,0xF8,0xA5,0xB3,0x4D,0x86,0x03,0x31,0x1B,
            0x94,};
