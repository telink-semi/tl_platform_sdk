                ...
                unsigned int byteLen, const unsigned char *key, unsigned short key_id, const unsigned char *iv, const unsigned char *std_cipher)
{
    ...
    if(wordAlign)
    {
        memcpy_(std_plain_buf, (const unsigned char *)std_plain, byteLen);
        memcpy_(std_cipher_buf, (const unsigned char *)std_cipher, byteLen);
        if(key != NULL)
            memcpy_(key_buf, (const unsigned char *)key, key_byteLen);
        if(SKE_MODE_ECB != mode)
            memcpy_(iv_buf, (const unsigned char *)iv, block_byteLen);
    ...
    {
        memcpy_(std_plain_buf + 1, (const unsigned char *)std_plain, byteLen);
        memcpy_(std_cipher_buf + 1, (const unsigned char *)std_cipher, byteLen);
        if(key != NULL)
            memcpy_(key_buf + 1, (const unsigned char *)key, key_byteLen);
        if(SKE_MODE_ECB != mode)
            memcpy_(iv_buf + 1, (const unsigned char *)iv, block_byteLen);
...
unsigned int SKE_DMA_test(SKE_ALG ske_alg, SKE_MODE mode, unsigned char *std_plain,
                unsigned int wordLen, unsigned char *key, unsigned short key_id, unsigned char *iv, const unsigned char *std_cipher)
{
    unsigned int i,j;
...
unsigned int SM4_Test(void)
{
    ...
    ret  = SKE_test(SKE_ALG_SM4, SKE_MODE_ECB, 1, std_in, 128, (const unsigned char*)std_key, 0, NULL,   std_sm4_ecb_out);
#endif
