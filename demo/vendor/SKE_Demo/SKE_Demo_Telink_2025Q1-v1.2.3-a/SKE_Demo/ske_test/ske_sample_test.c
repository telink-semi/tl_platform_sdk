...
/**
 * @brief           Test AES-128 ECB mode with a single block
...
unsigned int ske_lp_aes128_ecb_sample(void)
{
    unsigned char std_in[48] = {
        0x81, 0x70, 0x99, 0x44, 0xE0, 0xCB, 0x2E, 0x1D, 0xB5, 0xB0, 0xA4, 0x77, 0xD1, 0xA8, 0x53, 0x9B, 0x0A, 0x87, 0x86, 0xE3, 0x4E, 0xAA, 0xED, 0x99,
        0x30, 0x3E, 0xA6, 0x97, 0x55, 0x95, 0xB2, 0x45, 0x4D, 0x5D, 0x7F, 0x91, 0xEB, 0xBD, 0x4A, 0xCD, 0x72, 0x6C, 0x0E, 0x0E, 0x5E, 0x3E, 0xB5, 0x5E,
    };
    ...
    unsigned char std_cipher[48] = {
        0x0B, 0x54, 0xE5, 0x9F, 0x47, 0x14, 0x4F, 0xD8, 0xEE, 0x43, 0x56, 0xCA, 0x0B, 0x2D, 0x7A, 0x4B, 0x84, 0xD8, 0x17, 0x26, 0xE2, 0x8F, 0x59, 0xAD,
        0x95, 0x56, 0x8C, 0x52, 0xDA, 0x98, 0x3F, 0x8D, 0x65, 0xED, 0xDD, 0x87, 0x58, 0x90, 0xEC, 0xE7, 0xD8, 0x03, 0x66, 0x9D, 0xE2, 0xF3, 0x57, 0x95,
    };
    ...
    if (SKE_SUCCESS != ret || uint32_big_num_cmp(out, 48 / 4, std_out_, 48 / 4)) {
    ...
    if (SKE_SUCCESS != ret || uint32_big_num_cmp(out2, 48 / 4, std_in_, 48 / 4)) {
...
unsigned int ske_lp_aes128_cbc_sample(void)
{
    ...
    unsigned char std_cipher[48] = {
        0x2C, 0x1E, 0xD4, 0x56, 0x36, 0x2E, 0x00, 0x85, 0xA8, 0x1D, 0x8E, 0x61, 0x69, 0xAD, 0x38, 0xB7, 0xB4, 0x42, 0x60, 0xE1, 0x56, 0x8D, 0x9E, 0x85,
        0x0A, 0x0C, 0x95, 0x37, 0x44, 0x02, 0xDE, 0x28, 0x24, 0xD5, 0x05, 0x61, 0x30, 0x42, 0x42, 0x86, 0x0A, 0xE7, 0x17, 0x3D, 0xDD, 0x19, 0xEC, 0x5B,
    };
...
unsigned int ske_lp_aes128_gcm_sample(void)
{
    ...
    unsigned char std_cipher[] = {0x42, 0x83, 0x1e, 0xc2, 0x21, 0x77, 0x74, 0x24, 0x4b, 0x72, 0x21, 0xb7, 0x84, 0xd0, 0xd4, 0x9c, 0xe3, 0xaa, 0x21, 0x2f, 0x2c, 0x02,
    ...
    if (SKE_SUCCESS != ret || uint32_big_num_cmp(out, 64 / 4, (unsigned int *)std_cipher, 64 / 4) || uint32_big_num_cmp(out + 64 / 4, 4, (unsigned int *)std_mac, 4)) {
    ...
    if (SKE_SUCCESS != ret || uint32_big_num_cmp(out2, 64 / 4, (unsigned int *)std_plain, 64 / 4) || uint32_big_num_cmp(out2 + 64 / 4, 4, (unsigned int *)std_mac, 4)) {
    ...
    if (SKE_SUCCESS != ret || uint32_big_num_cmp(out, 64 / 4, (unsigned int *)std_cipher, 64 / 4) || uint32_big_num_cmp(out2 + 64 / 4, 4, (unsigned int *)std_mac, 4)) {
        printf("%s line-%d AES128 gcm dma encryption failure, ret=%u\r\n", __func__, __LINE__, ret);
        fflush(stdout);
