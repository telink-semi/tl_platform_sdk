...
/**
 * @brief           Internal test function for AES-XCBC-MAC-96
 ...
 */
unsigned int ske_aes_xcbc_mac_96_test_internal(const unsigned char *std_key, unsigned short sp_key_idx, const unsigned char *std_mac1, const unsigned char *std_mac2,
                                               const unsigned char *std_mac3)
{
    ...
    unsigned int len[5][15] = //sum of each line is 500
        {
            ...
            {59, 111, 29, 34, 27, 89, 9, 6, 79, 22, 34, 1, 0, 0, 0}, {333, 3, 4, 39, 77, 16, 24, 4, 0, 0, 0, 0, 0, 0, 0},
        };
    unsigned int ret;
    ...
    /*************************** test1 ***************************/
    //generate mac
    ret = ske_lp_aes_xcbc_mac_96_init(ctx, SKE_GENERATE_MAC, std_key, sp_key_idx);
    ...
    if (SKE_SUCCESS != ret || memcmp_(mac, std_mac1, mac_bytes)) {
        ...
        print_buf_u8(message, 3, "message");
        print_buf_u8(std_key, ske_lp_get_key_byte_len(SKE_ALG_AES_128), "key");
        print_buf_u8(mac, mac_bytes, "mac");
        return 1;
    } else {
    ...
    //verify mac
    ret = ske_lp_aes_xcbc_mac_96_init(ctx, SKE_VERIFY_MAC, std_key, sp_key_idx);
    ret |= ske_lp_aes_xcbc_mac_96_update(ctx, message, 3);
    ret |= ske_lp_aes_xcbc_mac_96_final(ctx, mac);
    if (SKE_SUCCESS != ret) {
    ...
    //generate mac
#if 1
    ret = ske_lp_aes_xcbc_mac_96(SKE_GENERATE_MAC, std_key, sp_key_idx, message, 500, mac);
#else
    ret = ske_lp_aes_xcbc_mac_96_init(ctx, SKE_GENERATE_MAC, std_key, sp_key_idx);
    ...
    //verify mac
#if 1
    ret = ske_lp_aes_xcbc_mac_96(SKE_VERIFY_MAC, std_key, sp_key_idx, message, 500, mac);
#else
    ret = ske_lp_aes_xcbc_mac_96_init(ctx, SKE_VERIFY_MAC, std_key, sp_key_idx);
    ret |= ske_lp_aes_xcbc_mac_96_update(ctx, message, 500);
    ret |= ske_lp_aes_xcbc_mac_96_final(ctx, mac);
...
/**
 * @brief           Speed test function for AES-XCBC-MAC-96
 * @return          0 if successful, otherwise error code
 */
...
/**
 * @brief           Internal DMA test function for AES-XCBC-MAC-96
 * @param[in]       std_key              - Pointer to the standard key
 * @param[in]       sp_key_idx           - Special key index (if applicable)
 * @param[in]       std_mac1             - Pointer to the first standard MAC value
 * @param[in]       std_mac2             - Pointer to the second standard MAC value
 * @param[in]       std_mac3             - Pointer to the third standard MAC value
 * @param[in]       callback             - Callback function to handle DMA completion
 * @return          0 if successful, otherwise error code
 */
unsigned int ske_dma_aes_xcbc_mac_96_test_internal(const unsigned char *std_key, unsigned short sp_key_idx, const unsigned char *std_mac1, const unsigned char *std_mac2,
                                                   const unsigned char *std_mac3, SKE_CALLBACK callback)
{
...
#if 0
    ret = ske_lp_dma_aes_xcbc_mac_96(std_key, sp_key_idx, msg, 3, tmp_out_, mac, ske_call_manage);
#else
    ret = ske_lp_dma_aes_xcbc_mac_96_init(ctx, std_key, sp_key_idx);
...
//the two styles are equivalent
#if 0
    ret = ske_lp_dma_aes_xcbc_mac_96(std_key, sp_key_idx, msg, 500, tmp_out_, mac, ske_call_manage);
#else
    ret = ske_lp_dma_aes_xcbc_mac_96_init(ctx, std_key, sp_key_idx);
    ...
    if (SKE_SUCCESS != ret || memcmp_(std_mac2, mac, 0x0C)) {
        printf("\r\n %s AES_XCBC_MAC_96 DMA test 2 failure, ret=%x \r\n", name, ret);
        fflush(stdout);
        print_buf_u8(message, 500, "message");
        print_buf_u8((unsigned char *)mac, 0x0C, "mac");
        print_buf_u8(std_mac2, 0x0C, "std mac");
        return 1;
    } else {
    ...
    for (i = 0; i <= 500; i++) {
        ...
        ret |= ske_lp_dma_aes_xcbc_mac_96(std_key, sp_key_idx, msg, i, tmp_out_, mac, ske_call_manage);
        memcpy_(message, mac, 0x0C);
    }

    if (SKE_SUCCESS != ret || memcmp_(std_mac3, mac, 0x0C)) {
        printf("\r\n %s AES_XCBC_MAC_96 DMA test 3 failure, ret=%x \r\n", name, ret);
        fflush(stdout);
        //print_buf_u8(message, 500, "message");
        print_buf_u8((unsigned char *)mac, 0x0C, "mac");
        print_buf_u8(std_mac3, 0x0C, "std mac");
        return 1;
    } else {
...
/**
 * @brief           Speed test function for AES-XCBC-MAC-96 using DMA
 * @return          0 if successful, otherwise error code
 */
...
const unsigned char std_aes_xcbc_mac_96_key[16] = {
    0xE0, 0x70, 0x99, 0xF1, 0xBF, 0xAF, 0xFD, 0x7F, 0x24, 0x0C, 0xD7, 0x90, 0xCA, 0x4F, 0xE1, 0x34,
};
const unsigned char std_aes_xcbc_mac_96_mac1[12] = {
    0x91, 0x1D, 0xCC, 0xAC, 0xBA, 0x0B, 0x4B, 0x4C, 0x29, 0x84, 0x32, 0x8F,
};
const unsigned char std_aes_xcbc_mac_96_mac2[12] = {
    0x1B, 0xAE, 0x78, 0xC8, 0x3A, 0x93, 0x13, 0xD0, 0x4E, 0xD0, 0x03, 0xB9,
};
const unsigned char std_aes_xcbc_mac_96_mac3[12] = {
    0x4B, 0x97, 0x99, 0x36, 0x70, 0x28, 0x58, 0x8D, 0x87, 0x2C, 0x7A, 0xCE,
};

/**
 * @brief           Test function for AES-XCBC-MAC-96
 * @return          0 if successful, otherwise error code
 */
unsigned int aes_xcbc_mac_96_test(void)
{
    ...
    ret = ske_aes_xcbc_mac_96_test_internal(std_aes_xcbc_mac_96_key, 0, std_aes_xcbc_mac_96_mac1, std_aes_xcbc_mac_96_mac2, std_aes_xcbc_mac_96_mac3);
    if (ret) {
        return 1;
    }

#ifdef SKE_LP_DMA_FUNCTION
    ret = ske_dma_aes_xcbc_mac_96_test_internal(std_aes_xcbc_mac_96_key, 0, std_aes_xcbc_mac_96_mac1, std_aes_xcbc_mac_96_mac2, std_aes_xcbc_mac_96_mac3, ske_call_manage);
    if (ret) {
        return 1;
    }
#endif
...
/**
 * @brief           Sample test function for AES-XCBC-MAC-96
 * @return          0 if successful, otherwise error code
 */
unsigned int aes_xcbc_mac_96_sample_test(void)
{
    ...
    unsigned char std_aes_xcbc_mac_96_mac[6][12] = {
        {0x75, 0xF0, 0x25, 0x1D, 0x52, 0x8A, 0xC0, 0x1C, 0x45, 0x73, 0xDF, 0xD5}, {0x5B, 0x37, 0x65, 0x80, 0xAE, 0x2F, 0x19, 0xAF, 0xE7, 0x21, 0x9C, 0xEE},
        {0xD2, 0xA2, 0x46, 0xFA, 0x34, 0x9B, 0x68, 0xA7, 0x99, 0x98, 0xA4, 0x39}, {0x47, 0xF5, 0x1B, 0x45, 0x64, 0x96, 0x62, 0x15, 0xB8, 0x98, 0x5C, 0x63},
        {0xF5, 0x4F, 0x0E, 0xC8, 0xD2, 0xB9, 0xF3, 0xD3, 0x68, 0x07, 0x73, 0x4B}, {0xBE, 0xCB, 0xB3, 0xBC, 0xCD, 0xB5, 0x18, 0xA3, 0x06, 0x77, 0xD5, 0x48},
    };
    ...
    ske_aes_xcbc_mac_96_ctx_t     ctx[1];
    ske_aes_xcbc_mac_96_dma_ctx_t ctx2[1];
    ...
    for (i = 0; i < vector_num; i++) {
        ...
        ret = ske_lp_aes_xcbc_mac_96(SKE_VERIFY_MAC, std_key, sp_key_idx, std_aes_xcbc_mac_96_plain, vector_bytes[i], mac);
        if (SKE_SUCCESS != ret) {
    ...
    ret = ske_lp_aes_xcbc_mac_96(SKE_VERIFY_MAC, std_key, sp_key_idx, std_aes_xcbc_mac_96_plain_large, 1000, mac);
    if (SKE_SUCCESS != ret) {
        printf("\r\n verify AES_XCBC_MAC_96 test %u failure, ret=%x \r\n", i + 1, ret);
        print_buf_u8(mac, 0x0C, "mac");
        return 1;
    ...
    for (i = 0; i < vector_num; i++) {
        if (vector_bytes[i]) {
            ...
            ret = ske_lp_dma_aes_xcbc_mac_96_init(ctx2, std_key, sp_key_idx);
...
#else
    ret = ske_lp_dma_aes_xcbc_mac_96(std_key, sp_key_idx, std_in_, 1000, tmp_out_, mac2, ske_call_manage);
#endif
...
/**
 * @brief           Comprehensive test function for AES-XCBC-MAC-96
 * @return          0 if all tests pass, otherwise error code
 */
