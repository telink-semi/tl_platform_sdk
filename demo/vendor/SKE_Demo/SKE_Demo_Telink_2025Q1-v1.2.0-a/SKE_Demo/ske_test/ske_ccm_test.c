...
/**
 * @brief           Tests the CCM (Counter with CBC-MAC) mode implementation.
 * @param[in]       alg                  - SKE algorithm (AES or SM4).
 * @param[in]       wordAlign            - Whether to align data to word boundaries.
 * @param[in]       std_plain            - Standard plaintext data.
 * @param[in]       byteLen              - Length of plaintext/ciphertext in bytes.
 * @param[in]       key                  - Encryption key.
 * @param[in]       sp_key_idx           - Secure port key index.
 * @param[in]       nonce                - Nonce value.
 * @param[in]       M                    - Length of MAC in bytes.
 * @param[in]       L                    - Length of nonce in bytes.
 * @param[in]       aad                  - Additional authenticated data (AAD).
 * @param[in]       aad_len              - Length of AAD in bytes.
 * @param[in]       std_cipher           - Standard ciphertext data.
 * @param[in]       std_mac              - Standard MAC value.
 * @return          SKE_SUCCESS if successful, otherwise error code.
 */
unsigned int ske_ccm_test(ske_alg_e alg, unsigned char wordAlign, const unsigned char *std_plain, unsigned int byteLen, const unsigned char *key, unsigned short sp_key_idx,
                          const unsigned char *nonce, unsigned char M, unsigned char L, const unsigned char *aad, unsigned char aad_len, const unsigned char *std_cipher,
                          const unsigned char *std_mac)
{
    ...
    printf("\r\n %s CCM test(M=%u bytes(mac bytes),L=%u bytes(nonce bytes is %u),AAD=%u bytes,P/C=%u bytes)", name[alg], M, L, 15u - L, aad_len, byteLen);
...
#if 1
    // ENCRYPT
    ret = ske_lp_ccm_init(ctx, alg, SKE_CRYPTO_ENCRYPT, key_, sp_key_idx, nonce_, M, L, aad_len, byteLen);
    if (SKE_SUCCESS != ret)
    {
    ...
    // DECRYPT
    ret = ske_lp_ccm_init(ctx, alg, SKE_CRYPTO_DECRYPT, key_, sp_key_idx, nonce_, M, L, aad_len, byteLen);
    if (SKE_SUCCESS != ret)
    {
    ...
    // ENCRYPT
    ret = ske_lp_ccm_crypto(alg, SKE_CRYPTO_ENCRYPT, key_, sp_key_idx, nonce_, M, L, aad_, aad_len, std_plain_, cipher_, byteLen, mac_);
    if (SKE_SUCCESS != ret)
    {
    ...
    // DECRYPT
    ret = ske_lp_ccm_crypto(alg, SKE_CRYPTO_DECRYPT, key_, sp_key_idx, nonce_, M, L, aad_, aad_len, cipher_, replain_, byteLen, mac_);
    if (SKE_SUCCESS != ret)
    {
...
/**
 * @brief           Tests the DMA-based CCM mode implementation.
 * @param[in]       alg                  - SKE algorithm (AES or SM4).
 * @param[in]       std_plain            - Standard plaintext data.
 * @param[in]       byteLen              - Length of plaintext/ciphertext in bytes.
 * @param[in]       key                  - Encryption key.
 ...
 * @return          SKE_SUCCESS if successful, otherwise error code.
 */
unsigned int ske_dma_ccm_test(ske_alg_e alg, const unsigned char *std_plain, unsigned int byteLen, const unsigned char *key, unsigned short sp_key_idx, const unsigned char *nonce,
                              unsigned char M, unsigned char L, const unsigned char *aad, unsigned char aad_len, const unsigned char *std_cipher, const unsigned char *std_mac)
{
    ...
    printf("\r\n %s CCM DMA test(M=%u bytes(mac bytes),L=%u bytes(nonce bytes is %u),AAD=%u bytes,P/C=%u bytes)", name[alg], M, L, 15u - L, aad_len, byteLen);
    fflush(stdout);
    ...
    ske_lp_ccm_get_B0(nonce, M, L, aad_len, byteLen, (unsigned char *)in);
    if (aad_len)
    {
        ske_lp_ccm_get_B1(aad, aad_len, &aad_offset, (unsigned char *)(in + 4));
        if (aad_offset == aad_len)
        {
            uint32_copy(in + 8, (unsigned int *)std_plain_buf, ((byteLen + 15) / 16) * 4);
            aad_with_prefix_bytes = 2 + aad_len;
        }
        else
        {
            tmp_len = aad_len - aad_offset;
#if 0
    ...
    {
        ret = ske_lp_dma_ccm_crypto(alg, SKE_CRYPTO_ENCRYPT, key, sp_key_idx, nonce, M, L, aad_len, in, out, byteLen, ske_call_manage);
    }
    else
    {
        ret = ske_lp_dma_ccm_init(ctx, alg, SKE_CRYPTO_ENCRYPT, key, sp_key_idx, nonce, M, L, aad_len, byteLen);
        if (SKE_SUCCESS != ret)
        {
        ...
        memcmp_(out + ((byteLen + 15) / 16) * 4, std_mac_buf, M))
    {
        ...
        print_buf_u8((const unsigned char *)std_mac_buf, M, "std_mac");
        print_buf_u8((unsigned char *)in, 16 + (aad_len ? ((aad_len + 15) / 16) * 16 : 0) + ((byteLen + 15) / 16) * 16, "in");
    ...
    if (aad_len && (0 == byteLen))
    {
        ret = ske_lp_dma_ccm_crypto(alg, SKE_CRYPTO_DECRYPT, key, sp_key_idx, nonce, M, L, aad_len, in, out, byteLen, ske_call_manage);
    }
    else
    {
        ret = ske_lp_dma_ccm_init(ctx, alg, SKE_CRYPTO_DECRYPT, key, sp_key_idx, nonce, M, L, aad_len, byteLen);
        if (SKE_SUCCESS != ret)
        {
            printf("\r\n ske ccm decrypt 1 error, ret=%u", ret);
            fflush(stdout);
...
typedef struct
{
    ...
    const unsigned char *std_mac;
} ske_ccm_test_vector_t;

/**
 * @brief           Internal function to test CCM mode with multiple test vectors.
 ...
 */
unsigned int ske_ccm_test_internal(ske_ccm_test_vector_t *vector, unsigned int num)
{
    char *name[] = {"", "", "", "", "", "AES_128", "AES_192", "AES_256", "SM4"};
    unsigned int i, ret;
    ...
    for (i = 0; i < num; i++)
    {
        ret = ske_ccm_test(vector[i].alg, 1, vector[i].std_plain, vector[i].c_bytes, vector[i].key, vector[i].keyid, vector[i].nonce, vector[i].M, vector[i].L, vector[i].aad,
                           vector[i].aad_bytes, vector[i].std_cipher, vector[i].std_mac);
        if (ret)
        {
            return 1;
        }

        ret = ske_ccm_test(vector[i].alg, 0, vector[i].std_plain, vector[i].c_bytes, vector[i].key, vector[i].keyid, vector[i].nonce, vector[i].M, vector[i].L, vector[i].aad,
                           vector[i].aad_bytes, vector[i].std_cipher, vector[i].std_mac);
        if (ret)
        {
            return 1;
...
#ifdef SKE_LP_DMA_FUNCTION
        ret = ske_dma_ccm_test(vector[i].alg, vector[i].std_plain, vector[i].c_bytes, vector[i].key, vector[i].keyid, vector[i].nonce, vector[i].M, vector[i].L, vector[i].aad,
                               vector[i].aad_bytes, vector[i].std_cipher, vector[i].std_mac);
        if (ret)
        {
            return 1;
...
const unsigned char std_ccm_plain[] = {
    0x81,
    0x70,
    0x99,
    0x44,
...
const unsigned char std_ccm_key[32] = {0xE0, 0x70, 0x99, 0xF1, 0xBF, 0xAF, 0xFD, 0x7F, 0x24, 0x0C, 0xD7, 0x90, 0xCA, 0x4F, 0xE1, 0x34,
                                       0xB4, 0x42, 0x60, 0xE1, 0x56, 0x8D, 0x9E, 0x85, 0x0A, 0x0C, 0x95, 0x37, 0x44, 0x02, 0xDE, 0x28};

const unsigned char std_ccm_nonce[] = {0xC7, 0x2B, 0x65, 0x91, 0xA0, 0xD7, 0xDE, 0x8F, 0x6B, 0x40, 0x72, 0x33, 0xAD, 0x35, 0x81, 0xD6};

const unsigned char std_ccm_aad[65] = {
    0xAE,
    0x30,
    0x14,
    0xAA,
...
const unsigned char std_aes128_ccm_mac_M16_L2_aad0_p256[] = {0x55, 0xEA, 0x2A, 0x8F, 0x0A, 0x72, 0x1F, 0x7D, 0xFA, 0xD4, 0xB9, 0x77, 0xC1, 0x0D, 0xB3, 0x8A};
const unsigned char std_aes128_ccm_mac_M14_L3_aad3_p201[] = {0x8D, 0x8D, 0x3D, 0xBC, 0xF9, 0x71, 0xB4, 0xDE, 0x3B, 0xE1, 0x13, 0xB1, 0xAD, 0x21};
const unsigned char std_aes128_ccm_mac_M12_L4_aad15_p120[] = {0xAD, 0xF4, 0xF9, 0x42, 0x9C, 0x32, 0xCA, 0x51, 0xE2, 0xCD, 0x9D, 0xCB};
const unsigned char std_aes128_ccm_mac_M10_L5_aad16_p65[] = {0xEF, 0x67, 0xD8, 0xE8, 0x7C, 0x4B, 0xA5, 0x72, 0x7A, 0x96};
const unsigned char std_aes128_ccm_mac_M8_L6_aad17_p32[] = {0x02, 0xED, 0x46, 0xC9, 0xF9, 0xF0, 0xF1, 0x47};
const unsigned char std_aes128_ccm_mac_M6_L7_aad32_p15[] = {
    0xCF,
    0x8D,
    0x20,
    0x41,
...
const unsigned char std_aes128_ccm_mac_M4_L8_aad33_p0[] = {0x72, 0xF4, 0xA8, 0x70};
const unsigned char std_aes128_ccm_mac_M16_L2_aad65_p256[] = {0x0F, 0x4C, 0xA8, 0x4D, 0x80, 0xA4, 0xE6, 0xB2, 0xD3, 0x3F, 0x2E, 0x2F, 0x61, 0x7A, 0x07, 0xF3};

const unsigned char std_aes128_ccm_L2_cipher[256] = {
    0xAC,
    0x5B,
    0xCC,
    0x7B,
...
const unsigned char std_aes128_ccm_L3_cipher[201] = {
    0x2F,
    0x95,
    0xD2,
    0x46,
...
const unsigned char std_aes128_ccm_L4_cipher[120] = {
    0x1B,
    0xDE,
    0x2B,
    0x30,
...
const unsigned char std_aes128_ccm_L5_cipher[65] = {
    0x67,
    0x33,
    0x4E,
    0xD9,
...
const unsigned char std_aes128_ccm_L6_cipher[32] = {
    0x31,
    0x66,
    0x94,
    0x79,
...
const unsigned char std_aes128_ccm_L7_cipher[15] = {
    0x59,
    0xBE,
    0x56,
    0x1D,
...
/**
 * @brief           Tests AES-128 CCM mode with various test vectors.
 * @return          SKE_SUCCESS if all tests pass, otherwise error code.
 */
unsigned int aes_128_ccm_test(void)
{
    ske_ccm_test_vector_t vector[8] = {
...
const unsigned char std_aes192_ccm_mac_M16_L2_aad0_p256[] = {0x38, 0x46, 0xF9, 0xCA, 0x7E, 0x38, 0x73, 0x86, 0xB7, 0xB3, 0xF8, 0x27, 0xC0, 0xF5, 0xAE, 0x85};
const unsigned char std_aes192_ccm_mac_M14_L3_aad3_p201[] = {0x17, 0xA2, 0xE6, 0xFC, 0xF9, 0x86, 0xA8, 0x87, 0xE8, 0x03, 0x41, 0x75, 0x56, 0xD5};
const unsigned char std_aes192_ccm_mac_M12_L4_aad15_p120[] = {0x98, 0x63, 0x22, 0x56, 0x54, 0x11, 0xED, 0x89, 0x50, 0x84, 0x55, 0x55};
const unsigned char std_aes192_ccm_mac_M10_L5_aad16_p65[] = {0x61, 0x21, 0xA7, 0x6B, 0x4E, 0xEF, 0x54, 0xAC, 0xB7, 0xB5};
const unsigned char std_aes192_ccm_mac_M8_L6_aad17_p32[] = {0x28, 0x40, 0x84, 0x96, 0x2B, 0xE4, 0x08, 0xD6};
const unsigned char std_aes192_ccm_mac_M6_L7_aad32_p15[] = {
    0x3A,
    0x46,
    0x59,
    0x0A,
...
const unsigned char std_aes192_ccm_mac_M4_L8_aad33_p0[] = {0x1A, 0x53, 0x07, 0x6F};
const unsigned char std_aes192_ccm_mac_M16_L2_aad65_p256[] = {0xF4, 0xED, 0x57, 0x02, 0x61, 0xAA, 0x23, 0x66, 0xA7, 0xDF, 0x8D, 0x49, 0xA9, 0xC2, 0x88, 0xF1};

const unsigned char std_aes192_ccm_L2_cipher[256] = {
    0xAF,
    0xB0,
    0x9E,
    0x94,
...
const unsigned char std_aes192_ccm_L3_cipher[201] = {
    0xFE,
    0xA4,
    0x8A,
    0xA4,
...
const unsigned char std_aes192_ccm_L4_cipher[120] = {
    0x71,
    0xBF,
    0xE7,
    0x42,
...
const unsigned char std_aes192_ccm_L5_cipher[65] = {
    0xCD,
    0x5C,
    0xB5,
    0x53,
...
const unsigned char std_aes192_ccm_L6_cipher[32] = {
    0xD5,
    0xF6,
    0x4B,
    0x51,
...
const unsigned char std_aes192_ccm_L7_cipher[15] = {
    0x4A,
    0xAF,
    0xA8,
    0x09,
...
/**
 * @brief           Tests AES-192 CCM mode with various test vectors.
 * @return          SKE_SUCCESS if all tests pass, otherwise error code.
 */
...
const unsigned char std_aes256_ccm_mac_M16_L2_aad0_p256[] = {0x5B, 0x95, 0x7A, 0x4F, 0xC0, 0x05, 0xD4, 0xA0, 0x75, 0x46, 0xCC, 0xA7, 0xA6, 0x8C, 0x5D, 0xF4};
const unsigned char std_aes256_ccm_mac_M14_L3_aad3_p201[] = {0x0D, 0x0D, 0x80, 0xE1, 0xDB, 0x0C, 0xB9, 0x0C, 0x58, 0x41, 0xF9, 0x04, 0xD0, 0xAD};
const unsigned char std_aes256_ccm_mac_M12_L4_aad15_p120[] = {0xDA, 0xA3, 0x68, 0xFF, 0x53, 0xEC, 0x4D, 0xC0, 0x7B, 0xA3, 0xF5, 0xAD};
const unsigned char std_aes256_ccm_mac_M10_L5_aad16_p65[] = {0x5D, 0x7E, 0x79, 0x27, 0xAD, 0x13, 0xED, 0x47, 0x25, 0xCA};
const unsigned char std_aes256_ccm_mac_M8_L6_aad17_p32[] = {0x44, 0x8A, 0x84, 0xC5, 0x5F, 0xB1, 0x88, 0xA6};
const unsigned char std_aes256_ccm_mac_M6_L7_aad32_p15[] = {
    0x05,
    0x8A,
    0x77,
    0xD3,
...
const unsigned char std_aes256_ccm_mac_M4_L8_aad33_p0[] = {0xBA, 0x44, 0x43, 0x17};
const unsigned char std_aes256_ccm_mac_M16_L2_aad65_p256[] = {0xFA, 0x18, 0x05, 0xA8, 0xEE, 0xEC, 0x1A, 0x21, 0x01, 0x0E, 0x2D, 0xE9, 0xCE, 0x82, 0x83, 0xE8};

const unsigned char std_aes256_ccm_L2_cipher[256] = {
    0xB3,
    0x5E,
    0x02,
    0xD0,
...
const unsigned char std_aes256_ccm_L3_cipher[201] = {
    0x58,
    0xCF,
    0x26,
    0xE1,
...
const unsigned char std_aes256_ccm_L4_cipher[120] = {
    0x69,
    0x08,
    0x23,
    0x77,
...
const unsigned char std_aes256_ccm_L5_cipher[65] = {
    0xF5,
    0xCA,
    0x16,
    0x09,
...
const unsigned char std_aes256_ccm_L6_cipher[32] = {
    0xFD,
    0xDD,
    0xF0,
    0x27,
...
const unsigned char std_aes256_ccm_L7_cipher[15] = {
    0xC8,
    0xF0,
    0xAB,
    0xC4,
...
/**
 * @brief           Tests AES-256 CCM mode with various test vectors.
 * @return          SKE_SUCCESS if all tests pass, otherwise error code.
 */
...
const unsigned char std_sm4_ccm_mac_M16_L2_aad0_p256[] = {0x11, 0xDE, 0xC4, 0xDD, 0xB4, 0x4B, 0x3E, 0xEC, 0x67, 0xBB, 0x9F, 0x68, 0x0E, 0x67, 0x3F, 0x68};
const unsigned char std_sm4_ccm_mac_M14_L3_aad3_p201[] = {0x22, 0xAB, 0x10, 0xA7, 0x35, 0x96, 0x32, 0xE5, 0x69, 0xA5, 0x01, 0xCA, 0x71, 0x0C};
const unsigned char std_sm4_ccm_mac_M12_L4_aad15_p120[] = {0x3F, 0xDF, 0x2B, 0x89, 0xB1, 0xC7, 0x9C, 0xC3, 0x25, 0x49, 0x47, 0x09};
const unsigned char std_sm4_ccm_mac_M10_L5_aad16_p65[] = {0x96, 0x84, 0x8A, 0x88, 0xF5, 0x68, 0x06, 0x99, 0x9E, 0xCD};
const unsigned char std_sm4_ccm_mac_M8_L6_aad17_p32[] = {0x91, 0xE2, 0x0B, 0x03, 0xFD, 0x87, 0xB4, 0x32};
const unsigned char std_sm4_ccm_mac_M6_L7_aad32_p15[] = {
    0x49,
    0xBE,
    0x2F,
    0xD6,
...
const unsigned char std_sm4_ccm_mac_M4_L8_aad33_p0[] = {0xB6, 0xD6, 0xD2, 0xD9};
const unsigned char std_sm4_ccm_mac_M16_L2_aad65_p256[] = {0xD5, 0x8F, 0xDC, 0x6B, 0xAD, 0xEF, 0x8A, 0xDC, 0x9A, 0x5A, 0x34, 0x37, 0x93, 0xAD, 0xBF, 0xDA};

const unsigned char std_sm4_ccm_L2_cipher[256] = {
    0xBA,
    0x5C,
    0x58,
    0xCB,
...
const unsigned char std_sm4_ccm_L3_cipher[201] = {
    0x37,
    0xDA,
    0x7B,
    0x2D,
...
const unsigned char std_sm4_ccm_L4_cipher[120] = {
    0x7A,
    0x88,
    0xBD,
    0xD9,
...
const unsigned char std_sm4_ccm_L5_cipher[65] = {
    0x99,
    0xB9,
    0x71,
    0xD0,
...
const unsigned char std_sm4_ccm_L6_cipher[32] = {
    0xCB,
    0xF3,
    0xC2,
    0x5F,
...
const unsigned char std_sm4_ccm_L7_cipher[15] = {
    0x5B,
    0x0D,
    0x74,
    0xBF,
...
/**
 * @brief           Tests SM4 CCM mode with various test vectors.
 * @return          SKE_SUCCESS if all tests pass, otherwise error code.
 */
...
/**
 * @brief           Internal function to test the speed of CCM mode for a given algorithm and operation.
 * @param[in]       alg                  - SKE algorithm (AES or SM4).
 * @param[in]       crypto               - Encryption or decryption operation.
 * @param[in]       info                 - Description of the test being performed.
 * @return          SKE_SUCCESS if successful, otherwise error code.
 */
unsigned int ske_ccm_speed_test_internal(ske_alg_e alg, ske_crypto_e crypto, char *info)
{
...
/**
 * @brief           Tests the speed of CCM mode for various algorithms and operations.
 * @return          SKE_SUCCESS if all tests pass, otherwise error code.
 */
...
#ifdef SKE_LP_DMA_FUNCTION
/**
 * @brief           Internal function to test the speed of DMA-based CCM mode for a given algorithm and operation.
 * @param[in]       alg                  - SKE algorithm (AES or SM4).
 * @param[in]       crypto               - Encryption or decryption operation.
 * @param[in]       info                 - Description of the test being performed.
 * @return          SKE_SUCCESS if successful, otherwise error code.
 */
unsigned int ske_dma_ccm_speed_test_internal(ske_alg_e alg, ske_crypto_e crypto, char *info)
{
...
/**
 * @brief           Tests the speed of DMA-based CCM mode for various algorithms and operations.
 * @return          SKE_SUCCESS if all tests pass, otherwise error code.
 */
