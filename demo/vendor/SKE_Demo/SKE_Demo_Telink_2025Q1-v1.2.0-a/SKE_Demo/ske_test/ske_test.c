 ...
 * @param[out]      rand                 - Buffer to store generated random bytes
 * @param[in]       bytes                - Number of random bytes to generate
 */
 ...
 * @param[in]       max_number           - Upper bound of the range (exclusive)
 * @return          Random number between 1 and (max_number - 1)
 */
unsigned int ske_get_rand_number(unsigned int max_number)
{
    unsigned int i;
...
/**
 * @brief           Callback function for SKE operations
 */
...
/**
 * @brief           Test function for SKE cryptographic operations
 * @param[in]       ske_alg              - SKE algorithm to test
 * @param[in]       mode                 - Operation mode (ECB, CBC, etc.)
 * @param[in]       wordAlign            - Flag for word alignment
 * @param[in]       std_plain            - Pointer to standard plaintext
 * @param[in]       byteLen              - Length of data in bytes
 * @param[in]       key                  - Pointer to encryption key
 * @param[in]       key_id               - Key identifier
 * @param[in]       iv                   - Pointer to initialization vector
 * @param[in]       std_cipher           - Pointer to standard ciphertext
 * @return          0 if test passes, 1 if test fails
 */
unsigned int ske_test(ske_alg_e ske_alg, ske_mode_e mode, unsigned char wordAlign, const unsigned char *std_plain, unsigned int byteLen, const unsigned char *key,
                      unsigned short key_id, const unsigned char *iv, const unsigned char *std_cipher)
{
    unsigned int i, j;
    ...
    if (wordAlign)
    {
        memcpy_(std_plain_buf, std_plain, byteLen);
        memcpy_(std_cipher_buf, std_cipher, byteLen);
        if (key != NULL)
        {
            memcpy_(key_buf, key, key_byteLen);
        }
        if (SKE_MODE_ECB != mode)
        {
            memcpy_(iv_buf, iv, block_byteLen);
        }
    ...
    {
        memcpy_(std_plain_buf + 1, std_plain, byteLen);
        memcpy_(std_cipher_buf + 1, std_cipher, byteLen);
        if (key != NULL)
        {
            memcpy_(key_buf + 1, key, key_byteLen);
        }
        if (SKE_MODE_ECB != mode)
        {
            memcpy_(iv_buf + 1, iv, block_byteLen);
        }
    ...
    // ENCRYPT
#ifdef SKE_SEC_API
    ret = ske_sec_crypto(ske_alg, mode, SKE_CRYPTO_ENCRYPT, key_, key_id, iv, std_plain_, cipher_, byteLen);
#else
    ret = ske_lp_crypto(ske_alg, mode, SKE_CRYPTO_ENCRYPT, key_, key_id, iv, std_plain_, cipher_, byteLen);
#endif
    if (SKE_SUCCESS != ret)
    {
    ...
    // DECRYPT
#ifdef SKE_SEC_API
    ret = ske_sec_crypto(ske_alg, mode, SKE_CRYPTO_DECRYPT, key_, key_id, iv, cipher_, replain_, byteLen);
#else
    ret = ske_lp_crypto(ske_alg, mode, SKE_CRYPTO_DECRYPT, key_, key_id, iv, cipher_, replain_, byteLen);
#endif
    if (SKE_SUCCESS != ret)
    {
...
/**
 * @brief           Test function for SKE DMA cryptographic operations
 * @param[in]       ske_alg              - SKE algorithm to test
 * @param[in]       mode                 - Operation mode (ECB, CBC, etc.)
 * @param[in]       std_plain            - Pointer to standard plaintext
 * @param[in]       wordLen              - Length of data in words
 * @param[in]       key                  - Pointer to encryption key
 * @param[in]       key_id               - Key identifier
 * @param[in]       iv                   - Pointer to initialization vector
 * @param[in]       std_cipher           - Pointer to standard ciphertext
 * @return          0 if test passes, 1 if test fails
 */
unsigned int ske_dma_test(ske_alg_e ske_alg, ske_mode_e mode, const unsigned char *std_plain, unsigned int wordLen, const unsigned char *key, unsigned short key_id,
                          const unsigned char *iv, const unsigned char *std_cipher)
{
    unsigned int i, j;
    ...
    uint32_copy(std_plain_buf, (const unsigned int *)std_plain, wordLen);   // memcpy_(std_plain_buf, std_plain, byteLen);
    uint32_copy(std_cipher_buf, (const unsigned int *)std_cipher, wordLen); // memcpy_(std_cipher_buf, std_cipher, byteLen);
...
#else
    // ENCRYPT
    ret = ske_lp_dma_crypto(ske_alg, mode, SKE_CRYPTO_ENCRYPT, key, key_id, iv, std_plain_, cipher_, wordLen, ske_call_manage);
    if (SKE_SUCCESS != ret)
    {
        printf("\r\n ske dma encrypt 1 error");
        return 1;
    ...
    // DECRYPT
    ret = ske_lp_dma_crypto(ske_alg, mode, SKE_CRYPTO_DECRYPT, key, key_id, iv, cipher_, replain_, wordLen, ske_call_manage);
    if (SKE_SUCCESS != ret)
    {
    ...
    if (uint32_big_num_cmp(cipher_, wordLen, std_cipher_, wordLen) || uint32_big_num_cmp(replain_, wordLen, std_plain_, wordLen))
    {
        printf("\r\n %s %s one time input test failure!!", name[ske_alg], oper_mode[mode]);
    ...
    // ENCRYPT
    ret = ske_lp_dma_init(ske_alg, mode, SKE_CRYPTO_ENCRYPT, key, key_id, iv);
    if (SKE_SUCCESS != ret)
    {
        printf("\r\n ske dma int 3 error");
        return 1;
    ...
    if (uint32_big_num_cmp(replain_, 256, cipher_, 256))
    {
...
const unsigned char std_iv[16] = {0xC7, 0x2B, 0x65, 0x91, 0xA0, 0xD7, 0xDE, 0x8F, 0x6B, 0x40, 0x72, 0x33, 0xAD, 0x35, 0x81, 0xD6};
...
/**
 * @brief           Test function for SM4 cryptographic operations
...
/**
 * @brief           Test function for AES-192 cryptographic operations
 * @return          0 if all tests pass, 1 if any test fails
 */
...
/**
 * @brief           Test function for AES-256 cryptographic operations
 * @return          0 if all tests pass, 1 if any test fails
 */
...
/**
 * @brief           Test function for DES cryptographic operations
 * @return          0 if all tests pass, 1 if any test fails
 */
...
/**
 * @brief           Test function for TDES-128 cryptographic operations
 * @return          0 if all tests pass, 1 if any test fails
 */
...
/**
 * @brief           Test function for TDES-192 cryptographic operations
 * @return          0 if all tests pass, 1 if any test fails
 */
...
/**
 * @brief           Test function for TDES-EEE-128 cryptographic operations
 * @return          0 if all tests pass, 1 if any test fails
 */
...
const unsigned char std_tdes_eee_192_cbc_out[128] = {
    };
const unsigned char std_tdes_eee_192_cfb_out[128] = {
    };
const unsigned char std_tdes_eee_192_ofb_out[128] = {
    };
const unsigned char std_tdes_eee_192_ctr_out[128] = {
    };

/**
 * @brief           Test function for TDES-EEE-192 cryptographic operations
 * @return          0 if all tests pass, 1 if any test fails
 */
...
/**
 * @brief           Get the number of rounds for speed testing based on algorithm
 * @param[in]       alg                  - SKE algorithm type
 * @return          Number of rounds to use for speed testing
 */
unsigned int speed_get_round_by_alg(ske_alg_e alg)
{
    unsigned int round;
    ...
    case SKE_ALG_TDES_EEE_192:
        ...
        round = 20; // since hardware does not support 3DES.
        break;
#endif
...
unsigned int ske_speed_test_internal(ske_alg_e alg, ske_mode_e mode, ske_crypto_e crypto, unsigned int round, unsigned int once_bytes)
{
    char *name_alg[] = {
        "DES",
        "TDES_128",
...
/**
 * @brief           Perform speed tests for a specific SKE algorithm
 * @param[in]       alg                  - SKE algorithm to test
 * @return          0 if all tests pass
 */
unsigned int ske_speed_test_by_alg(ske_alg_e alg)
{
...
/**
 * @brief           Perform speed tests for all supported SKE algorithms
 * @return          0 if all tests pass
 */
...
#ifdef SKE_LP_DMA_FUNCTION
/**
 * @brief           Internal function for DMA-based speed testing
 * @param[in]       alg                  - SKE algorithm to test
 * @param[in]       mode                 - Operation mode (ECB, CBC, etc.)
 * @param[in]       crypto               - Encryption or decryption
 * @param[in]       round                - Number of test rounds
 * @param[in]       once_words           - Number of words to process per round
 * @return          0 if test passes, 1 if test fails
 */
unsigned int ske_dma_speed_test_internal(ske_alg_e alg, ske_mode_e mode, ske_crypto_e crypto, unsigned int round, unsigned int once_words)
{
    char *name_alg[] = {
        "DES",
        "TDES_128",
...
/**
 * @brief           Perform DMA-based speed tests for a specific SKE algorithm
 * @param[in]       alg                  - SKE algorithm to test
 * @return          0 if all tests pass
 */
unsigned int ske_dma_speed_test_by_alg(ske_alg_e alg)
{
...
/**
 * @brief           Perform DMA-based speed tests for all supported SKE algorithms
 * @return          0 if all tests pass
 */
...
/**
 * @brief           Comprehensive test function for all supported SKE algorithms and modes
 * @return          0 if all tests pass, 1 if any test fails
 */
