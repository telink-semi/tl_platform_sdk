...
/* data route :
 *
 * RAM ->FIFO_1 --> sidetone_ADC---^-sidetone_OUTPUT-- i2sTx -> i2sRx ->RAM
 * RAM ->FIFO_0 --> sidetone_DAC---|
 * Connect i2s adc_dat_pin and dac_dat_pin with a wire.
 */
...
//#define SIDETONE_GAIN_SEL         SIDETONE_m30_DB
#define SIDETONE_GAIN_SEL           SIDETONE_0DB
//#define SIDETONE_GAIN_SEL         SIDETONE_15DB

#define SIDETONE_NORMAL             1
#define SIDETONE_SIMPLE             2
#define SIDETONE_MODE               SIDETONE_SIMPLE

#define SIDETONE_AUDIO_CLK          43008000
#define SIDETONE_I2S_SAMPLE         48000
#define SIDETONE_CH_NUM             2

#define I2S_RX_BUFF_OFFSET_LEN      32

#define CHUNK_SIZE                  (2048)

//unsigned short audio_i2s_48k_config[5] = { 1, 14, 2, 64, 64 };
unsigned short audio_i2s_48k_config[5] = { 1, 7, 2, 64, 64 };

int AUDIO_BUFFER_OUT[CHUNK_SIZE + I2S_RX_BUFF_OFFSET_LEN] = { 0 };
int AUDIO_BUFFER_in_zero[CHUNK_SIZE + I2S_RX_BUFF_OFFSET_LEN] = { 0 };

volatile int results_report[16];
int offset = 0;
...
void user_init(void)
{
    ...
    if(SIDETONE_MODE == SIDETONE_NORMAL)
    {
        audio_side_tone_set_dac_gain(SIDE_TONE_CHN0, SIDETONE_GAIN_SEL);
        audio_side_tone_set_dac_gain(SIDE_TONE_CHN1, SIDETONE_GAIN_SEL);
        audio_side_tone_set_adc_gain(SIDE_TONE_CHN0, SIDETONE_GAIN_SEL);
        audio_side_tone_set_adc_gain(SIDE_TONE_CHN1, SIDETONE_GAIN_SEL);

        audio_side_tone_dma_en(SIDE_TONE_CHN01);
        audio_side_tone_ch_en(SIDE_TONE_CHN0);
        audio_side_tone_ch_en(SIDE_TONE_CHN1);
    }
    audio_side_tone_set_req_count(SIDE_TONE_CHN01, 43008000, 48000, 2);
    ...
    if(SIDETONE_MODE == SIDETONE_NORMAL)
    {
        ...
        audio_side_tone_set_dac_gain(SIDE_TONE_CHN2, SIDETONE_GAIN_SEL);
        audio_side_tone_set_dac_gain(SIDE_TONE_CHN3, SIDETONE_GAIN_SEL);
        audio_side_tone_set_adc_gain(SIDE_TONE_CHN2, SIDETONE_GAIN_SEL);
        audio_side_tone_set_adc_gain(SIDE_TONE_CHN3, SIDETONE_GAIN_SEL);
    ...
    audio_side_tone_set_req_count(SIDE_TONE_CHN23, 43008000, 48000, 2);
    ...
    if(SIDETONE_MODE == SIDETONE_NORMAL)
    {
        ...
        audio_side_tone_set_dac_gain(SIDE_TONE_CHN4, SIDETONE_GAIN_SEL);
        audio_side_tone_set_dac_gain(SIDE_TONE_CHN5, SIDETONE_GAIN_SEL);
        audio_side_tone_set_adc_gain(SIDE_TONE_CHN4, SIDETONE_GAIN_SEL);
        audio_side_tone_set_adc_gain(SIDE_TONE_CHN5, SIDETONE_GAIN_SEL);
    ...
    audio_side_tone_set_req_count(SIDE_TONE_CHN45, 43008000, 48000, 2);
    ...
    audio_rx_dma_chain_init(FIFO0, DMA0, (unsigned short *)AUDIO_BUFFER_OUT, (CHUNK_SIZE + I2S_RX_BUFF_OFFSET_LEN) * 4);
...
//    audio_tx_dma_chain_init(FIFO0, DMA1, (unsigned short *)AUDIO_BUFFER_in_zero, (CHUNK_SIZE + I2S_RX_BUFF_OFFSET_LEN) * 4);
...
//    audio_tx_dma_chain_init(FIFO1, DMA2, (unsigned short *)sidetone_data_in, (CHUNK_SIZE + I2S_RX_BUFF_OFFSET_LEN) * 4);
//    audio_tx_dma_chain_init(FIFO1, DMA2, (unsigned short *)AUDIO_BUFFER_in_zero, (CHUNK_SIZE + I2S_RX_BUFF_OFFSET_LEN) * 4);
    ...
    if(SIDETONE_MODE == SIDETONE_SIMPLE) {
#if (SIDETONE_CH_SEL == SIDETONE_CH01_TEST)
        audio_side_tone_dma_en(SIDE_TONE_CHN01);
        audio_side_tones_set_simple_mode(SIDE_TONE_CHN01);
#elif (SIDETONE_CH_SEL == SIDETONE_CH23_TEST)
        audio_side_tone_dma_en(SIDE_TONE_CHN23);
        audio_side_tones_set_simple_mode(SIDE_TONE_CHN23);
#elif (SIDETONE_CH_SEL == SIDETONE_CH45_TEST)
        audio_side_tone_dma_en(SIDE_TONE_CHN45);
        audio_side_tones_set_simple_mode(SIDE_TONE_CHN45);
#endif
    }
    ...
    delay_ms(CHUNK_SIZE*1000/SIDETONE_I2S_SAMPLE+1); /* waiting for dma rx complete. */
    audio_rx_dma_dis(DMA0);
    audio_tx_dma_dis(DMA1);
    audio_tx_dma_dis(DMA2);
    audio_i2s_clk_dis(audio_i2s_config.i2s_select);

    while (AUDIO_BUFFER_OUT[offset] == 0)
    {
        offset++;
    }
    results_report[8] = offset;
    int *data_alg = NULL;
    if(SIDETONE_MODE == SIDETONE_NORMAL)
    {
        if(SIDETONE_GAIN_SEL == SIDETONE_m30_DB)
            data_alg = sidetone_data_out_m30db;
        else if(SIDETONE_GAIN_SEL == SIDETONE_15DB)
            data_alg = sidetone_data_out_15db;
        else
            data_alg = sidetone_data_out_0db;
    }
    else
    {
        data_alg = sidetone_data_out_0db;
//      data_alg = sidetone_data_in;
//      data_alg = &sidetone_data_in[CHUNK_SIZE];
    }

    for(unsigned int i = 0; i < 2048; i++)
    {
        if (data_alg[i] != AUDIO_BUFFER_OUT[i + offset])
        {
            results_report[0] = i;
            results_report[1] = data_alg[i];
            results_report[2] = AUDIO_BUFFER_OUT[i + offset];
//          results_report[3] = data_alg[i-1];
//          results_report[4] = AUDIO_BUFFER_OUT[i-1 + offset];
            while(1);
        }
    }
    results_report[5] = 'O';
