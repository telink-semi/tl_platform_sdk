...
#define DMIC0_INPUT_TO_BUF_TO_I2S (1) //DMIC0 support STEREO
#define DMIC1_INPUT_TO_BUF_TO_I2S (2) //DMIC1 only support MONO

#define DMIC_MODE_SEL                 DMIC0_INPUT_TO_BUF_TO_I2S

#define BIT24_USED                                 1//I2S0

int AUDIO_BUFF[9600];
unsigned short audio_i2s_16k_config[5]     = {1, 14, 6, 64, 64};
unsigned short audio_i2s_32k_config[5]     = {2, 14, 6, 64, 64};
...
unsigned short audio_i2s_48k_config_PDM[5]  = {1, 28, 0, 64, 64};
unsigned short audio_i2s_96k_config[5]     = {1, 14, 1, 64, 64};

unsigned short audio_i2s_192k_config[5]     = {2, 14, 1, 64, 64};
...
void user_init(void)
{
...
#if (DMIC_MODE_SEL == DMIC0_INPUT_TO_BUF_TO_I2S)
#if (0)
    ...
    /* matrix input config. */
    audio_matrix_set_rx_fifo_route(FIFO0, FIFO_RX_ROUTE_DMIC_192K, FIFO_RX_CODEC_OR_DMIC_A0_A1_32BIT);
    /* rx dma init. */
...
#if (FIFO_USED)
#if (BIT24_USED == 1)
        audio_matrix_set_rx_fifo_route(FIFO0, FIFO_RX_ROUTE_DMIC_16K_OR_48K, FIFO_RX_CODEC_OR_DMIC_A0_32BIT);
#else
        audio_matrix_set_rx_fifo_route(FIFO0, FIFO_RX_ROUTE_DMIC_16K_OR_48K, FIFO_RX_CODEC_OR_DMIC_A0_16BIT);
#endif
#else
#define ANC_CHANNEL_SELECT                      ANC0
    ...
    audio_matrix_set_anc_ref_route(ANC_CHANNEL_SELECT, ANC0_REF0, ANC_REF_ROUTE_DMIC_384K_LL, ANC_REF_CODEC_OR_DMIC_A0_32_BIT);

#endif
        /* rx dma init. */
        audio_rx_dma_chain_init(FIFO0, DMA0, (unsigned short *)AUDIO_BUFF, sizeof(AUDIO_BUFF));

        audio_dmic0_set_pin(GPIO_FC_PD2, GPIO_FC_PD7, GPIO_NONE_PIN);
        audio_dmic_input_init(&dmic_input_config);
...
#define I2S_SEL                                 1//I2S0
    ...
    /* matrix input config. */
#if (BIT24_USED == 1)
    audio_matrix_set_rx_fifo_route(FIFO0, FIFO_RX_ROUTE_DMIC_16K_OR_48K, FIFO_RX_CODEC_OR_DMIC_A2_32BIT);
#else
    audio_matrix_set_rx_fifo_route(FIFO0, FIFO_RX_ROUTE_DMIC_384K, FIFO_RX_CODEC_OR_DMIC_A0_16BIT);
#endif
    /* rx dma init. */
    audio_rx_dma_chain_init(FIFO0, DMA0, (unsigned short *)AUDIO_BUFF, sizeof(AUDIO_BUFF));


    audio_dmic1_set_pin(GPIO_FC_PD2, GPIO_FC_PD7, GPIO_NONE_PIN);
    audio_dmic_input_init(&dmic_input_config);
    audio_rx_dma_en(DMA0);
...
#define I2S_SEL                                 1//I2S0
    ...
    audio_i2s_clk_en(audio_i2s_config.i2s_select);
#if (BIT24_USED == 1)
    audio_matrix_set_i2s_tx_route(I2S1_CHN0, I2S_TX_ROUTE_FIFO, I2S_TX_FIFO0_20_OR_24_MONO);
#else
    audio_matrix_set_i2s_tx_route(I2S1_CHN0, I2S_TX_ROUTE_FIFO, I2S_TX_FIFO0_16_MONO);
#endif
    audio_tx_dma_chain_init(FIFO0, DMA1, (unsigned short *)AUDIO_BUFF, sizeof(AUDIO_BUFF));
    audio_tx_dma_en(DMA1);
#endif
