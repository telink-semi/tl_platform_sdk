...
/**************************************************************CAN_MODULE_SEL********************************************************/
    ...
    for(volatile unsigned char i=0;i< sizeof(rxmb_cfg)/sizeof(can_fd_rx_mb_config_t);i++){
        canfd_set_rx_mailbox_cfg(CAN_MODULE_SEL,i+RXMB_START_INDEX,&rxmb_cfg[i],1);
    ...
    can_self_reception_enable(CAN_MODULE_SEL);
    can_loop_back_enable(CAN_MODULE_SEL);

}

volatile unsigned char tx_done=1;
volatile unsigned char rx_done=0;
volatile unsigned int debug_loop=0;
void main_loop (void)
{
    delay_ms(200);
    gpio_toggle(LED1);
...
#endif
        for(volatile unsigned char j=0;j<rx_fd_frame[i].length;j++){
            if(*((&(rx_fd_frame[i].data_byte0))+j) !=*((&(tx_fd_frame.data_byte0))+j)){
    ...
    if(can_get_err_irq_status(CAN_MODULE_SEL,CAN_ERR_STATUS)){
     ...
     for(unsigned char i=RXMB_START_INDEX;i<RXMB_START_INDEX+MB_RX_NUM;i++){
        if(can_get_mb_irq_status(CAN_MODULE_SEL,i)){
            canfd_read_rx_mb(CAN_MODULE_SEL,i,&rx_fd_frame[i]);
#if IS_LITTLE_END_CONVERT
        data_convert_by_word((unsigned char*)rx_fd_frame[i].data_word,DLC_LENGTH_DECODE(rx_fd_frame[i].length));
#endif
