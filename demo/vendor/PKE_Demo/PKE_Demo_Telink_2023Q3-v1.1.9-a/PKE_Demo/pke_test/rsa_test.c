...
unsigned int rsa_modexp_sample_test_internal(RSA_MODEXP_SAMPLE_TEST_VECTOR *vector)
{
    ...
    unsigned int  i, wordLen = GET_WORD_LEN(nBitLen);
    unsigned int  ret;
    ...
    for (i = 0; i < 2; i++) {
        ...
        if ((RSA_SUCCESS != ret) || (uint32_BigNumCmp(out, wordLen, vector->std_out, wordLen) != 0)) {
        ...
        //sign(CRT mode)
        memset_((unsigned char *)out, 0, wordLen << 2);
        ...
        if ((RSA_SUCCESS != ret) || (uint32_BigNumCmp(out2, wordLen, vector->std_in, wordLen) != 0)) {
            printf("\r\n RSA-%u MOD_EXP test failure, ret=%x", nBitLen, ret);
            fflush(stdout);
            print_BN_buf_U32(out, wordLen, "out");
...
unsigned int rsa_modexp_speed_test_internal(RSA_MODEXP_SAMPLE_TEST_VECTOR *vector)
{
    ...
    if ((RSA_SUCCESS != ret) || (uint32_BigNumCmp(out, wordLen, vector->std_in, wordLen) != 0)) {
    ...
    if ((RSA_SUCCESS_S != ret) || (uint32_BigNumCmp(out, wordLen, vector->std_out, wordLen) != 0)) {
...
unsigned int rsa_modexp_corner_test(unsigned int eBitLen, unsigned int nBitLen, unsigned int round)
{
    ...
    for (i = 0; i < round; i++) {
        ...
        if (uint32_BigNumCmp(out2, nWordLen, in, nWordLen) != 0) {
        ...
        if ((RSA_SUCCESS != ret) || (0u == uint32_BigNum_Check_Zero(out, nWordLen))) {
...
unsigned int rsa_crt_modexp_corner_test(unsigned int eBitLen, unsigned int nBitLen, unsigned int round)
{
    ...
    for (i = 0; i < round; i++) {
        ...
        if ((RSA_SUCCESS != ret) || (1u != uint32_BigNum_Check_Zero(out, nWordLen))) {
