...
unsigned int x25519_std_test(void)
{
    ...
    {
        printf("\r\n X25519 key agreement success ");fflush(stdout);
    }
...
unsigned int x25519_get_key_corner_test(void)
{
    ...
    unsigned char  pubkey2[32]={
        0x84,0x7C,0x0D,0x2C,0x37,0x52,0x34,0xF3,0x65,0xE6,0x60,0x95,0x51,0x87,0xA3,0x73,
    ...
    {
        printf("\r\n X25519 get key corner test 1 error, ret=%x", ret);fflush(stdout);
        print_buf_U8(pubkey, 32, "pubkey 1");
        return 1;
    }
    else
    {
        printf("\r\n X25519 get key corner test 1 success");fflush(stdout);
    }
    ...
    {
        printf("\r\n X25519 get key corner test 2 error, ret=%x", ret);fflush(stdout);
        print_buf_U8(pubkey, 32, "pubkey 2");
        return 1;
    }
    else
    {
        printf("\r\n X25519 get key corner test 2 success");fflush(stdout);
    }
...
unsigned int x25519_corner_test(void)
{
    ...
    {
        printf("\r\n X25519 corner case test(pubkey all zero) error, ret=%x", ret);fflush(stdout);
        return 1;
    }
    else
    {
        printf("\r\n X25519 corner case test(pubkey all zero) success");fflush(stdout);
        return 0;
    }
}
...
unsigned int x25519_speed_test(void)
{
    ...
    printf("\r\n\r\n ============= x25519_getkey begin =============");fflush(stdout);
    ...
    endP(1,0,round);
#else
    printf("\r\n x25519_getkey finished");fflush(stdout);
#endif
    ...
    printf("\r\n\r\n ============= x25519_pointMul begin =============");fflush(stdout);
#ifdef X25519_SPEED_TEST_BY_TIMER
    startP();
#endif
    ...
    for(i=0;i<round;i++)
    {
        ...
        if(X25519_SUCCESS != ret)
        {
            printf("\r\n x25519_pointMul error, ret=%x", ret);
            return 1;
        }
    }
    ...
    endP(1,0,round);
#else
    printf("\r\n x25519_pointMul finished");fflush(stdout);
#endif
    ...
    for(i=0;i<100;i++)
    {
        ret = x25519_compute_key(A_prikey, B_pubkey, A_key, keyByteLen, NULL);
        if(ret)
        {
            printf("\r\n X25519 A calc error, ret=%x", ret);
            return 1;
    ...
    printf("\r\n\r\n ============= x25519_compute_key begin =============");fflush(stdout);
#ifdef X25519_SPEED_TEST_BY_TIMER
    startP();
#endif
    for(i=0;i<round;i++)
    {
        ret = x25519_compute_key(B_prikey, A_pubkey, B_key, keyByteLen, NULL);
        if(ret)
        {
            printf("\r\n X25519 B calc error, ret=%x", ret);
            return 1;
    ...
    endP(1,0,round);
#else
    printf("\r\n x25519_compute_key finished");fflush(stdout);
#endif
