...
/**
 * @brief           Multiplies an unsigned 8-bit integer array by 2^n
 * @param[in,out]   a                    - Pointer to the unsigned 8-bit integer array
 * @param[in]       a_len                - Length of array a in bytes
 * @param[in]       n                    - Power of 2 to multiply by (1 <= n <= 7)
 */
void uint8_big_multiple2n(unsigned char *a, unsigned int a_len, unsigned int n)
{
    unsigned int i;
...
/**
 * @brief           Gets the r value for ECDSA signature using a given k value
 * @param[in]       curve                - Pointer to the elliptic curve parameters
 * @param[in]       rand_k_in            - Input k value (NULL to generate random k)
 * @param[out]      rand_k_out           - Output k value
 * @param[out]      uint8_r              - Output r value as byte string (big endian)
 * @param[out]      uint32_r             - Output r value as word array (little endian)
 * @param[out]      uint8_m_r            - Output -r mod n as byte string (big endian)
 * @param[out]      uint32_m_r           - Output -r mod n as word array (little endian)
 * @return          0 on success, 1 on failure
 */
unsigned int ecdsa_test_get_r_by_k(const eccp_curve_t *curve, unsigned char *rand_k_in, unsigned char *rand_k_out, unsigned char *uint8_r, unsigned int *uint32_r,
                                   unsigned char *uint8_m_r, unsigned int *uint32_m_r)
{
    unsigned int k[ECCP_MAX_WORD_LEN];
    unsigned int tmp[ECCP_MAX_WORD_LEN];
    unsigned int p_wlen, n_len, n_wlen;
    unsigned int ret;
    ...
    uint32_clear(k, n_wlen);
    if (NULL != rand_k_in)
    {
        memcpy_(rand_k_out, rand_k_in, n_len);
        reverse_byte_array(rand_k_in, (unsigned char *)k, n_len);
    }
    else
    {
        ret = get_rand((unsigned char *)k, (n_wlen - 1) << 2);
        if (TRNG_SUCCESS != ret)
        {
            return 1;
        }
        reverse_byte_array((unsigned char *)k, rand_k_out, n_len);
    }

    // tmp = x coordinate of [k]G mod n
    ret = eccp_pointmul(curve, k, curve->eccp_Gx, curve->eccp_Gy, tmp, NULL);
    if (PKE_SUCCESS != ret)
    {
        printf("\r\n ecdsa_test_get_r_by_k eccp_pointmul error, ret = %u", ret);
        return 1;
    }

#if (defined(PKE_LP) || defined(PKE_SECURE))
    ret = pke_mod(tmp, p_wlen, curve->eccp_n, curve->eccp_n_h, curve->eccp_n_n0, n_wlen, tmp);
#else
    ret = pke_mod(tmp, p_wlen, curve->eccp_n, curve->eccp_n_h, n_wlen, tmp);
#endif
    if (PKE_SUCCESS != ret)
    {
        printf("\r\n ecdsa_test_get_r_by_k pke_mod error, ret = %u", ret);
        return 1;
    }

    if (NULL != uint8_r)
    {
        reverse_byte_array((unsigned char *)tmp, uint8_r, n_len);
        if (0u != (curve->eccp_n_bitLen & 7)) // this is for ECDSA E value.
        {
            uint8_big_multiple2n(uint8_r, n_len, 8u - (curve->eccp_n_bitLen & 7));
        }
    }

    if (NULL != uint32_r)
    {
        uint32_copy(uint32_r, tmp, n_wlen);
    }

    if ((NULL != uint8_m_r) || (NULL != uint32_m_r))
    {
        uint32_clear(k, n_wlen);
        (void)pke_modsub(curve->eccp_n, k, tmp, tmp, n_wlen);

        if (NULL != uint8_m_r)
        {
            reverse_byte_array((unsigned char *)tmp, uint8_m_r, n_len);
            if (0u != (curve->eccp_n_bitLen & 7)) // this is for ECDSA E value.
            {
                uint8_big_multiple2n(uint8_m_r, n_len, 8u - (curve->eccp_n_bitLen & 7));
            }
        }

        if (NULL != uint32_m_r)
        {
            uint32_copy(uint32_m_r, tmp, n_wlen);
        }
    }
...
/**
 * @brief           Internal function for ECDSA sign and verify test
 * @param[in]       curve                - Pointer to the elliptic curve parameters
 * @param[in]       curve_name           - Name of the curve
 * @param[in]       E                    - Message hash or random data
 * @param[in]       e_len                - Length of E in bytes
 * @param[in]       rand_k               - Random k value for signing
 * @param[in]       prikey               - Private key
 * @param[in]       pubkey               - Public key
 * @param[in]       signature_r          - Expected r value of signature
 * @param[in]       signature_s          - Expected s value of signature
 * @param[in]       sign_ret_value       - Expected return value from signing
 * @param[in]       info                 - Additional test information
 * @return          0 on success, 1 on failure
 */
unsigned int ecdsa_std_sign_verify_test_internal(const eccp_curve_t *curve, char *curve_name, unsigned char *E, unsigned int e_len, unsigned char *rand_k, unsigned char *prikey,
                                                 unsigned char *pubkey, unsigned char *signature_r, unsigned char *signature_s, unsigned int sign_ret_value, char *info)
{
    ...
    unsigned int p_len, n_len, n_wlen;
    unsigned int pubkey_len, signature_len;
    unsigned int ret;
    char *inf = "";
    ...
    n_wlen = get_word_len(curve->eccp_n_bitLen);
    pubkey_len = 2 * p_len;
    signature_len = 2 * n_len;

    ret = ecdsa_sign(curve, E, e_len, rand_k, prikey, buf);
    if ((sign_ret_value != ret))
    {
        printf("\r\n %s ECDSA sign %s failure, ret=%u", curve_name, inf, ret);
        return 1;
    }
    ...
    if (ECDSA_SUCCESS == ret)
    {
        ...
        {
            memcpy_(signature, signature_r, n_len);
            memcpy_(signature + n_len, signature_s, n_len);
            if (memcmp_(buf, signature, signature_len))
            {
                printf("\r\n %s ECDSA sign %s error, ret = %u", curve_name, inf, ret);
                print_buf_u8(E, e_len, "E");
                print_buf_u8(rand_k, n_len, "rand_k");
                print_buf_u8(prikey, n_len, "prikey");
                print_buf_u8(buf, signature_len, "signature");
                return 1;
            }
        }

        ret = ecdsa_verify(curve, E, e_len, pubkey, buf);
        if (ECDSA_SUCCESS != ret)
        {
            printf("\r\n %s ECDSA verify %s error, ret = %u", curve_name, inf, ret);
            return 1;
        }

        /********** valid signature test (s=-s). **********/
        // since [k]G = [e*s^(-1)]G + [r*s^(-1)]PA, and only compare x coordinate
        // of result point in verify.
        tmp2[n_wlen - 1] = 0;
        reverse_byte_array(buf + n_len, (unsigned char *)tmp2, n_len);
        ret = pke_sub(curve->eccp_n, tmp2, tmp2, n_wlen);
        if (PKE_SUCCESS != ret)
        {
            printf("\r\n valid signature test (s=-s) failure, ret=%u", ret);
            return 1;
        }
        else
        {
        }
        memcpy_(tmp, buf, n_len);
        reverse_byte_array((unsigned char *)tmp2, tmp + n_len, n_len);

        ret = ecdsa_verify(curve, E, e_len, pubkey, tmp);
        if (ECDSA_SUCCESS != ret)
        {
            printf("\r\n ECDSA verify %s failure(valid: s=-s), ret=%u", inf, ret);
            print_buf_u8(tmp, signature_len, "tmp");
            return 1;
        }

        /********** valid signature test (r = 0). **********/
        // it is infeasible to get r=0, since [k]G = (0,sqrt(b)), you are unable to
        // get k.

        /********** invalid signature test (s = k^(-1)(e+r*dA) = 0 mod n) with 2
         * cases. **********/
        // get k and buf2 = r = (x coordinate of [k]G or [k](-G)) mod n
        ret = ecdsa_test_get_r_by_k(curve, rand_k, k, NULL, buf2, NULL, NULL);
        if (0 != ret)
        {
            return 1;
        }
        ...
        // get tmp2 = -e mod n
        uint32_clear(tmp2, n_wlen);
        if (curve->eccp_n_bitLen >= (e_len << 3))
        {
            reverse_byte_array(E, (unsigned char *)tmp2, e_len);
        }
        else
        {
            reverse_byte_array(E, (unsigned char *)tmp2, n_len);
            if ((curve->eccp_n_bitLen) & 7)
            {
                big_div_2n(tmp2, n_wlen, 8 - ((curve->eccp_n_bitLen) & 7));
            }
            else
            {
            }
        ...
        ret = pke_mod(tmp2, n_wlen, curve->eccp_n, curve->eccp_n_h, curve->eccp_n_n0, n_wlen, tmp2);

        uint32_clear(buf3, n_wlen);
        ret |= pke_modsub(curve->eccp_n, buf3, tmp2, tmp2, n_wlen);

        // get tmp2 = -e*r^(-1) mod n or dA=0(when e=0)
        ret |= pke_modinv(curve->eccp_n, buf2, buf3, n_wlen, n_wlen);
        ret |= pke_modmul(curve->eccp_n, tmp2, buf3, tmp2, n_wlen);
        if (PKE_SUCCESS != ret)
        {
            printf("\r\n invalid signature test (s=0 case 1) failure, ret=%u", ret);
            return 1;
        }
        else
        {
        }
        reverse_byte_array((unsigned char *)tmp2, (unsigned char *)tmp2, n_len);

        ret = ecdsa_sign(curve, E, e_len, k, (unsigned char *)tmp2, buf);
        if (ECDSA_ZERO_ALL != ret)
        {
            printf("\r\n ECDSA sign %s failure(invalid: s=0 case 2), ret=%u", inf, ret);
            print_buf_u8(buf, signature_len, "signature");
            print_buf_u8((unsigned char *)tmp2, n_len, "dA");
            return 1;
        }

        // case 2: e = -r*dA mod n
        uint32_clear(buf3, n_wlen);
        reverse_byte_array(prikey, (unsigned char *)buf3, n_len);
        ret = pke_modmul(curve->eccp_n, buf2, buf3, tmp2, n_wlen);
        uint32_clear(buf3, n_wlen);
        ret |= pke_modsub(curve->eccp_n, buf3, tmp2, tmp2, n_wlen);
        reverse_byte_array((unsigned char *)tmp2, (unsigned char *)tmp2, n_len);
        if (curve->eccp_n_bitLen & 7)
        {
            uint8_big_multiple2n((unsigned char *)tmp2, n_len, 8 - (curve->eccp_n_bitLen & 7));
        }

        ret = ecdsa_sign(curve, (unsigned char *)tmp2, n_len, k, prikey, buf);
        if (ECDSA_ZERO_ALL != ret)
        {
            printf("\r\n ECDSA sign %s failure(invalid: s=0 case 2), ret=%u", inf, ret);
            print_buf_u8(buf, signature_len, "signature");
            print_buf_u8((unsigned char *)tmp2, n_len, "E");
            return 1;
        }
        ...
        memcpy_(tmp, E, e_len);
        tmp[0] += 3;
        ret = ecdsa_verify(curve, tmp, e_len, pubkey, buf);
        if (ECDSA_VERIFY_FAILED != ret)
        {
            printf("\r\n %s ECDSA verify %s failure(invalid E), ret=%u", curve_name, inf, ret);
            return 1;
        }

        // invalid pubkey test 1
        memcpy_(tmp, pubkey, pubkey_len);
        tmp[0] += 3;
        ret = ecdsa_verify(curve, E, e_len, tmp, buf);
        if (PKE_NOT_ON_CURVE != ret)
        {
            printf("\r\n %s ECDSA verify %s failure(invalid public key 1), ret=%u", curve_name, inf, ret);
            return 1;
        }
        ...
        if (PKE_NOT_ON_CURVE != ret)
        {
            printf("\r\n %s ECDSA verify %s failure(invalid public key 2), ret=%u", curve_name, inf, ret);
            return 1;
        }
        ...
        if (PKE_NOT_ON_CURVE != ret)
        {
            printf("\r\n %s ECDSA verify %s failure(invalid public key 3), ret=%u", curve_name, inf, ret);
            return 1;
        }

        // invalid signature test 1(corrupted r)
        memcpy_(tmp, buf, signature_len);
        tmp[2] += 3;
        ret = ecdsa_verify(curve, E, e_len, pubkey, tmp);
        if ((ECDSA_VERIFY_FAILED != ret) && (ECDSA_INTEGER_TOO_BIG != ret))
        {
            printf("\r\n %s ECDSA verify %s failure(invalid signature: corrupted r), "
                   "ret=%u",
                   curve_name, inf, ret);
            return 1;
        }
        ...
        if (ECDSA_ZERO_ALL != ret)
        {
            printf("\r\n %s ECDSA verify %s failure(invalid signature: r=0), ret=%u", curve_name, inf, ret);
            return 1;
        }

        // invalid signature test 3(r=n-1)
        memcpy_(tmp, buf, signature_len);
        reverse_byte_array((const unsigned char *)(curve->eccp_n), tmp, n_len);
        tmp[n_len - 1] -= 1;
        ret = ecdsa_verify(curve, E, e_len, pubkey, tmp);
        if (ECDSA_VERIFY_FAILED != ret)
        {
            printf("\r\n %s ECDSA verify %s failure(invalid signature: r=n-1), ret=%u", curve_name, inf, ret);
            return 1;
        }
        ...
        {
            printf("\r\n %s ECDSA verify %s failure(invalid signature: r=n), ret=%u", curve_name, inf, ret);
            return 1;
        }
        ...
        memcpy_(tmp, buf, signature_len);
        tmp[signature_len - 1] += 3;
        ret = ecdsa_verify(curve, E, e_len, pubkey, tmp);
        if (ECDSA_VERIFY_FAILED != ret)
        {
            printf("\r\n %s ECDSA verify %s failure(invalid signature: corrupted s), "
                   "ret=%u",
                   curve_name, inf, ret);
            return 1;
        }
        ...
        memcpy_(tmp, buf, signature_len);
        memset_(tmp + n_len, 0, n_len);
        ret = ecdsa_verify(curve, E, e_len, pubkey, tmp);
        if (ECDSA_ZERO_ALL != ret)
        {
            printf("\r\n %s ECDSA verify %s failure(invalid signature: s=0), ret=%u", curve_name, inf, ret);
            return 1;
        }

        // invalid signature test 8(s=n-1)
        memcpy_(tmp, buf, signature_len);
        reverse_byte_array((const unsigned char *)(curve->eccp_n), tmp + n_len, n_len);
        tmp[signature_len - 1] -= 1;
        ret = ecdsa_verify(curve, E, e_len, pubkey, tmp);
        if (ECDSA_VERIFY_FAILED != ret)
        {
            printf("\r\n %s ECDSA verify %s failure(invalid signature: s=n-1), ret=%u", curve_name, inf, ret);
            return 1;
        }
        ...
        if (ECDSA_INTEGER_TOO_BIG != ret)
        {
            printf("\r\n %s ECDSA verify %s failure(invalid signature: s=n), ret=%u", curve_name, inf, ret);
            return 1;
        }
...
/**
 * @brief           Standard ECDSA sign and verify test
 * @param[in]       curve                - Pointer to the elliptic curve parameters
 * @param[in]       curve_name           - Name of the curve
 * @param[in]       vector               - Pointer to test vector containing test data
 * @return          0 on success, 1 on failure
 */
unsigned int ecdsa_std_sign_verify_test(const eccp_curve_t *curve, char *curve_name, ecdsa_test_vector_t *vector)
{
    unsigned char E[ECCP_MAX_BYTE_LEN];
    unsigned int tmp_len, n_len;
    ...
    if (ecdsa_std_sign_verify_test_internal(curve, curve_name, vector->std_E_md5, 16, vector->rand_k, vector->priKey, vector->pubKey, vector->std_signature_r,
                                            vector->std_signature_s_md5, ECDSA_SUCCESS, "standard data test(E-128bits)"))
    {
        return 1;
    }

    // E(hash digest) 160 bits, actually here E is sha1("abc")
    if (ecdsa_std_sign_verify_test_internal(curve, curve_name, vector->std_E_sha1, 20, vector->rand_k, vector->priKey, vector->pubKey, vector->std_signature_r,
                                            vector->std_signature_s_sha1, ECDSA_SUCCESS, "standard data test(E-160bits)"))
    {
        return 1;
    }

    // E(hash digest) 192 bits, here E is random data
    if (ecdsa_std_sign_verify_test_internal(curve, curve_name, vector->std_E_192, 24, vector->rand_k, vector->priKey, vector->pubKey, vector->std_signature_r,
                                            vector->std_signature_s_192, ECDSA_SUCCESS, "standard data test(E-192bits)"))
    {
        return 1;
    }

    // E(hash digest) 224 bits, actually here E is sha224("abc")
    if (ecdsa_std_sign_verify_test_internal(curve, curve_name, vector->std_E_sha224, 28, vector->rand_k, vector->priKey, vector->pubKey, vector->std_signature_r,
                                            vector->std_signature_s_sha224, ECDSA_SUCCESS, "standard data test(E-224bits)"))
    {
        return 1;
    }

    // E(hash digest) 256 bit, actually here E is sha256("abc")
    if (ecdsa_std_sign_verify_test_internal(curve, curve_name, vector->std_E_sha256, 32, vector->rand_k, vector->priKey, vector->pubKey, vector->std_signature_r,
                                            vector->std_signature_s_sha256, ECDSA_SUCCESS, "standard data test(E-256bits)"))
    {
        return 1;
    }

    // E(hash digest) 384 bit, actually here E is sha384("abc")
    if (ecdsa_std_sign_verify_test_internal(curve, curve_name, vector->std_E_sha384, 48, vector->rand_k, vector->priKey, vector->pubKey, vector->std_signature_r,
                                            vector->std_signature_s_sha384, ECDSA_SUCCESS, "standard data test(E-384bits)"))
    {
        return 1;
    }

    // E(hash digest) 512 bit, actually here E is sha512("abc")
    if (ecdsa_std_sign_verify_test_internal(curve, curve_name, vector->std_E_sha512, 64, vector->rand_k, vector->priKey, vector->pubKey, vector->std_signature_r,
                                            vector->std_signature_s_sha512, ECDSA_SUCCESS, "standard data test(E-512bits)"))
    {
        return 1;
    }

    // E(hash digest) 544 bit, here E is random data
    if (ecdsa_std_sign_verify_test_internal(curve, curve_name, vector->std_E_544, 68, vector->rand_k, vector->priKey, vector->pubKey, vector->std_signature_r,
                                            vector->std_signature_s_544, ECDSA_SUCCESS, "standard data test(E-544bits)"))
    {
        return 1;
    }
    ...
    if (ecdsa_std_sign_verify_test_internal(curve, curve_name, E, n_len, vector->rand_k, vector->priKey, vector->pubKey, vector->std_signature_r, vector->std_signature_s_zero,
                                            ECDSA_SUCCESS, "test(E=0)"))
    {
        return 1;
    }
    ...
    memset_(E, 0, ECCP_MAX_BYTE_LEN);
    E[n_len - 1] = 1;
    tmp_len = (curve->eccp_n_bitLen) & 7;
    if (tmp_len)
    {
        uint8_big_multiple2n(E, n_len,
                             (8u - tmp_len)); // E[n_len-1] <<= (8-((curve->eccp_n_bitLen)&7));//
    }
    if (ecdsa_std_sign_verify_test_internal(curve, curve_name, E, n_len, vector->rand_k, vector->priKey, vector->pubKey, vector->std_signature_r, vector->std_signature_s_1,
                                            ECDSA_SUCCESS, "test(E=1)"))
    {
        return 1;
    }

    /********** e = n **********/
    reverse_byte_array((const unsigned char *)(curve->eccp_n), E, n_len);
    // make sure the MSB nBitLen is n
    tmp_len = (curve->eccp_n_bitLen) & 7;
    if (tmp_len)
    {
        uint8_big_multiple2n(E, n_len, (8u - tmp_len));
    }
    if (ecdsa_std_sign_verify_test_internal(curve, curve_name, E, n_len, vector->rand_k, vector->priKey, vector->pubKey, vector->std_signature_r, vector->std_signature_s_zero,
                                            ECDSA_SUCCESS, "test(E=n)"))
    {
        return 1;
    }

    /********** e = 0xFF...FF **********/
    memset_(E, 0xFF, n_len);
    if (ecdsa_std_sign_verify_test_internal(curve, curve_name, E, n_len, vector->rand_k, vector->priKey, vector->pubKey, vector->std_signature_r, vector->std_signature_s_FF,
                                            ECDSA_SUCCESS, "test(E=0xFF..FF)"))
    {
        return 1;
    }
...
/**
 * @brief           Standard ECDSA test for secp160k1 curve
 * @return          0 on success, 1 on failure
 */
unsigned int ecdsa_std_test_secp160k1(void)
{
    ...
    // standard signature(the default data is r, it is fixed for the curve and
    // rand_k) r:0x005E2FCE3B4F9996A75370E581291006E0753C7EDF
    unsigned char std_signature_r[] = {
        0x00,
        0x5E,
        0x2F,
...
/**
 * @brief           Standard ECDSA test for secp192k1 curve
 * @return          0 on success, 1 on failure
 */
unsigned int ecdsa_std_test_secp192k1(void)
{
    ...
    // standard signature(the default data is r, it is fixed for the curve and
    // rand_k) r:0x40C0FEA9D0219769FB462F366FB0BB42B5441ED895EB7DCF
    unsigned char std_signature_r[] = {
        0x40,
        0xC0,
        0xFE,
...
/**
 * @brief           Standard ECDSA test for secp224k1 curve
 * @return          0 on success, 1 on failure
 */
unsigned int ecdsa_std_test_secp224k1(void)
{
    ...
    // standard signature(the default data is r, it is fixed for the curve and
    // rand_k) r:0x00E15B44E94FDCFA62756F6416925F4795A108B43826A66CEBEA2AC19B
    unsigned char std_signature_r[] = {
        0x00,
        0xE1,
        0x5B,
...
/**
 * @brief           Standard ECDSA test for secp256k1 curve
 * @return          0 on success, 1 on failure
 */
...
/**
 * @brief           Standard ECDSA test for brainpoolp160r1 curve
 * @return          0 on success, 1 on failure
 */
unsigned int ecdsa_std_test_brainpoolp160r1(void)
{
    ...
    // standard signature(the default data is r, it is fixed for the curve and
    // rand_k) r:0x88AA4223B200E547B54C0BC5A739601FFE4D6DDA
    unsigned char std_signature_r[] = {
        0x88,
        0xAA,
        0x42,
...
/**
 * @brief           Standard ECDSA test for secp160r1 curve
 * @return          0 on success, 1 on failure
 */
unsigned int ecdsa_std_test_secp160r1(void)
{
    ...
    // standard signature(the default data is r, it is fixed for the curve and
    // rand_k) r:0x00B27B810EAD0FD85ADE2AE2E54C78A6819B7987D2
    unsigned char std_signature_r[] = {
        0x00,
        0xB2,
        0x7B,
...
/**
 * @brief           Standard ECDSA test for secp160r2 curve
 * @return          0 on success, 1 on failure
 */
unsigned int ecdsa_std_test_secp160r2(void)
{
    ...
    // standard signature(the default data is r, it is fixed for the curve and
    // rand_k) r:0x00D4A6DD49243C8036F3A4ABB8F2F3019456978AA4
    unsigned char std_signature_r[] = {
        0x00,
        0xD4,
        0xA6,
...
/**
 * @brief           Standard ECDSA test for secp192r2 curve
 * @return          0 on success, 1 on failure
 */
unsigned int ecdsa_std_test_secp192r1(void)
{
    ...
    // standard signature(the default data is r, it is fixed for the curve and
    // rand_k) r:0x1B17A8469AF9620FE5254D5CBC41B4C75E9D005302981ADE
    unsigned char std_signature_r[] = {
        0x1B,
        0x17,
        0xA8,
...
/**
 * @brief           Standard ECDSA test for secp224r1 curve
 * @return          0 on success, 1 on failure
 */
unsigned int ecdsa_std_test_secp224r1(void)
{
    ...
    // standard signature(the default data is r, it is fixed for the curve and
    // rand_k) r:0x206ABC1874FDF4001BAD8C842812897869CB1C84338E4961C3DE23EC
    unsigned char std_signature_r[] = {
        0x20,
        0x6A,
        0xBC,
...
/**
 * @brief           Standard ECDSA test for brainpoolp256r1 curve
 * @return          0 on success, 1 on failure
 */
unsigned int ecdsa_std_test_brainpoolp256r1(void)
{
    ...
    unsigned char std_signature_s_1[] = {0x2C, 0x51, 0xBD, 0x8B, 0x59, 0x4E, 0x27, 0xE9, 0x14, 0xE2, 0x62, 0x1F, 0x44, 0x8A, 0xBC, 0x18,
    ...
    unsigned char std_signature_s_FF[] = {0x52, 0xAC, 0xF9, 0xE5, 0xA4, 0x8C, 0x30, 0xD1, 0xC6, 0xC1, 0xA4, 0xF3, 0xD0, 0x38, 0xF2, 0xA9,
    ...
    const eccp_curve_t *curve = brainpoolp256r1;
...
/**
 * @brief           Standard ECDSA test for secp256r1 curve
 * @return          0 on success, 1 on failure
 */
...
/**
 * @brief           Standard ECDSA test for secp384r1 curve
 * @return          0 on success, 1 on failure
 */
...
/**
 * @brief           Standard ECDSA test for brainpoolp512r1 curve
 * @return          0 on success, 1 on failure
 */
...
/**
 * @brief           Standard ECDSA test for secp521r1 curve
 * @return          0 on success, 1 on failure
 */
...
/**
 * @brief           Standard ECDSA test for anders_1024_1 curve
 * @return          0 on success, 1 on failure
 */
...
unsigned int ecdsa_corner_case_test(const eccp_curve_t *curve, char *curve_name)
{
    unsigned char k[ECCP_MAX_BYTE_LEN];
    ...
    unsigned int i, m, n_len; //, n_wlen;
    unsigned int ret;

    n_len = get_byte_len(curve->eccp_n_bitLen);
    // n_wlen = get_word_len(curve->eccp_n_bitLen);

    printf("\r\n\r\n -------------- %s ECDSA sign & verify corner test "
           "-------------- ",
           curve_name);
    ...
    // e_n = n
    reverse_byte_array((const unsigned char *)curve->eccp_n, e_n, n_len);
    m = (curve->eccp_n_bitLen) & 7;
    if (0u != m)
    {
        uint8_big_multiple2n(e_n, n_len, 8u - m);
    }

    // get k, e_r = r = (x coordinate of [k]G or [k](-G)) mod n, and e_m_r = -r
    // mod n
    ret = ecdsa_test_get_r_by_k(curve, NULL, k, e_r, NULL, e_m_r, NULL);
    if (0 != ret)
    {
        return 1;
    }
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        else if (1 == i) // key pair is (n-1, [n-1]G)
        {
            ...
            reverse_byte_array((const unsigned char *)curve->eccp_n, priKey, n_len);
            priKey[n_len - 1] -= 0x01;
            ret = eccp_get_pubkey_from_prikey(curve, priKey, pubKey);
        }
        if (PKE_SUCCESS != ret)
        {
            printf("\r\n %s ECDSA eccp_get_pubkey_from_prikey error, ret = %u", curve_name, ret);
            return 1;
        }

        // E = 0
        if (ecdsa_std_sign_verify_test_internal(curve, curve_name, e_0, n_len, k, priKey, pubKey, NULL, NULL, ECDSA_SUCCESS, "corner case test(E = 0)"))
        {
            return 1;
        }

        // E = n
        if (ecdsa_std_sign_verify_test_internal(curve, curve_name, e_n, n_len, k, priKey, pubKey, NULL, NULL, ECDSA_SUCCESS, "corner case test(E = n)"))
        {
            return 1;
        }
        ...
        if (ecdsa_std_sign_verify_test_internal(curve, curve_name, e_r, n_len, k, priKey, pubKey, NULL, NULL, ret, "corner case test(E = r)"))
        {
            return 1;
        }
        ...
        if (ecdsa_std_sign_verify_test_internal(curve, curve_name, e_m_r, n_len, k, priKey, pubKey, NULL, NULL, ret, "corner case test(E = -r)"))
        {
            return 1;
        }
    }
...
unsigned int ecdsa_rand_test(const eccp_curve_t *curve, char *curve_name)
{
    unsigned char priKey[ECCP_MAX_BYTE_LEN];
    unsigned char pubKey[2 * ECCP_MAX_BYTE_LEN];
    ...
    unsigned char signature[2 * ECCP_MAX_BYTE_LEN];
    unsigned int i, j, e_len, p_len, n_len, signature_len;
    unsigned int ret;
    ...
    i=1: key pair is (1, G), k is random but fixed, e = r mod n, to check
    [e*s^(-1)]G + [r*s^(-1)]G i=2: key pair is (1, G), e and k are random. i=3:
    key pair is (n-1, [n-1]G = -G), e = 0 mod n, to check [0]G + [r*s^(-1)](-G)
    i=4: key pair is (n-1, [n-1]G = -G), k is random but fixed, e = -r mod n, to
    check [e*s^(-1)]G + [r*s^(-1)](-G) i=5: key pair is (n-1, [n-1]G = -G), e and
    k are random i=6: all random i=7: all random
    *****************************************/
    for (i = 0; i < ECDSA_SIGN_VERIFY_ROUND1; i++)
    {
        ...
        e_len = signature[0] % (sizeof(E)) + 1;
        ret = get_rand(E, e_len);
        if (TRNG_SUCCESS != ret)
        {
            printf("\r\n get rand error, ret = %u", ret);
            return 1;
        ...
        if (i <= 2) // corner case 1/2/3: key pair is (1, G)
        {
            ...
            else if (1 == i)
            {
                ...
                ret = ecdsa_test_get_r_by_k(curve, k, k, E, NULL, NULL, NULL);
                if (0 != ret)
                {
                    return 1;
                }
        ...
        else if (i <= 5) // corner case 4/5/6: key pair is (n-1, [n-1]G)
        {
            ...
            {
                // corner case 5: set e = -r mod n, to check [e*s^(-1)]G +
                // [r*s^(-1)](-G)
                ret = ecdsa_test_get_r_by_k(curve, k, k, NULL, NULL, E, NULL);
                if (0 != ret)
                {
                    return 1;
                }
        ...
        if (PKE_SUCCESS != ret)
        {
            printf("\r\n %s ECDSA get key error, ret = %u", curve_name, ret);
            return 1;
        }
        ...
        for (j = 0; j < ECDSA_SIGN_VERIFY_ROUND2; j++)
        {
            ...
            ret = ecdsa_sign(curve, E, e_len, k, priKey, signature);
            if (ECDSA_SUCCESS != ret)
            {
                printf("\r\n %s ECDSA sign error, ret = %u", curve_name, ret);
                print_buf_u8(priKey, n_len, "prikey");
                print_buf_u8(pubKey, p_len * 2, "pubkey");
                if (k)
                {
                    print_buf_u8(k, n_len, "k");
                }
                print_buf_u8(E, e_len, "E");
                print_buf_u8(signature, signature_len, "signature");
                return 1;
            }

            ret = ecdsa_verify(curve, E, e_len, pubKey, signature);
            if (ECDSA_SUCCESS != ret)
            {
                printf("\r\n %s ECDSA verify error, ret = %u", curve_name, ret);
                print_buf_u8(priKey, n_len, "prikey");
                print_buf_u8(pubKey, p_len * 2, "pubkey");
                if (k)
                {
...
/**
 * @brief           Perform ECDSA speed test
 *                  This function tests the performance of ECDSA signing and verification
 *                  operations. It sets different test rounds based on the curve's bit length and
 *                  measures the speed of key generation, signing, and verification separately.
 * @param[in]       curve                - Pointer to the elliptic curve parameters
 * @param[in]       curve_name           - Name of the curve as a string
 * @return          0 if the test succeeds
 * @return          1 if the test fails
 */
unsigned int ecdsa_speed_test(const eccp_curve_t *curve, char *curve_name)
{
    unsigned char priKey[ECCP_MAX_BYTE_LEN];
    unsigned char pubKey[2 * ECCP_MAX_BYTE_LEN];
    unsigned char E[ECCP_MAX_BYTE_LEN];
    unsigned char signature[2 * ECCP_MAX_BYTE_LEN];
    unsigned int i, e_len, p_len, n_len, signature_len;
    unsigned int round;
    unsigned int ret;
    ...
    p_len = (curve->eccp_p_bitLen + 7) / 8;
    n_len = (curve->eccp_n_bitLen + 7) / 8;
    signature_len = n_len * 2;
    e_len = 32;
    ...
    if (PKE_SUCCESS != ret)
    {
        printf("\r\n ECDSA get key error, ret = %u", ret);
        print_buf_u8(priKey, n_len, "prikey");
        print_buf_u8(pubKey, 2 * p_len, "pubkey");
        return 1;
    }
    ...
    for (i = 0; i < round; i++)
    {
        ret |= ecdsa_sign(curve, E, e_len, NULL, priKey, signature);
    }
...
/**
 * @brief           Perform ECDSA random number generator (RNG) test
 *                  This function tests the ECDSA implementation using random values for key
 *                  generation, signing, and verification. It selects different elliptic curves
 *                  randomly and performs ECDSA operations to verify the correctness of the
 *                  implementation.
...
/**
 * @brief           Perform ECDSA speed tests on all supported curves
 *                  This function executes speed tests for ECDSA operations (key generation,
 *                  signing, and verification) across all supported elliptic curves. The number
 *                  of test rounds is determined by the curve's bit length.
 * @return          0 if all tests pass successfully, 1 if any test fails
 */
...
/**/
#if ((ECCP_MAX_BIT_LEN >= 160) && defined(SUPPORT_SECP160K1))
    if (ecdsa_speed_test(secp160k1, "secp160k1"))
    {
        return 1;
    }
#endif
...
#if ((ECCP_MAX_BIT_LEN >= 192) && defined(SUPPORT_SECP192K1))
    if (ecdsa_speed_test(secp192k1, "secp192k1"))
    {
        return 1;
    }
#endif
...
#if ((ECCP_MAX_BIT_LEN >= 224) && defined(SUPPORT_SECP224K1))
    if (ecdsa_speed_test(secp224k1, "secp224k1"))
    {
        return 1;
    }
#endif
...
#if ((ECCP_MAX_BIT_LEN >= 256) && defined(SUPPORT_SECP256K1))
    if (ecdsa_speed_test(secp256k1, "secp256k1"))
    {
        return 1;
    }
#endif
...
#if ((ECCP_MAX_BIT_LEN >= 160) && defined(SUPPORT_SECP160R1))
    if (ecdsa_speed_test(secp160r1, "secp160r1"))
    {
        return 1;
    }
#endif
...
#if ((ECCP_MAX_BIT_LEN >= 160) && defined(SUPPORT_SECP160R2))
    if (ecdsa_speed_test(secp160r2, "secp160r2"))
    {
        return 1;
    }
#endif
...
#if ((ECCP_MAX_BIT_LEN >= 192) && defined(SUPPORT_SECP192R1))
    if (ecdsa_speed_test(secp192r1, "secp192r1"))
    {
        return 1;
    }
#endif
...
#if ((ECCP_MAX_BIT_LEN >= 224) && defined(SUPPORT_SECP224R1))
    if (ecdsa_speed_test(secp224r1, "secp224r1"))
    {
        return 1;
    }
#endif
...
#if ((ECCP_MAX_BIT_LEN >= 256) && defined(SUPPORT_SECP256R1))
    if (ecdsa_speed_test(secp256r1, "secp256r1"))
    {
        return 1;
    }
#endif
...
#if ((ECCP_MAX_BIT_LEN >= 384) && defined(SUPPORT_SECP384R1))
    if (ecdsa_speed_test(secp384r1, "secp384r1"))
    {
        return 1;
    }
#endif
...
#if ((ECCP_MAX_BIT_LEN >= 521) && defined(SUPPORT_SECP521R1))
    if (ecdsa_speed_test(secp521r1, "secp521r1"))
    {
        return 1;
    }
#endif
...
#if ((ECCP_MAX_BIT_LEN >= 160) && defined(SUPPORT_BRAINPOOLP160R1))
    if (ecdsa_speed_test(brainpoolp160r1, "brainpoolp160r1"))
    {
        return 1;
    }
#endif
    ...
    if (ecdsa_speed_test(brainpoolp192r1, "brainpoolp192r1"))
    {
        return 1;
    }
#endif
    ...
    if (ecdsa_speed_test(brainpoolp224r1, "brainpoolp224r1"))
    {
        return 1;
    }
#endif
...
#if ((ECCP_MAX_BIT_LEN >= 256) && defined(SUPPORT_BRAINPOOLP256R1))
    if (ecdsa_speed_test(brainpoolp256r1, "brainpoolp256r1"))
    {
        return 1;
    }
#endif
    ...
    if (ecdsa_speed_test(brainpoolp320r1, "brainpoolp320r1"))
    {
        return 1;
    }
#endif
    ...
    if (ecdsa_speed_test(brainpoolp384r1, "brainpoolp384r1"))
    {
        return 1;
    }
#endif
...
#if ((ECCP_MAX_BIT_LEN >= 512) && defined(SUPPORT_BRAINPOOLP512R1))
    if (ecdsa_speed_test(brainpoolp512r1, "brainpoolp512r1"))
    {
        return 1;
    }
#endif
    ...
    if (ecdsa_speed_test(bn256, "bn256"))
    {
        return 1;
    }
#endif
    ...
    if (ecdsa_speed_test(bn638, "bn638"))
    {
        return 1;
    }
#endif
...
#if ((ECCP_MAX_BIT_LEN >= 1024) && defined(SUPPORT_ANDERS_1024_1))
    if (ecdsa_speed_test(anders_1024_1, "anders_1024_1"))
    {
        return 1;
    }
#endif
...
/**
 * @brief           Runs standard data tests for ECDSA on all supported curves
 *                  This function executes standard ECDSA sign/verify tests using predefined
 *                  test vectors for all supported elliptic curves. It verifies the correctness
 *                  of ECDSA implementation against known good values.
...
/**
 * @brief           Perform ECDSA corner case tests on all supported curves
 *                  This function executes corner case tests for ECDSA operations across all
 *                  supported elliptic curves. It verifies the implementation's behavior in
 *                  edge cases such as zero values, curve order values, and special key pairs.
 * @return          0 if all tests pass successfully, 1 if any test fails
 */
...
#if ((ECCP_MAX_BIT_LEN >= 1024) && defined(SUPPORT_ANDERS_1024_1))
    if (ecdsa_corner_case_test(anders_1024_1, "anders_1024_1"))
    {
        return 1;
    }
#endif
...
/**
 * @brief           Perform ECDSA random tests on all supported curves
 *                  This function executes random tests for ECDSA operations (key generation,
 *                  signing, and verification) across all supported elliptic curves. It uses
 *                  random values for keys, messages, and signatures to verify the robustness
 *                  of the ECDSA implementation.
 * @return          0 if all tests pass successfully, 1 if any test fails
 */
