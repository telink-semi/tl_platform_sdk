...
// #define X25519_SPEED_TEST_BY_TIMER

#ifdef X25519_SPEED_TEST_BY_TIMER
...
/**
 * @brief           X25519  standard test
 * @return          0 for success, 1 for failure
 */
unsigned int x25519_std_test(void)
{
    // the test vectors are from RFC7748
    unsigned char A_prikey[] = {0x77, 0x07, 0x6D, 0x0A, 0x73, 0x18, 0xA5, 0x7D, 0x3C, 0x16, 0xC1, 0x72, 0x51, 0xB2, 0x66, 0x45,
                                0xDF, 0x4C, 0x2F, 0x87, 0xEB, 0xC0, 0x99, 0x2A, 0xB1, 0x77, 0xFB, 0xA5, 0x1D, 0xB9, 0x2C, 0x2A};
    unsigned char A_pubkey[] = {0x85, 0x20, 0xF0, 0x09, 0x89, 0x30, 0xA7, 0x54, 0x74, 0x8B, 0x7D, 0xDC, 0xB4, 0x3E, 0xF7, 0x5A,
                                0x0D, 0xBF, 0x3A, 0x0D, 0x26, 0x38, 0x1A, 0xF4, 0xEB, 0xA4, 0xA9, 0x8E, 0xAA, 0x9B, 0x4E, 0x6A};
    unsigned char B_prikey[] = {0x5D, 0xAB, 0x08, 0x7E, 0x62, 0x4A, 0x8A, 0x4B, 0x79, 0xE1, 0x7F, 0x8B, 0x83, 0x80, 0x0E, 0xE6,
                                0x6F, 0x3B, 0xB1, 0x29, 0x26, 0x18, 0xB6, 0xFD, 0x1C, 0x2F, 0x8B, 0x27, 0xFF, 0x88, 0xE0, 0xEB};
    unsigned char B_pubkey[] = {0xDE, 0x9E, 0xDB, 0x7D, 0x7B, 0x7D, 0xC1, 0xB4, 0xD3, 0x5B, 0x61, 0xC2, 0xEC, 0xE4, 0x35, 0x37,
                                0x3F, 0x83, 0x43, 0xC8, 0x5B, 0x78, 0x67, 0x4D, 0xAD, 0xFC, 0x7E, 0x14, 0x6F, 0x88, 0x2B, 0x4F};
    unsigned char std_key[] = {0x4A, 0x5D, 0x9D, 0x5B, 0xA4, 0xCE, 0x2D, 0xE1, 0x72, 0x8E, 0x3B, 0xF4, 0x80, 0x35, 0x0F, 0x25,
    ...
    {
        printf("\r\n X25519 A get key error, ret=%u", ret);
        return 1;
    }
    ...
    {
        printf("\r\n X25519 B get key error, ret=%u", ret);
        return 1;
    }
...
/**
 * @brief           X25519  get key corner test
 * @return          0 for success, 1 for failure
 */
unsigned int x25519_get_key_corner_test(void)
{
    // prikey is all 0
    unsigned char const pubkey1[32] = {0x2F, 0xE5, 0x7D, 0xA3, 0x47, 0xCD, 0x62, 0x43, 0x15, 0x28, 0xDA, 0xAC, 0x5F, 0xBB, 0x29, 0x07,
                                       0x30, 0xFF, 0xF6, 0x84, 0xAF, 0xC4, 0xCF, 0xC2, 0xED, 0x90, 0x99, 0x5F, 0x58, 0xCB, 0x3B, 0x74};

    // prikey is all 1
    unsigned char const pubkey2[32] = {0x84, 0x7C, 0x0D, 0x2C, 0x37, 0x52, 0x34, 0xF3, 0x65, 0xE6, 0x60, 0x95, 0x51, 0x87, 0xA3, 0x73,
                                       0x5A, 0x0F, 0x76, 0x13, 0xD1, 0x60, 0x9D, 0x3A, 0x6A, 0x4D, 0x8C, 0x53, 0xAE, 0xAA, 0x5A, 0x22};
    ...
    {
        printf("\r\n X25519 get key corner test 1 error, ret=%u", ret);
        fflush(stdout);
    ...
    {
        printf("\r\n X25519 get key corner test 2 error, ret=%u", ret);
        fflush(stdout);
...
unsigned int x25519_corner_test(void)
{
    ...
    {
        printf("\r\n X25519 corner case test(pubkey all zero) error, ret=%u", ret);
        fflush(stdout);
        return 1;
    }
    else
...
unsigned int x25519_rand_test(void)
{
    ...
    for (i = 0; i < X25519_KEY_EXCHANGE_ROUND1; i++)
    {
        ...
        for (j = 0; j < 50; j++)
        {
            ...
            get_rand((unsigned char *)&key_len, 4);
            key_len %= 200;
            if (0 == key_len)
            {
                key_len += 1;
            ...
            if (NULL == kdf)
            {
                if (key_len > get_byte_len(255))
                {
                    truekey_len = get_byte_len(255);
                }
            }

            printf(" key_len=%u(%u actually)", key_len, truekey_len);

            ret = x25519_compute_key(A_prikey, B_pubkey, A_key, truekey_len, kdf);
            if (X25519_SUCCESS != ret)
            {
                printf("\r\n X25519 A calc error, ret=%u", ret);
                return 1;
            ...
            ret = x25519_compute_key(B_prikey, A_pubkey, B_key, truekey_len, kdf);
            if (X25519_SUCCESS != ret)
            {
                printf("\r\n X25519 B calc error, ret=%u", ret);
                return 1;
            ...
            if (memcmp_(A_key, B_key, truekey_len))
            {
                printf("\r\n X25519 key agreement error ");
                return 1;
            }
...
unsigned int x25519_speed_test(void)
{
    ...
    for (i = 0; i < round; i++)
    {
        ret = x25519_pointmul(c25519, k, c25519->u, k);
        if (X25519_SUCCESS != ret)
        {
            printf("\r\n x25519_pointmul error, ret=%u", ret);
            return 1;
        }
    }
    ...
    for (i = 0; i < 100; i++)
    {
        ret = x25519_compute_key(A_prikey, B_pubkey, A_key, key_len, NULL);
        if (ret)
        {
            printf("\r\n X25519 A calc error, ret=%u", ret);
            return 1;
    ...
    for (i = 0; i < round; i++)
    {
        ret = x25519_compute_key(B_prikey, A_pubkey, B_key, key_len, NULL);
        if (ret)
        {
            printf("\r\n X25519 B calc error, ret=%u", ret);
            return 1;
    ...
    endp(1, 0, round);
#else
    printf("\r\n x25519_compute_key finished");
    fflush(stdout);
#endif

    if (memcmp_(A_key, B_key, key_len))
    {
        printf("\r\n\r\n X25519 key agreement error ");
        return 1;
    }
