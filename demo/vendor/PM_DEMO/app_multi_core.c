...
void user_init(void)
{
    ...
    //If it is timer or mdec wake up, you need to initialize 32K
        ...
        if(PM_MODE == SUSPEND_32K_RC_WAKEUP || PM_MODE == DEEP_32K_RC_WAKEUP || PM_MODE == DEEP_RET128K_32K_RC_WAKEUP \
            || PM_MODE == DEEP_RET256K_32K_RC_WAKEUP || PM_MODE == DEEP_RET384K_32K_RC_WAKEUP)
        {
    ...
    g_pm_sleep_mode.n22_ret_mode = N22_RET_MODE_SRAM_LOW512K;
    g_pm_sleep_mode.dsp_ret_mode = DSP_RET_MODE_SRAM_LOW384K;
    pm_sleep_wakeup(g_pm_sleep_mode, PM_WAKEUP_PAD, PM_TICK_STIMER, 0);
