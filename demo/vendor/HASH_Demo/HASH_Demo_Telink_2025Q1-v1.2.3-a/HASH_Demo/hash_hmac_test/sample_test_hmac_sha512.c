...
unsigned int sample_test_hmac_sha512(void)
{
    ...
    unsigned char std_mac[64] = {0x90, 0xDB, 0x1F, 0x0F, 0x80, 0xEF, 0x3B, 0xAD, 0xA4, 0x33, 0x28, 0xE5, 0x66, 0xA9, 0xCE, 0x02, 0xF9, 0x92, 0xF2, 0xA9, 0xD0, 0xDB,
                                 0x5C, 0xF3, 0x5E, 0x45, 0x3A, 0x42, 0x91, 0x70, 0xB6, 0x7A, 0xEC, 0xDE, 0xE0, 0xE9, 0xA5, 0x5A, 0xBB, 0xDB, 0xC3, 0x30, 0x56, 0xD4,
                                 0x86, 0xBD, 0x99, 0xF2, 0x95, 0x25, 0x98, 0xFD, 0xAE, 0xF8, 0x62, 0x36, 0x62, 0x9B, 0x80, 0xAD, 0xC2, 0x93, 0x27, 0x54};
    ...
    hmac_sha512_ctx_t hmac_sha512_ctx[1];
    hmac_ctx_t       *hmac_ctx = hmac_sha512_ctx;
    ...
    hmac_sha512_dma_ctx_t hmac_sha512_dma_ctx[1];
    hmac_dma_ctx_t       *hmac_dma_ctx = hmac_sha512_dma_ctx;
    ...
    if (memcmp_(std_mac, mac, mac_byte_len)) {
        printf("\r\n HMAC-SHA512 sample test 1 error");
        fflush(stdout);
        return 1;
    } else {
        printf("\r\n HMAC-SHA512 sample test 1 success");
        fflush(stdout);
    }
    ...
    if (memcmp_(std_mac, mac, mac_byte_len)) {
        printf("\r\n HMAC-SHA512 sample test 2 error");
        fflush(stdout);
        return 1;
    } else {
        printf("\r\n HMAC-SHA512 sample test 2 success");
        fflush(stdout);
    }
    ...
    node[2].msg_len  = 32;
    ret              = hmac_sha512_node_steps(key, 0, key_byte_len, node, 3, mac);
    if (HASH_SUCCESS != ret) {
        return 1;
    }

    if (memcmp_(std_mac, mac, mac_byte_len)) {
        printf("\r\n HMAC-SHA512 sample test 3 error");
        fflush(stdout);
        return 1;
    } else {
        printf("\r\n HMAC-SHA512 sample test 3 success");
        fflush(stdout);
    }
#endif
    ...
    if (memcmp_(std_mac, mac, mac_byte_len)) {
        printf("\r\n HMAC-SHA512 sample test 4 error");
        fflush(stdout);
        return 1;
    } else {
        printf("\r\n HMAC-SHA512 sample test 4 success");
        fflush(stdout);
    }
    ...
    if (memcmp_(std_mac, mac, mac_byte_len)) {
        printf("\r\n HMAC-SHA512 sample test 5 error");
        fflush(stdout);
        return 1;
    } else {
        printf("\r\n HMAC-SHA512 sample test 5 success");
        fflush(stdout);
    }
    ...
    node[2].msg_addr = msg + 48;
    node[2].msg_len  = 32;
    ret              = hmac_node_steps(HASH_SHA512, key, 0, key_byte_len, node, 3, mac);
    if (HASH_SUCCESS != ret) {
        return 1;
    }

    if (memcmp_(std_mac, mac, mac_byte_len)) {
        printf("\r\n HMAC-SHA512 sample test 6 error");
        fflush(stdout);
        return 1;
    } else {
        printf("\r\n HMAC-SHA512 sample test 6 success");
        fflush(stdout);
    }
#endif
    ...
    ret = hmac_sha512_dma_update_blocks(hmac_sha512_dma_ctx, msg_buf, blocks_bytes);
    if (HASH_SUCCESS != ret) {
        return 1;
    }

    ret = hmac_sha512_dma_final(hmac_sha512_dma_ctx, msg_buf + blocks_bytes / 4, remainder_bytes, mac_buf);
    if (HASH_SUCCESS != ret) {
        return 1;
    }

    if (memcmp_(std_mac, (unsigned char *)mac_buf, mac_byte_len)) {
        printf("\r\n HMAC-SHA512 dma sample test 1 error");
        fflush(stdout);
        return 1;
    } else {
        printf("\r\n HMAC-SHA512 dma sample test 1 success");
        fflush(stdout);
    }
    ...
    if (memcmp_(std_mac, (unsigned char *)mac_buf, mac_byte_len)) {
        printf("\r\n HMAC-SHA512 dma sample test 2 error");
        fflush(stdout);
        return 1;
    } else {
        printf("\r\n HMAC-SHA512 dma sample test 2 success");
        fflush(stdout);
    }
    ...
    ret                  = hmac_sha512_dma_node_steps(key, 0, key_byte_len, dma_node, 2, mac_buf, hash_call_manage);
    ...
    if (memcmp_(std_mac, (unsigned char *)mac_buf, mac_byte_len)) {
        printf("\r\n HMAC-SHA512 dma sample test 3 error");
        fflush(stdout);
        return 1;
    } else {
        printf("\r\n HMAC-SHA512 dma sample test 3 success");
        fflush(stdout);
    }
    ...
    if (memcmp_(std_mac, (unsigned char *)mac_buf, mac_byte_len)) {
        printf("\r\n HMAC-SHA512 dma sample test 4 error");
        fflush(stdout);
        return 1;
    } else {
        printf("\r\n HMAC-SHA512 dma sample test 4 success");
        fflush(stdout);
    }
    ...
    if (memcmp_(std_mac, (unsigned char *)mac_buf, mac_byte_len)) {
        printf("\r\n HMAC-SHA512 dma sample test 5 error");
        fflush(stdout);
        return 1;
    } else {
        printf("\r\n HMAC-SHA512 dma sample test 5 success");
        fflush(stdout);
    }
    ...
    dma_node[1].msg_addr = dma_node[0].msg_addr + dma_node[0].msg_len / 4;
    dma_node[1].msg_len  = remainder_bytes;
    ret                  = hmac_dma_node_steps(HASH_SHA512, key, 0, key_byte_len, dma_node, 2, mac_buf, hash_call_manage);
    ...
    if (memcmp_(std_mac, (unsigned char *)mac_buf, mac_byte_len)) {
        printf("\r\n HMAC-SHA512 dma sample test 6 error");
        fflush(stdout);
        return 1;
    } else {
        printf("\r\n HMAC-SHA512 dma sample test 6 success");
        fflush(stdout);
    }
