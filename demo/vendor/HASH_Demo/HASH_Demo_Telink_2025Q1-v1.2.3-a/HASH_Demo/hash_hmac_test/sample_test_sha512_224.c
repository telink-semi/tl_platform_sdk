...
unsigned int sample_test_sha512_224(void)
{
    ...
    unsigned char std_digest[28] = {0x3F, 0x2F, 0x0E, 0xC0, 0xAC, 0x01, 0x2F, 0x5C, 0x18, 0xAF, 0x44, 0xE8, 0x4B, 0xF3,
                                    0x18, 0x65, 0x15, 0x6D, 0xF6, 0xDB, 0xF6, 0x98, 0xB1, 0x6C, 0x71, 0x43, 0xC4, 0x7F};
    ...
    sha512_224_ctx_t sha512_224_ctx[1];
    hash_ctx_t      *hash_ctx = sha512_224_ctx;
    ...
    sha512_224_ctx_dma_t sha512_224_dma_ctx[1];
    hash_dma_ctx_t      *hash_dma_ctx = sha512_224_dma_ctx;
    ...
    node[2].msg_len  = 32;
    ret              = sha512_224_node_steps(node, 3, digest);
    if (HASH_SUCCESS != ret) {
        return 1;
    }

    if (memcmp_(std_digest, digest, digest_byte_len)) {
        printf("\r\n SHA512_224 sample test 3 error");
        fflush(stdout);
        return 1;
    } else {
        printf("\r\n SHA512_224 sample test 3 success");
        fflush(stdout);
    }
#endif
    ...
    if (memcmp_(std_digest, digest, digest_byte_len)) {
        printf("\r\n SHA512_224 sample test 5 error"); //fflush(stdout);
        return 1;
    } else {
        printf("\r\n SHA512_224 sample test 5 success"); //fflush(stdout);
    }
    ...
    node[2].msg_addr = msg + 48;
    node[2].msg_len  = 32;
    ret              = hash_node_steps(HASH_SHA512_224, node, 3, digest);
    if (HASH_SUCCESS != ret) {
        return 1;
    }

    if (memcmp_(std_digest, digest, digest_byte_len)) {
        printf("\r\n SHA512_224 sample test 6 error");
        fflush(stdout);
        return 1;
    } else {
        printf("\r\n SHA512_224 sample test 6 success");
        fflush(stdout);
    }
#endif
    ...
    ret = sha512_224_dma_update_blocks(sha512_224_dma_ctx, msg_buf, blocks_bytes);
    if (HASH_SUCCESS != ret) {
        return 1;
    }

    ret = sha512_224_dma_final(sha512_224_dma_ctx, msg_buf + blocks_bytes / 4, remainder_bytes, digest_buf);
    if (HASH_SUCCESS != ret) {
        return 1;
    }
    ...
    ret                  = sha512_224_dma_node_steps(dma_node, 2, digest_buf, hash_call_manage);
    ...
    if (uint32_BigNumCmp((unsigned int *)std_digest, digest_byte_len / 4, digest_buf, digest_byte_len / 4)) {
        printf("\r\n SHA512_224 dma sample test 3 error");
        fflush(stdout);
        return 1;
    } else {
        printf("\r\n SHA512_224 dma sample test 3 success");
        fflush(stdout);
    }
    ...
    if (uint32_BigNumCmp((unsigned int *)std_digest, digest_byte_len / 4, digest_buf, digest_byte_len / 4)) {
        printf("\r\n SHA512_224 dma sample test 5 error"); //fflush(stdout);
        return 1;
    } else {
        printf("\r\n SHA512_224 dma sample test 5 success"); //fflush(stdout);
    }
    ...
    dma_node[1].msg_addr = dma_node[0].msg_addr + dma_node[0].msg_len / 4;
    dma_node[1].msg_len  = remainder_bytes;
    ret                  = hash_dma_node_steps(HASH_SHA512_224, dma_node, 2, digest_buf, hash_call_manage);
    ...
    if (uint32_BigNumCmp((unsigned int *)std_digest, digest_byte_len / 4, digest_buf, digest_byte_len / 4)) {
        printf("\r\n SHA512_224 dma sample test 6 error");
        fflush(stdout);
        return 1;
    } else {
        printf("\r\n SHA512_224 dma sample test 6 success");
        fflush(stdout);
    }
