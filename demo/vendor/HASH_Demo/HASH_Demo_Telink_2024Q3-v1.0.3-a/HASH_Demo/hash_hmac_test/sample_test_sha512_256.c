...
unsigned int sample_test_sha512_256(void)
{
    ...
    if (HASH_SUCCESS != ret) {
        ...
        #ifdef CONFIG_HASH_SUPPORT_ADDRESS_HIGH_LOW
    ret = sha512_256_dma_update_blocks(sha512_256_dma_ctx, addr_h_0, (unsigned int)msg_buf, blocks_bytes);
        #else
    ...
    if (HASH_SUCCESS != ret) {
        ...
        #ifdef CONFIG_HASH_SUPPORT_ADDRESS_HIGH_LOW
    ret = sha512_256_dma_final(sha512_256_dma_ctx, addr_h_0, ((unsigned int)msg_buf) + blocks_bytes, remainder_bytes, addr_h_0, (unsigned int)digest_buf);
        #else
    ...
    uint32_clear(digest_buf, digest_byte_len / 4);
        #ifdef CONFIG_HASH_SUPPORT_ADDRESS_HIGH_LOW
    ret = sha512_256_dma(addr_h_0, (unsigned int)msg_buf, msg_byte_len, addr_h_0, (unsigned int)digest_buf, hash_call_manage);
        #else
    ...
    ret                    = sha512_256_dma_node_steps(dma_node, 2, addr_h_0, (unsigned int)digest_buf, hash_call_manage);
        #else
    ...
    uint32_clear(digest_buf, digest_byte_len / 4);
        #ifdef CONFIG_HASH_SUPPORT_ADDRESS_HIGH_LOW
    ret = hash_dma(HASH_SHA512_256, addr_h_0, (unsigned int)msg_buf, msg_byte_len, addr_h_0, (unsigned int)digest_buf, hash_call_manage);
        #else
    ...
    dma_node[1].msg_addr_l = dma_node[0].msg_addr_l + dma_node[0].msg_bytes;
    dma_node[1].msg_bytes  = remainder_bytes;
    ret                    = hash_dma_node_steps(HASH_SHA512_256, dma_node, 2, addr_h_0, (unsigned int)digest_buf, hash_call_manage);
        #else
    dma_node[0].msg_addr  = msg_buf;
    dma_node[0].msg_bytes = blocks_bytes;
    dma_node[1].msg_addr  = dma_node[0].msg_addr + dma_node[0].msg_bytes / 4;
