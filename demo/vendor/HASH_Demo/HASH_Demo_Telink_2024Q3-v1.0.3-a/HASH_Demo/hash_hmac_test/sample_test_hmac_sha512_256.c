...
unsigned int sample_test_hmac_sha512_256(void)
{
    ...
    if (HASH_SUCCESS != ret) {
        ...
        #ifdef CONFIG_HASH_SUPPORT_ADDRESS_HIGH_LOW
    ret = hmac_sha512_256_dma_update_blocks(hmac_sha512_256_dma_ctx, addr_h_0, (unsigned int)msg_buf, blocks_bytes);
        #else
    ...
    if (HASH_SUCCESS != ret) {
        ...
        #ifdef CONFIG_HASH_SUPPORT_ADDRESS_HIGH_LOW
    ret = hmac_sha512_256_dma_final(hmac_sha512_256_dma_ctx, addr_h_0, ((unsigned int)msg_buf) + blocks_bytes, remainder_bytes, addr_h_0, (unsigned int)mac_buf);
        #else
    ...
    uint32_clear(mac_buf, mac_byte_len / 4);
        #ifdef CONFIG_HASH_SUPPORT_ADDRESS_HIGH_LOW
    ret = hmac_sha512_256_dma(key, 0, key_byte_len, addr_h_0, (unsigned int)msg_buf, msg_byte_len, addr_h_0, (unsigned int)mac_buf, hash_call_manage);
        #else
    ...
    ret                    = hmac_sha512_256_dma_node_steps(key, 0, key_byte_len, dma_node, 2, addr_h_0, (unsigned int)mac_buf, hash_call_manage);
        #else
    ...
    uint32_clear(mac_buf, mac_byte_len / 4);
        #ifdef CONFIG_HASH_SUPPORT_ADDRESS_HIGH_LOW
    ret = hmac_dma(HASH_SHA512_256, key, 0, key_byte_len, addr_h_0, (unsigned int)msg_buf, msg_byte_len, addr_h_0, (unsigned int)mac_buf, hash_call_manage);
        #else
    ...
    dma_node[1].msg_addr_l = dma_node[0].msg_addr_l + dma_node[0].msg_bytes;
    dma_node[1].msg_bytes  = remainder_bytes;
    ret                    = hmac_dma_node_steps(HASH_SHA512_256, key, 0, key_byte_len, dma_node, 2, addr_h_0, (unsigned int)mac_buf, hash_call_manage);
        #else
    dma_node[0].msg_addr  = msg_buf;
    dma_node[0].msg_bytes = blocks_bytes;
    dma_node[1].msg_addr  = dma_node[0].msg_addr + dma_node[0].msg_bytes / 4;
