...
unsigned int sample_test_hmac_sha512_224(void)
{
    ...
    ret = hmac_sha512_224_dma_update_blocks(hmac_sha512_224_dma_ctx, msg_buf, msg_words);
    if (HASH_SUCCESS != ret) {
        return 1;
    }

    ret = hmac_sha512_224_dma_final(hmac_sha512_224_dma_ctx, msg_buf + msg_words, remainder_bytes, mac_buf);
    if (HASH_SUCCESS != ret) {
        return 1;
    }
