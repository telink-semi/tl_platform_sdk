...
unsigned int sample_test_hmac_sha512_256(void)
{
    ...
    ret = hmac_sha512_256_dma_update_blocks(hmac_sha512_256_dma_ctx, msg_buf, msg_words);
    if(HASH_SUCCESS != ret)
    {
        return 1;
    }

    ret = hmac_sha512_256_dma_final(hmac_sha512_256_dma_ctx, msg_buf+msg_words, remainder_bytes, mac_buf);
    if(HASH_SUCCESS != ret)
    {
        return 1;
    }
    ...
    if(memcmp_(std_mac, mac_buf, mac_byte_len))
    {
        printf("\r\n HMAC-SHA512_256 dma sample test 3 error");fflush(stdout);
        return 1;
    }
    else
    {
        printf("\r\n HMAC-SHA512_256 dma sample test 3 success");fflush(stdout);
    }
