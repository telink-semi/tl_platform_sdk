...
unsigned char hmac_test(HASH_ALG hash_alg, unsigned char *std_mac1, unsigned char *std_mac2, unsigned char *std_mac3,
				unsigned char *std_mac4, unsigned char *std_mac5)
{
	...
	/*************************** test1 ***************************/
	ret = hmac_init(ctx, hash_alg, key, 3);
	...
	if(HASH_SUCCESS != ret || memcmp(mac, std_mac1, outLen[hash_alg]))
	{
	...
	/*************************** test2 ***************************/
	ret = hmac_init(ctx, hash_alg, key, block_byte_len);
	ret |= hmac_update(ctx, message, 3);
	ret |= hmac_final(ctx, mac);
	if(HASH_SUCCESS != ret || memcmp(mac, std_mac2, outLen[hash_alg]))
	{
	...
	/*************************** test3 ***************************/
	ret = hmac_init(ctx, hash_alg, key, 250);
	ret |= hmac_update(ctx, message, 3);
	ret |= hmac_final(ctx, mac);
	if(HASH_SUCCESS != ret || memcmp(mac, std_mac3, outLen[hash_alg]))
	{
	...
	//test4-1
#if 1
	ret = hmac(hash_alg, key, 250, message, 500, mac);
#else
	ret = hmac_init(ctx, hash_alg, key, 250);
	ret |= hmac_update(ctx, message, 500);
	ret |= hmac_finish(ctx, digest);
#endif
	if(HASH_SUCCESS != ret || memcmp(mac, std_mac4, outLen[hash_alg]))
	{
		printf("\r\n %s test 4-%d failure, ret =%02x \r\n", name[hash_alg], 1, ret);
	    return 1;
	}
	else
	{
		printf("\r\n %s test 4-%d success ", name[hash_alg], 1);
	}
	...
	for(i=0;i<5;i++)
	{
		...
		{
			printf("\r\n %s test 4-%d success ", name[hash_alg], i+2);
		}
	}
...
//			printf("4-%d ", i+2);
	...
	if(HASH_SUCCESS != ret || memcmp(message, std_mac5, outLen[hash_alg]))
	{
	...
	{
		printf("\r\n %s test 5 success ", name[hash_alg]);
	}
#endif
...
unsigned char HMAC_SHA256_SHA512_cross_test()
{
	unsigned char key[250];
    ...
    ret = hmac_init(hmac_sha256_ctx, HASH_SHA256, key, 250);
    ret |= hmac_init(hmac_sha512_ctx, HASH_SHA512, key, 250);
	...
	if(HASH_SUCCESS != ret || memcmp(mac1, std_hmac_sha256_mac, 32))
	{
	...
	if(HASH_SUCCESS != ret || memcmp(mac2, std_hmac_sha512_mac, 64))
	{
	...
	{
		printf("\r\n HMAC_SHA512 test success ");fflush(stdout);
	}		
...
#define HMAC_SPEED_TEST_ROUND             (409600)
#define HMAC_SPEED_TEST_BYTE_LEN          (1024)
#define HMAC_DMA_SPEED_TEST_ROUND         (1000)
#define HMAC_DMA_SPEED_TEST_WORD_LEN      (262144)    //1024*16*SM3_BLOCK_WORD_LEN

void hmac_speed_test()
{
	...
	unsigned int i;
	HMAC_CTX ctx[1];
	HASH_ALG hash_alg = HASH_SHA512_224;//HASH_SHA512_256;//HASH_SM3;//HASH_SHA512;//HASH_SHA384;//HASH_SHA256;//HASH_MD5;//HASH_SHA1;//HASH_SHA224;//

	printf("\r\n hash begin");fflush(stdout);
	hmac_init(ctx, hash_alg, key, 16);
	for(i=0;i<HMAC_SPEED_TEST_ROUND;i++)
		hmac_update(ctx, message, HMAC_SPEED_TEST_BYTE_LEN);
	hmac_final(ctx, mac);
...
void hmac_dma_speed_test()
{
	...
	unsigned int *iterator = msg+512;
	HASH_ALG hash_alg = HASH_SHA1;//HASH_MD5;//HASH_SHA384;//HASH_SHA256;//HASH_SM3;//HASH_SHA512;//HASH_SHA512_224;//HASH_SHA512_256;//HASH_SHA224;//

	//dma speed test
	printf("\r\n hmac dma begin");fflush(stdout);
	hmac_dma_init(ctx, hash_alg, key, 16, hash_call_manage);
	for(i=0;i<HMAC_DMA_SPEED_TEST_ROUND;i++)
		hmac_dma_update_blocks(ctx, msg, HMAC_DMA_SPEED_TEST_WORD_LEN, iterator);
	hmac_dma_final(ctx, msg, 0, iterator, mac);
	printf("\r\n hmac dma finished");fflush(stdout);
}


unsigned char hmac_dma_test(HASH_ALG hash_alg, unsigned char *std_mac1, unsigned char *std_mac2, unsigned char *std_mac3,
					  unsigned char *std_mac4, unsigned char *std_mac5)
{
	HMAC_DMA_CTX ctx[1];

	unsigned char key[250];
	unsigned char message[512]={0x61,0x62,0x63,0x00};
	unsigned char tmp_buf[500];
	unsigned int *msg = (unsigned int *)(DMA_RAM_BASE);
	unsigned int *tmp_iterator = msg+512;
	unsigned char mac[64];
	char *name[]={"HMAC_SM3","HMAC_MD5","HMAC_SHA256","HMAC_SHA384","HMAC_SHA512","HMAC_SHA1","HMAC_SHA224","HMAC_SHA512_224","HMAC_SHA512_256"};
	...
	memset(mac, 0, digest_byte_len);
	key_bytes = 3;
	msg_bytes = 3;
#if 0
	ret = hmac_dma(hash_alg, key, keyByteLen, msg, msgByteLen, tmp_iterator, mac, hash_call_manage);
#else
	ret = hmac_dma_init(ctx, hash_alg, key, key_bytes, hash_call_manage);
	ret |= hmac_dma_update_blocks(ctx, msg, 0, tmp_iterator);
	ret |= hmac_dma_final(ctx, msg, msg_bytes-block_byte_len*0, tmp_iterator, mac);
#endif
	if(HASH_SUCCESS != ret || memcmp(std_mac1, mac, digest_byte_len))
	{
	...
	ret |= hmac_dma_final(ctx, msg, msg_bytes-block_byte_len*0, tmp_iterator, mac);
#endif
	if(HASH_SUCCESS != ret || memcmp(std_mac2, mac, digest_byte_len))
	{
	...
	msg_bytes = 3;
#if 0
	ret = hmac_dma(hash_alg, key, keyByteLen, msg, msgByteLen, tmp_iterator, mac);
#else
	ret = hmac_dma_init(ctx, hash_alg, key, key_bytes, hash_call_manage);
	ret |= hmac_dma_update_blocks(ctx, msg, 0, tmp_iterator);
	ret |= hmac_dma_final(ctx, msg, msg_bytes-block_byte_len*0, tmp_iterator, mac);
#endif
	if(HASH_SUCCESS != ret || memcmp(std_mac3, mac, digest_byte_len))
	{
	...
	{
		ret |= hmac_dma_init(ctx, hash_alg, key, key_bytes, hash_call_manage);
		ret |= hmac_dma_update_blocks(ctx, msg, block_word_len, tmp_iterator);
		ret |= hmac_dma_update_blocks(ctx, msg+block_word_len, block_word_len*4, tmp_iterator);
		ret |= hmac_dma_update_blocks(ctx, msg+block_word_len*5, block_word_len*2, tmp_iterator);
		ret |= hmac_dma_final(ctx, msg+block_word_len*7, msg_bytes-block_byte_len*7, tmp_iterator, mac);
	}
	else if(32 == block_word_len)  //for sha384,sha512,etc.
	{
		ret |= hmac_dma_init(ctx, hash_alg, key, key_bytes, hash_call_manage);
		ret |= hmac_dma_update_blocks(ctx, msg, block_word_len, tmp_iterator);
		ret |= hmac_dma_update_blocks(ctx, msg+block_word_len, block_word_len*2, tmp_iterator);
		ret |= hmac_dma_final(ctx, msg+block_word_len*3, msg_bytes-block_byte_len*3, tmp_iterator, mac);
	}
#endif
	if(HASH_SUCCESS != ret || memcmp(std_mac4, mac, digest_byte_len))
	{
...
///*
	...
	for(i=0;i<=500; i++)
	{
		...
		ret |= hmac_dma(hash_alg, key, 250, msg, i, tmp_iterator, mac, hash_call_manage);

		memcpy(message, mac, digest_byte_len);
    }
	if(HASH_SUCCESS != ret || memcmp(message, std_mac5, digest_byte_len))
	{
...
unsigned char HMAC_SHA224_test()
{
...
unsigned char HMAC_SHA256_test()
{
...
unsigned char HMAC_SHA384_test()
{
...
unsigned char HMAC_SHA512_test()
{
...
unsigned char HMAC_SHA512_224_test()
{
...
unsigned char HMAC_SHA512_256_test()
{
