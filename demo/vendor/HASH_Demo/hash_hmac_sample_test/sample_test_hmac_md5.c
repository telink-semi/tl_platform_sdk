...
//DMA mode is not supported
	...
	/**************** test 1 ****************/
	ret = hmac_md5_init(hmac_md5_ctx, key, key_byte_len);
	if(HASH_SUCCESS != ret)
	{
		return 1;
	}
	...
	ret = hmac_md5(key, key_byte_len, msg, msg_byte_len, mac);
	if(HASH_SUCCESS != ret)
	{
		return 1;
	}
	...
	/**************** test 3 ****************/
	memset(mac, 0, mac_byte_len);
	ret = hmac_init(hmac_ctx, HASH_MD5, key, key_byte_len);
	if(HASH_SUCCESS != ret)
	{
		return 1;
	}
	...
	/**************** test 4 ****************/
	memset(mac, 0, mac_byte_len);
	ret = hmac(HASH_MD5, key, key_byte_len, msg, msg_byte_len, mac);
	if(HASH_SUCCESS != ret)
	{
		return 1;
	}
...
//DMA mode is not supported
	...
	/**************** dma test 1 ****************/
	ret = hmac_md5_dma_init(hmac_md5_dma_ctx, key, key_byte_len, hash_call_manage);
	if(HASH_SUCCESS != ret)
	{
		return 1;
	}

	ret = hmac_md5_dma_update_blocks(hmac_md5_dma_ctx, msg_buf, msg_words, iterator_buf);
	if(HASH_SUCCESS != ret)
	{
		return 1;
	}

	ret = hmac_md5_dma_final(hmac_md5_dma_ctx, msg_buf+msg_words, remainder_bytes, iterator_buf, mac);
	if(HASH_SUCCESS != ret)
	{
		return 1;
	}
	...
	ret = hmac_md5_dma(key, key_byte_len, msg_buf, msg_byte_len, iterator_buf, mac, hash_call_manage);
	if(HASH_SUCCESS != ret)
	{
		return 1;
	}
	...
	/**************** dma test 3 ****************/
	ret = hmac_dma_init(hmac_dma_ctx, HASH_MD5, key, key_byte_len, hash_call_manage);
	if(HASH_SUCCESS != ret)
	{
		return 1;
	}
	...
	/**************** dma test 4 ****************/
	memset_(mac, 0, mac_byte_len);
	ret = hmac_dma(HASH_MD5, key, key_byte_len, msg_buf, msg_byte_len, iterator_buf, mac, hash_call_manage);
	if(HASH_SUCCESS != ret)
	{
		return 1;
	}
