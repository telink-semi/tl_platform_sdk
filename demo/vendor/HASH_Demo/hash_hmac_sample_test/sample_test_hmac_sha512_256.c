...
unsigned char sample_test_hmac_sha512_256()
{
	...
	unsigned char std_mac[32] = {
		0xD3,0x71,0x1E,0x06,0x80,0x81,0xF2,0xC4,0xA2,0xB1,0x1A,0x35,0xF7,0x36,0xAF,0xAD,
...
//	unsigned char block_byte_len = 128;
	...
	HMAC_SHA512_256_CTX hmac_sha512_256_ctx[1];
	HMAC_CTX *hmac_ctx = hmac_sha512_256_ctx;
...
//DMA mode is not supported
	...
	HMAC_SHA512_256_DMA_CTX hmac_sha512_256_dma_ctx[1];
	HMAC_DMA_CTX *hmac_dma_ctx = hmac_sha512_256_dma_ctx;
	...
	/**************** test 1 ****************/
	ret = hmac_sha512_256_init(hmac_sha512_256_ctx, key, key_byte_len);
	if(HASH_SUCCESS != ret)
	{
		return 1;
	}
	...
	{
		printf("\r\n HMAC-SHA512_256 sample test 1 error");fflush(stdout);
		return 1;
	}
	else
	{
		printf("\r\n HMAC-SHA512_256 sample test 1 success");fflush(stdout);
	}


	/**************** test 2 ****************/
	memset(mac, 0, mac_byte_len);
	ret = hmac_sha512_256(key, key_byte_len, msg, msg_byte_len, mac);
	if(HASH_SUCCESS != ret)
	{
		return 1;
	}
	...
	if(memcmp(std_mac, mac, mac_byte_len))
	{
		printf("\r\n HMAC-SHA512_256 sample test 2 error");fflush(stdout);
		return 1;
	}
	else
	{
		printf("\r\n HMAC-SHA512_256 sample test 2 success");fflush(stdout);
	}

	/**************** test 3 ****************/
	memset(mac, 0, mac_byte_len);
	ret = hmac_init(hmac_ctx, HASH_SHA512_256, key, key_byte_len);
	if(HASH_SUCCESS != ret)
	{
		return 1;
	}
	...
	if(memcmp(std_mac, mac, mac_byte_len))
	{
		printf("\r\n HMAC-SHA512_256 sample test 3 error");fflush(stdout);
		return 1;
	}
	else
	{
		printf("\r\n HMAC-SHA512_256 sample test 3 success");fflush(stdout);
	}


	/**************** test 4 ****************/
	memset(mac, 0, mac_byte_len);
	ret = hmac(HASH_SHA512_256, key, key_byte_len, msg, msg_byte_len, mac);
	if(HASH_SUCCESS != ret)
	{
		return 1;
	}
	...
	if(memcmp(std_mac, mac, mac_byte_len))
	{
		printf("\r\n HMAC-SHA512_256 sample test 4 error");fflush(stdout);
		return 1;
	}
	else
	{
		printf("\r\n HMAC-SHA512_256 sample test 4 success");fflush(stdout);
	}
...
//DMA mode is not supported
	...
	/**************** dma test 1 ****************/
	ret = hmac_sha512_256_dma_init(hmac_sha512_256_dma_ctx, key, key_byte_len, hash_call_manage);
	if(HASH_SUCCESS != ret)
	{
		return 1;
	}

	ret = hmac_sha512_256_dma_update_blocks(hmac_sha512_256_dma_ctx, msg_buf, msg_words, iterator_buf);
	if(HASH_SUCCESS != ret)
	{
		return 1;
	}

	ret = hmac_sha512_256_dma_final(hmac_sha512_256_dma_ctx, msg_buf+msg_words, remainder_bytes, iterator_buf, mac);
	if(HASH_SUCCESS != ret)
	{
		return 1;
	}
	...
	if(memcmp(std_mac, mac, mac_byte_len))
	{
		printf("\r\n HMAC-SHA512_256 dma sample test 1 error");fflush(stdout);
		return 1;
	}
	else
	{
		printf("\r\n HMAC-SHA512_256 dma sample test 1 success");fflush(stdout);
	}

	/**************** dma test 2 ****************/
	memset(mac, 0, mac_byte_len);
	ret = hmac_sha512_256_dma(key, key_byte_len, msg_buf, msg_byte_len, iterator_buf, mac, hash_call_manage);
	if(HASH_SUCCESS != ret)
	{
		return 1;
	}
	...
	if(memcmp(std_mac, mac, mac_byte_len))
	{
		printf("\r\n HMAC-SHA512_256 dma sample test 2 error");fflush(stdout);
		return 1;
	}
	else
	{
		printf("\r\n HMAC-SHA512_256 dma sample test 2 success");fflush(stdout);
	}

	/**************** dma test 3 ****************/
	ret = hmac_dma_init(hmac_dma_ctx, HASH_SHA512_256, key, key_byte_len, hash_call_manage);
	if(HASH_SUCCESS != ret)
	{
		return 1;
	}
	...
	/**************** dma test 4 ****************/
	memset(mac, 0, mac_byte_len);
	ret = hmac_dma(HASH_SHA512_256, key, key_byte_len, msg_buf, msg_byte_len, iterator_buf, mac, hash_call_manage);
	if(HASH_SUCCESS != ret)
	{
		return 1;
	}
	...
	if(memcmp(std_mac, mac, mac_byte_len))
	{
		printf("\r\n HMAC-SHA512_256 dma sample test 4 error");fflush(stdout);
		return 1;
	}
	else
	{
		printf("\r\n HMAC-SHA512_256 dma sample test 4 success");fflush(stdout);
	}
#endif
