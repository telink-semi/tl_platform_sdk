...
unsigned int sample_test_sha224(void)
{
    ...
    if(memcmp_(std_digest, digest, digest_byte_len))
    {
        printf("\r\n SHA224 sample test 6 error");fflush(stdout);
        return 1;
    }
    else
    {
        printf("\r\n SHA224 sample test 6 success");fflush(stdout);
    }
#endif
    ...
    if(uint32_BigNumCmp((unsigned int *)std_digest, digest_byte_len/4, digest_buf, digest_byte_len/4))
    {
        printf("\r\n SHA224 dma sample test 6 error");fflush(stdout);
        return 1;
    }
    else
    {
        printf("\r\n SHA224 dma sample test 6 success");fflush(stdout);
    }
