...
unsigned int sample_test_sha256(void)
{
    ...
    if(memcmp_(std_digest, digest, digest_byte_len))
    {
        printf("\r\n SHA256 sample test 5 error");//fflush(stdout);
        return 1;
    }
    else
    {
        printf("\r\n SHA256 sample test 5 success");//fflush(stdout);
    }
    ...
    if(memcmp_(std_digest, digest, digest_byte_len))
    {
        printf("\r\n SHA256 sample test 6 error");fflush(stdout);
        return 1;
    }
    else
    {
        printf("\r\n SHA256 sample test 6 success");fflush(stdout);
    }
#endif
...
#ifdef CONFIG_HASH_SUPPORT_ADDRESS_HIGH_LOW
    ret = sha256_dma_update_blocks(sha256_dma_ctx, addr_h_0, (unsigned int)msg_buf, blocks_bytes);
#else
...
#ifdef CONFIG_HASH_SUPPORT_ADDRESS_HIGH_LOW
    ret = sha256_dma_final(sha256_dma_ctx, addr_h_0, ((unsigned int)msg_buf)+blocks_bytes, remainder_bytes, addr_h_0, (unsigned int)digest_buf);
#else
    ...
    uint32_clear(digest_buf, digest_byte_len/4);
#ifdef CONFIG_HASH_SUPPORT_ADDRESS_HIGH_LOW
    ret = sha256_dma(addr_h_0, (unsigned int)msg_buf, msg_byte_len, addr_h_0, (unsigned int)digest_buf, hash_call_manage);
#else
    ...
    ret = sha256_dma_node_steps(dma_node, 2, addr_h_0, (unsigned int)digest_buf, hash_call_manage);
#else
    ...
    uint32_clear(digest_buf, digest_byte_len/4);
#ifdef CONFIG_HASH_SUPPORT_ADDRESS_HIGH_LOW
    ret = hash_dma(HASH_SHA256, addr_h_0, (unsigned int)msg_buf, msg_byte_len, addr_h_0, (unsigned int)digest_buf, hash_call_manage);
#else
    ...
    if(uint32_BigNumCmp((unsigned int *)std_digest, digest_byte_len/4, digest_buf, digest_byte_len/4))
    {
        printf("\r\n SHA256 dma sample test 5 error");//fflush(stdout);
        return 1;
    }
    else
    {
        printf("\r\n SHA256 dma sample test 5 success");//fflush(stdout);
    }
    ...
    dma_node[1].msg_addr_l  = dma_node[0].msg_addr_l + dma_node[0].msg_bytes;
    dma_node[1].msg_bytes   = remainder_bytes;
    ret = hash_dma_node_steps(HASH_SHA256, dma_node, 2, addr_h_0, (unsigned int)digest_buf, hash_call_manage);
#else
    dma_node[0].msg_addr  = msg_buf;
    dma_node[0].msg_bytes = blocks_bytes;
    dma_node[1].msg_addr  = dma_node[0].msg_addr + dma_node[0].msg_bytes/4;
    ...
    if(uint32_BigNumCmp((unsigned int *)std_digest, digest_byte_len/4, digest_buf, digest_byte_len/4))
    {
        printf("\r\n SHA256 dma sample test 6 error");fflush(stdout);
        return 1;
    }
    else
    {
        printf("\r\n SHA256 dma sample test 6 success");fflush(stdout);
    }
