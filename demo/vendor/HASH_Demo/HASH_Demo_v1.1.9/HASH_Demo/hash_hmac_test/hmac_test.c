...
unsigned int hmac_test(HASH_ALG hash_alg, unsigned char *std_mac1, unsigned char *std_mac2, unsigned char *std_mac3, unsigned char *std_mac4, unsigned char *std_mac5, unsigned char *std_mac6)
{
    ...
    if (HASH_SUCCESS != ret || memcmp_(mac, std_mac4, outLen[hash_alg])) {
        printf("\r\n %s test 4-final%u(hash node style) failure, ret =%02x \r\n", name[hash_alg], i + 2, ret);
        print_buf_U8(mac, outLen[hash_alg], "mac");
        return 1;
    } else {
        printf("\r\n %s test 4-final%u(hash node style) success ", name[hash_alg], i + 2);
    }
#endif
    ...
    if (long_key_bytes > block_byte_len) {
        ...
        for (i = long_key_bytes - block_byte_len; i < long_key_bytes; i++) {
            ...
            if (HASH_SUCCESS != ret || memcmp_((unsigned char *)(ctx->K0), mac, outLen[hash_alg]) ||
                !uint8_BigNum_Check_Zero(((unsigned char *)(ctx->K0)) + outLen[hash_alg], block_byte_len - outLen[hash_alg])) {
                printf("\r\n %d test 6-%s(long key) failure, ret =%02x ", i, name[hash_alg], ret);
                print_buf_U8((unsigned char *)(ctx->K0), outLen[hash_alg], "ctx->K0");
                print_buf_U8(mac, outLen[hash_alg], "mac");
                print_buf_U8(long_key, long_key_bytes, "long_key");
                return 1;
            } else {
...
unsigned int HMAC_SHA256_SHA512_cross_test(void)
{
    ...
    if (HASH_SUCCESS != ret || memcmp_((unsigned char *)mac1, std_hmac_sha256_mac, 32)) {
    ...
    if (HASH_SUCCESS != ret || memcmp_((unsigned char *)mac2, std_hmac_sha512_mac, 64)) {
    ...
    memcpy_((unsigned char *)msg_buf, message, msg_byte_len);
    ...
    ret |= hmac_dma_init(hmac_sha512_dma_ctx, HASH_SHA512, key, key_id, 250, hash_call_manage);
        ...
        #ifdef CONFIG_HASH_SUPPORT_ADDRESS_HIGH_LOW
    ret |= hmac_dma_update_blocks(hmac_sha256_dma_ctx, addr_h_0, (unsigned int)msg_buf, 64 * 1);

    ret |= hmac_dma_update_blocks(hmac_sha512_dma_ctx, addr_h_0, (unsigned int)msg_buf, 128 * 1);

    ret |= hmac_dma_update_blocks(hmac_sha256_dma_ctx, addr_h_0, ((unsigned int)msg_buf) + 64 * 1, 64 * 2);

    ret |= hmac_dma_update_blocks(hmac_sha512_dma_ctx, addr_h_0, ((unsigned int)msg_buf) + 128 * 1, 128 * 1);

    ret |= hmac_dma_update_blocks(hmac_sha256_dma_ctx, addr_h_0, ((unsigned int)msg_buf) + 64 * 3, 64 * 4);

    ret |= hmac_dma_update_blocks(hmac_sha512_dma_ctx, addr_h_0, ((unsigned int)msg_buf) + 128 * 2, 128 * 1);

    ret = hmac_dma_final(hmac_sha256_dma_ctx, addr_h_0, ((unsigned int)msg_buf) + (64 * 7), 52, addr_h_0, (unsigned int)hmac1_buf);
        #else
    ...
    ret = hmac_dma_final(hmac_sha256_dma_ctx, msg_buf + (64 / 4 * 7), 52, hmac1_buf);
        #endif
    if (HASH_SUCCESS != ret || memcmp_((unsigned char *)hmac1_buf, std_hmac_sha256_mac, 32)) {
        printf("\r\n HMAC_SHA256 dma test failure \r\n");
        print_buf_U8((unsigned char *)hmac1_buf, 32, "HMAC_SHA256 mac");
        return 1;
    } else {
        ...
        #ifdef CONFIG_HASH_SUPPORT_ADDRESS_HIGH_LOW
    ret = hmac_dma_final(hmac_sha512_dma_ctx, addr_h_0, ((unsigned int)msg_buf) + (128 * 3), 116, addr_h_0, (unsigned int)hmac2_buf);
        #else
    ret = hmac_dma_final(hmac_sha512_dma_ctx, msg_buf + (128 / 4 * 3), 116, hmac2_buf);
        #endif
    if (HASH_SUCCESS != ret || memcmp_((unsigned char *)hmac2_buf, std_hmac_sha512_mac, 64)) {
        printf("\r\n HMAC_SHA512 dma test failure \r\n");
        print_buf_U8((unsigned char *)hmac2_buf, 64, "HMAC_SHA512 mac");
        return 1;
    } else {
...
#ifdef HASH_DMA_FUNCTION
unsigned int hmac_dma_speed_test_internal(HASH_ALG hash_alg, unsigned int round, unsigned int once_bytes)
{
    char *name_alg[] = {
        "SM3",
        "MD5",
    ...
    for (i = 0; i < round; i++) {
    #ifdef CONFIG_HASH_SUPPORT_ADDRESS_HIGH_LOW
        ret = hmac_dma_update_blocks(ctx, addr_h_0, (unsigned int)msg, once_bytes);
    #else
    ...
    #ifdef CONFIG_HASH_SUPPORT_ADDRESS_HIGH_LOW
    ret = hmac_dma_final(ctx, addr_h_0, (unsigned int)msg, 0, addr_h_0, (unsigned int)mac);
    #else
...
unsigned int hmac_dma_test(HASH_ALG hash_alg, unsigned char *std_mac1, unsigned char *std_mac2, unsigned char *std_mac3, unsigned char *std_mac4, unsigned char *std_mac5)
{
    HMAC_DMA_CTX   ctx[1];
    unsigned short key_id = 0;
    unsigned char  key[250];
    ...
    memcpy(message, (unsigned char *)"abc", 3); //to avoid some bad compiler's error(memset not found)
    ...
    #ifdef CONFIG_HASH_SUPPORT_ADDRESS_HIGH_LOW
        #if 0
    ret = hmac_dma(hash_alg, key, key_id, key_bytes, addr_h_0, (unsigned int)msg, msg_bytes, addr_h_0, (unsigned int)mac, hash_call_manage);
        #else
    ret = hmac_dma_init(ctx, hash_alg, key, key_id, key_bytes, hash_call_manage);
    ret |= hmac_dma_update_blocks(ctx, addr_h_0, (unsigned int)msg, 0);
    ret |= hmac_dma_final(ctx, addr_h_0, (unsigned int)msg, msg_bytes - block_byte_len * 0, addr_h_0, (unsigned int)mac);
        #endif
    #else
        #if 0
    ...
    #ifdef CONFIG_HASH_SUPPORT_ADDRESS_HIGH_LOW
    {
        ret |= hmac_dma_update_blocks(ctx, addr_h_0, ((unsigned int)msg) + i * block_byte_len, block_byte_len);
    }
    ret |= hmac_dma_final(ctx, addr_h_0, ((unsigned int)msg) + i * block_byte_len, msg_bytes - i * block_byte_len, addr_h_0, (unsigned int)mac);
    #else
    {
            ...
            if (hash_get_rand_number(156) & 1) {
                ...
                if (0 == (j % block_byte_len)) {
                    if (msg_bytes - total == j) {
                        ret |= hmac_dma_final(ctx, addr_h_0, ((unsigned int)msg) + total, j, addr_h_0, (unsigned int)mac);
                    } else {
                        ret |= hmac_dma_update_blocks(ctx, addr_h_0, (unsigned int)msg + total, j);
                    }
                } else {
                    j = 500 - total;
                    ret |= hmac_dma_final(ctx, addr_h_0, ((unsigned int)msg) + total, j, addr_h_0, (unsigned int)mac);
    ...
    hmac_dma_node[0].msg_addr_l = (unsigned int)msg;
    ...
    ret                         = hmac_dma_node_steps(hash_alg, key, key_id, key_bytes, hmac_dma_node, 3, addr_h_0, (unsigned int)mac, hash_call_manage);
        #else
    ...
    #ifdef CONFIG_HASH_SUPPORT_ADDRESS_HIGH_LOW
        ret |= hmac_dma(hash_alg, key, key_id, 250, addr_h_0, (unsigned int)msg, i, addr_h_0, (unsigned int)mac, hash_call_manage);
    #else
...
{
    unsigned char std_mac1[32] = {0xC7, 0x30, 0xD1, 0x1F, 0x7A, 0x09, 0xE1, 0x65, 0xDC, 0x6E, 0xD6, 0x96, 0x4B, 0x49, 0x22, 0x52, 0xB2, 0x5C, 0x07, 0x50, 0x8B, 0x3B, 0xAA, 0xD6, 0x28, 0x42, 0xAF, 0xE9, 0x2A, 0x45, 0x79, 0x0F};
    unsigned char std_mac2[32] = {0x14, 0xCC, 0xAD, 0xBE, 0xE9, 0x2A, 0x9B, 0xE2, 0x79, 0xC8, 0x49, 0xB7, 0x35, 0x9F, 0xAF, 0xAC, 0x65, 0xA9, 0xF0, 0x4B, 0x15, 0x6F, 0xA8, 0x72, 0x3A, 0x72, 0x70, 0x0E, 0x50, 0x69, 0x27, 0xD5};
    unsigned char std_mac3[32] = {0xCC, 0x05, 0x07, 0x7B, 0x6F, 0x37, 0xC7, 0xE5, 0xA1, 0x9D, 0xC4, 0xEF, 0x00, 0x08, 0xD4, 0xEB, 0x6B, 0xCD, 0xA8, 0xD7, 0xD6, 0x61, 0xCE, 0xF2, 0x22, 0x9D, 0xBA, 0xA1, 0x98, 0x8A, 0xE5, 0x50};
    unsigned char std_mac4[32] = {0x8A, 0x8B, 0xB2, 0x17, 0xA6, 0xA0, 0xC6, 0x0C, 0x7E, 0x81, 0x0C, 0x33, 0x9F, 0xBF, 0x82, 0x38, 0x77, 0x54, 0x25, 0x80, 0xCF, 0x54, 0xD1, 0x33, 0x29, 0x02, 0xC4, 0xD7, 0x50, 0x73, 0x49, 0x39};
    unsigned char std_mac5[32] = {0x54, 0xDE, 0x55, 0x11, 0x30, 0x7B, 0xEB, 0x47, 0xA9, 0x62, 0x8C, 0x3B, 0x68, 0x30, 0xCF, 0xBB, 0x55, 0x3F, 0x63, 0x89, 0xB6, 0xC5, 0x9F, 0x48, 0x12, 0x7E, 0x5B, 0xF8, 0xF0, 0xB0, 0x8D, 0x5D};
    unsigned char std_mac6[32] = {0xAE, 0xB2, 0x26, 0x53, 0x67, 0xF5, 0x14, 0x06, 0x4C, 0x57, 0x37, 0xA9, 0x04, 0x71, 0xD5, 0xBA, 0xFC, 0x4F, 0x53, 0xA2, 0x66, 0x4A, 0xBF, 0x45, 0x3E, 0x50, 0xA2, 0xD9, 0x6E, 0x1C, 0xBD, 0xF9};
    unsigned int  ret;
...
{
    unsigned char std_mac1[28] = {0xF4, 0x20, 0x89, 0x8A, 0xD4, 0x5F, 0x46, 0x12, 0x8B, 0x1D, 0xFD, 0x2D, 0x7D, 0xF3, 0x11, 0x05, 0x0E, 0x84, 0x95, 0x97, 0x52, 0xA5, 0x35, 0xA0, 0x08, 0x02, 0xDE, 0xCE};
    unsigned char std_mac2[28] = {0xD6, 0x5B, 0x4A, 0x91, 0x6E, 0xD9, 0x98, 0xE7, 0x20, 0xEE, 0xE0, 0xEF, 0xA6, 0xC9, 0x1E, 0x62, 0x3E, 0x96, 0x19, 0xF1, 0x0B, 0x07, 0x4F, 0x9C, 0x8A, 0x7C, 0x2D, 0x35};
    unsigned char std_mac3[28] = {0xFF, 0x6B, 0xCC, 0x17, 0x41, 0x12, 0xFB, 0x71, 0xB7, 0x36, 0x69, 0x07, 0xD5, 0x8A, 0x16, 0x3D, 0x3F, 0x01, 0x7D, 0x30, 0x17, 0x2D, 0x28, 0x93, 0xD6, 0x47, 0xC9, 0xCF};
    unsigned char std_mac4[28] = {0x02, 0xFD, 0x7B, 0xE5, 0x70, 0xA8, 0x8C, 0x9C, 0x23, 0x5B, 0x1E, 0x98, 0xC3, 0x76, 0x29, 0x08, 0x2C, 0x06, 0xE3, 0xC5, 0xAA, 0x31, 0xF5, 0x7D, 0x58, 0x35, 0xE2, 0xE6};
    unsigned char std_mac5[28] = {0xC3, 0x81, 0x12, 0x65, 0x5D, 0x56, 0x2C, 0x11, 0x73, 0xE2, 0xB7, 0x82, 0x36, 0x86, 0x53, 0x29, 0xDC, 0x6E, 0x50, 0x48, 0x54, 0xBA, 0xD0, 0x09, 0xD9, 0x13, 0xE4, 0x11};
    unsigned char std_mac6[28] = {0x0F, 0x7F, 0x6E, 0x14, 0x17, 0xDD, 0x4F, 0xB3, 0x6A, 0x86, 0xF8, 0x4B, 0xC2, 0xF1, 0xA0, 0x4C, 0xB3, 0xF9, 0x72, 0xA0, 0x53, 0x11, 0xBF, 0x6C, 0xC4, 0x9C, 0xBF, 0xA3};
    unsigned int  ret;
...
{
    unsigned char std_mac1[32] = {0xCA, 0x7B, 0x86, 0xA2, 0xFB, 0xF0, 0x03, 0x1A, 0xCC, 0xFC, 0x5F, 0xD5, 0x8E, 0xD4, 0x60, 0xC3, 0xAB, 0x2A, 0xEB, 0xAF, 0x8E, 0x86, 0xCB, 0xBC, 0x18, 0xC8, 0x38, 0xBC, 0x00, 0xAE, 0x87, 0xFF};
    unsigned char std_mac2[32] = {0x6A, 0xB5, 0x41, 0xB4, 0x86, 0x9D, 0xCA, 0x71, 0xC4, 0xCA, 0x11, 0xD8, 0xBB, 0x1B, 0x02, 0x53, 0x3B, 0x78, 0x9A, 0x55, 0x75, 0x83, 0x16, 0x14, 0x29, 0x29, 0x2C, 0x74, 0x04, 0xBC, 0x21, 0xF6};
