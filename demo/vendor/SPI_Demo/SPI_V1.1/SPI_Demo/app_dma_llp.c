...
void main_loop (void)
{
    ...
    while(!(end_irq_flag == 2));//Wait for the spi to finish sending the first packet of data.
#endif
#endif
    ...
    while(!(end_irq_flag == 1));//Wait for spi to finish receiving the first packet.
    ...
    while(!(end_irq_flag == 2));//Wait for spi to finish receiving the second packet.
#endif
#endif
#if(DMA_LLP_MODE == DMA_LLP_SINGLE_CHAIN)
    if(end_irq_flag == 2)
