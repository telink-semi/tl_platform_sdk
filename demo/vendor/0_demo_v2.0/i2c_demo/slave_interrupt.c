...
#if (I2C_MODE    ==  I2C_SLAVE_INTERRUPT)
unsigned char i2c_slave_it_rx_buf[1024];
unsigned int i2c_slave_it_rx_len = 1024;
unsigned char i2c_slave_it_tx_buf[1024] = { 1, 2, 3, 4 };
unsigned int i2c_slave_it_tx_len = 1024;
...
void i2c_slave_it_rx_test_cbk(i2c_handle_t *hi2c)
{
    soc_printf("slave end_len:%d\n", hi2c->rx_idx);
    for (unsigned int i = 0; i < hi2c->rx_idx; i++) {
        soc_printf("slave rx_buff[%d] %d, \n", i, hi2c->rx_buff[i]);
    }
}

void i2c_slave_it_tx_test_cbk(i2c_handle_t *hi2c)
{
    switch (hi2c->state) {
    case DEV_I2C_STATE_SUCCESS:
        soc_printf("slave tx_idx %d\n", hi2c->tx_idx);
        for (unsigned int i = 0; i < hi2c->tx_idx; i++) {
            soc_printf("slave tx_buff[%d] %d, \n", i, hi2c->tx_buff[i]);
        }
        break;

    case DEV_I2C_STATE_ERROR:
        soc_printf("slave tx_idx %d\n", hi2c->tx_idx);
        for (unsigned int i = 0; i < hi2c->tx_idx; i++) {
            soc_printf("slave tx_buff[%d] %d, \n", i, hi2c->tx_buff[i]);
        }
        soc_printf("slave repeat_cnt %d\n", hi2c->repeat_cnt);
        break;
    default:
        break;
    }
...
void i2c_slave_init(i2c_handle_t *hi2c)
{
    hi2c->init.i2c_mode = SLAVE_MODE;
    ...
    hi2c->tx_cplt_cb = i2c_slave_it_tx_test_cbk;
    hi2c->rx_cplt_cb = i2c_slave_it_rx_test_cbk;
    hi2c->rx_buff = i2c_slave_it_rx_buf;
    hi2c->tx_buff = i2c_slave_it_tx_buf;
    drv_i2c_init(hi2c);
    soc_printf("can start i2c slave test\n");
}
...
//      }
    ...
    i2c_slave_init(&i2c1_handle);
    for (int i = 0; i < 64; i++) {
        i2c_slave_it_tx_buf[i] = i;
    }

    while (1) {
        drv_i2c_slave_interrupt_read(&i2c1_handle, i2c_slave_it_rx_buf, 16);
        drv_i2c_slave_interrupt_write(&i2c1_handle, i2c_slave_it_tx_buf, 16);
        drv_delay_ms(1000);
    }
}
