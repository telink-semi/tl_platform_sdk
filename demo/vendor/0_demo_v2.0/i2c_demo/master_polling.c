...
#if (I2C_MODE    ==  I2C_MASTER_POLLING)
...
void drv_i2c_master_tx_rx_test(i2c_handle_t *hi2c)
{
    uint32_t i = 0;
    ...
    drv_i2c_master_sync_send(hi2c, 0x28, 0, gI2cTxBuf, 16);
    drv_delay_ms(100);
    drv_i2c_master_sync_receive(hi2c, 0x28, 0, gI2cRxBuf, 16);
    for (i = 0; i < 16; i++) {
        LOG_INF("gI2cRxBuf[%d]:0x%x\r\n", i, gI2cRxBuf[i]);
    }
}
...
//    }
    ...
    i2c1_handle.init.int_mask = DRV_RESET;
    drv_i2c_init(&i2c1_handle);
    while (1) {
        drv_i2c_master_tx_rx_test(&i2c1_handle);
        drv_delay_ms(1000);
    }
