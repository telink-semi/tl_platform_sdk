 ...
 *******************************************************************************************************/
#include "common.h"
#if(SPI_MODE   ==    SPI_MASTER_INTERRUPT_MODE)
/*************************** macro definition ********************************/
...
/**************************** function declear *******************************/
void drv_spim_txcplt_callback(void *hspi);
void drv_spim_rxcplt_callback(void *hspi);
void drv_spim_error_callback(void *val);
...
spi_handle_t hspim;
spi_cb_t spim_cd = {
    .rx_cplt_cb = drv_spim_rxcplt_callback,
    .tx_cplt_cb = drv_spim_txcplt_callback,
    .rx_error_cb = drv_spim_error_callback,
    .tx_error_cb = drv_spim_error_callback,
};
...
void DW_APB_SSI0_M_IRQHandler(void)
{
    drv_spim_irqhandler(&hspim);
}

ISR_REGISTER(DW_APB_SSI0_M_IRQHandler, DW_APB_SSI0_M_IRQn)
...
void spi_master_init(void)
{
    ...
    hspim.init.mclk = 48000000; //g_mclk0_freq; //spim_mclk = mclk0 = 48m
    hspim.init.spi_clock = 3000000;
    hspim.init.rsd = 0;
    drv_spi_deinit(&hspim);
    drv_spi_init(&hspim);

    drv_spi_register_cb(&hspim, &spim_cd);
...
void drv_spi_msp_init(spi_handle_t *hspi)
{
    ...
    drv_hw_int_enable(DW_APB_SSI0_M_IRQn); /*!< enable hw int */
}
...
void main_loop(void)
{
    ...
    drv_spi_tx_it(&hspim, (uint8_t *)spi_tx_buf, g_spi_tx_send_len);
    while (spi_tx_flag == 0);
    spi_tx_flag = 0;
    drv_delay_ms(5);
    soc_printf("spi_it_txrx\r\n");
    spi_rx_flag = 0;
    drv_spi_txrx_it(&hspim, (uint8_t *)spi_tx_buf, (uint8_t *)spi_rx_buf, g_spi_tx_send_len);
    while (spi_rx_flag == 0)
        ;
    drv_delay_ms(1);
    spi_tx_buf[0] ++;
