...
#define DSP_FW_DOWNLOAD_FLASH_ADDR         0x180000
#else
...
void user_init(void)
{
...
#if defined(MCU_CORE_TL752X)
    log_init();
    soc_printf("systimer sample start\r\n");
...
#if (!(defined(MCU_CORE_TL752X) || defined(MCU_CORE_TL7518) || defined(MCU_CORE_TL751X) || defined(MCU_CORE_TL322X) || defined(MCU_CORE_TL753X))) && (STIMER_MODE == STIMER_IRQ)
...
#elif ((defined(MCU_CORE_TL752X) || defined(MCU_CORE_TL7518) || defined(MCU_CORE_TL751X) || defined(MCU_CORE_TL322X) || defined(MCU_CORE_TL753X)) && ((STIMER_MODE == STIMER_IRQ_D25) || (STIMER_MODE == STIMER_TIMEOUT_IRQ)))
    ...
    #if defined(MCU_CORE_TL752X)
    plic_interrupt_enable(SYSTIMER_IRQn);
    #else
    plic_interrupt_enable(IRQ_SYSTIMER);
    #endif
    core_interrupt_enable();

#elif ((defined(MCU_CORE_TL752X) || defined(MCU_CORE_TL7518) || defined(MCU_CORE_TL751X)|| defined(MCU_CORE_TL322X)) && (STIMER_MODE == STIMER_IRQ_D25_N22_DSP))
    stimer_set_irq_capture_d25f(stimer_get_tick() + SYSTEM_TIMER_TICK_1S); //set capture tick
...
#elif ((!defined(MCU_CORE_TL752X)) && (STIMER_MODE == STIMER_GET_32K_TICK))
     clock_32k_init(CLK_32K_RC);
     clock_cal_32k_rc();

#elif ((!defined(MCU_CORE_TL752X)) && (STIMER_MODE == STIMER_SET_32K_TICK))
    clock_set_32k_tick(clock_get_32k_tick() + 32000); //1s
    #if defined(MCU_CORE_B91)||defined(MCU_CORE_B92)||defined(MCU_CORE_TL322X)
...
#elif ((!defined(MCU_CORE_TL752X)) && (!defined(MCU_CORE_B91)) && (STIMER_MODE == STIMER_SET_32K_TICK_LVL))
    clock_set_32k_tick(clock_get_32k_tick() + 32000); //1s
    pm_set_wakeup_src(PM_WAKEUP_TIMER);
    #if defined(MCU_CORE_B92)||defined(MCU_CORE_TL322X)
    pm_clr_irq_status(WAKEUP_STATUS_ALL);
    #else
    pm_clr_irq_status(FLD_WAKEUP_STATUS_ALL);
    #endif
...
#elif ((!defined(MCU_CORE_TL752X)) && (STIMER_MODE == STIMER_TRACK_32K_TICK))
    clock_32k_init(CLK_32K_RC);
    clock_cal_32k_rc();
...
#elif (defined(MCU_CORE_TL721X) || defined(MCU_CORE_TL321X) || defined(MCU_CORE_TL322X) || defined(MCU_CORE_TL323X)) && (STIMER_MODE == STIMER_INPUT_CAPTURE)
...
#elif (defined(MCU_CORE_TL721X) || defined(MCU_CORE_TL321X) || defined(MCU_CORE_TL322X) || defined(MCU_CORE_TL323X)) && (STIMER_MODE == STIMER_INPUT_CAPTURE_DMA)
    /**
...
#elif (defined(MCU_CORE_TL721X) || defined(MCU_CORE_TL321X) || defined(MCU_CORE_TL322X) || defined(MCU_CORE_TL323X)) && (STIMER_MODE == STIMER_INPUT_CAPTURE_DMA_LLP_MODE)
    /**
     * It is recommended to enable DMA before enabling capture.
     * Otherwise, the signal may be captured but not read by DMA, which could result in a data overflow.
...
#elif (!(defined(MCU_CORE_TL7518) || defined(MCU_CORE_TL751X) || defined(MCU_CORE_TL322X) || defined(MCU_CORE_TL753X))) && (STIMER_MODE == STIMER_TIMEOUT_IRQ)
    delay_ms(500);
    stimer_set_irq_capture_d25f(stimer_get_tick() - 10*SYSTEM_TIMER_TICK_1MS);
...
#elif ((!defined(MCU_CORE_TL752X)) && (STIMER_MODE == STIMER_GET_TICK))
...
#elif (defined(MCU_CORE_TL752X)) && ((STIMER_MODE == STIMER_IRQ_D25) || (STIMER_MODE == STIMER_IRQ_D25_N22_DSP) || (STIMER_MODE == STIMER_TIMEOUT_IRQ))
void SYSTIMER_IRQHandler(void)
{
    if (stimer_get_irq_status(FLD_SYSTEM_IRQ_D25F))
    {
        ...
        systimer_time_t time_t;
        time_t = drv_systimer_get_time();
        soc_printf("D25F ms:%d \n", time_t.ms);
        soc_printf("D25F us:%d \n", time_t.us);
        stimer_set_irq_capture_d25f(stimer_get_tick() + SYSTEM_TIMER_TICK_1S);
    }
}
PLIC_ISR_REGISTER(SYSTIMER_IRQHandler, SYSTIMER_IRQn)
#elif ((!defined(MCU_CORE_TL752X)) && (STIMER_MODE == STIMER_SET_32K_TICK))
...
#elif ((!defined(MCU_CORE_TL752X)) && (STIMER_MODE == STIMER_SET_32K_TICK_LVL))
...
_attribute_ram_code_sec_ void stimer_irq_handler(void)
{
    ...
    #endif
    #if defined(MCU_CORE_TL721X) || defined(MCU_CORE_TL323X)
