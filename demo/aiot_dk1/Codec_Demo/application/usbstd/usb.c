...
void usb_handle_set_intf(void)
{
    ...
    app_usb_handle_set_intf(intf_index, value_l);
#if (USB_MIC_ENABLE)
...
void usb_init_interrupt(void)
{
    ...
    usbhw_set_eps_en(FLD_USB_EDP8_EN | FLD_USB_EDP1_EN | FLD_USB_EDP2_EN | FLD_USB_EDP3_EN | FLD_USB_EDP4_EN | FLD_USB_EDP5_EN | FLD_USB_EDP6_EN |
                     FLD_USB_EDP7_EN); // attention: when test ch8 and ch9 need disable not used endpoint.
}
