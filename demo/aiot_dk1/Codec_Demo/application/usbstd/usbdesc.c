...
// #include "../../../../vendor/USB_AUDIO_DEMO/usb_default.h"
 ...
 */
const USB_Descriptor_String_t language_desc = {{sizeof(USB_Descriptor_Hdr_t) + 2, DTYPE_String}, {LANGUAGE_ID_ENG}};
...
const unsigned char OSFeatureDescriptor[] = {
    ...
    0x44, 0x00, 0x65, 0x00, 0x76, 0x00, 0x69, 0x00, 0x63, 0x00, 0x65, 0x00, 0x49, 0x00, 0x64, 0x00, 0x6C, 0x00, 0x65, 0x00, 0x45, 0x00, 0x6E, 0x00, 0x61, 0x00, 0x62, 0x00, 0x6C,
    ...
    0x44, 0x00, 0x65, 0x00, 0x66, 0x00, 0x61, 0x00, 0x75, 0x00, 0x6C, 0x00, 0x74, 0x00, 0x49, 0x00, 0x64, 0x00, 0x6C, 0x00, 0x65, 0x00, 0x53, 0x00, 0x74, 0x00, 0x61, 0x00, 0x74,
    ...
    0x44, 0x00, 0x65, 0x00, 0x66, 0x00, 0x61, 0x00, 0x75, 0x00, 0x6C, 0x00, 0x74, 0x00, 0x49, 0x00, 0x64, 0x00, 0x6C, 0x00, 0x65, 0x00, 0x54, 0x00, 0x69, 0x00, 0x6D, 0x00, 0x65,
    0x00, 0x6F, 0x00, 0x75, 0x00, 0x74, 0x00, 0x00, 0x00,
    ...
    0x55, 0x00, 0x73, 0x00, 0x65, 0x00, 0x72, 0x00, 0x53, 0x00, 0x65, 0x00, 0x74, 0x00, 0x44, 0x00, 0x65, 0x00, 0x76, 0x00, 0x69, 0x00, 0x63, 0x00, 0x65, 0x00, 0x49, 0x00, 0x64,
    0x00, 0x6C, 0x00, 0x65, 0x00, 0x45, 0x00, 0x6E, 0x00, 0x61, 0x00, 0x62, 0x00, 0x6C, 0x00, 0x65, 0x00, 0x64, 0x00, 0x00, 0x00,

    0x04, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00,

    //user-enabled remote wake setting.

    0x36, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x24, 0x00, 0x53, 0x00, 0x79, 0x00, 0x73, 0x00, 0x74, 0x00, 0x65, 0x00, 0x6D, 0x00, 0x57, 0x00, 0x61, 0x00, 0x6B, 0x00, 0x65,
    0x00, 0x45, 0x00, 0x6E, 0x00, 0x61, 0x00, 0x62, 0x00, 0x6C, 0x00, 0x65, 0x00, 0x64, 0x00, 0x00, 0x00,
...
#if USB_KEYBOARD_ENABLE
        {
            USB_INTF_KEYBOARD, 0x01, 'W', 'I', 'N', 'U', 'S', 'B', 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        },
#endif

#if USB_MOUSE_ENABLE
        {
            USB_INTF_MOUSE, 0x01, 'W', 'I', 'N', 'U', 'S', 'B', 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        },
#endif
    }};
#if 0
...
const USB_Descriptor_Device_t device_desc = {
    ...
    USB_CTR_ENDPOINT_SIZE, // Endpoint0Size, Maximum Packet Size for Zero Endpoint. B91/B92 only support 8, others can be config 8, 16, 32, 64.
    ID_VENDOR,             // VendorID
...
const USB_Descriptor_Configuration_t configuration_desc = {
    {
        {sizeof(USB_Descriptor_Configuration_Hdr_t), DTYPE_Configuration}, // Length, type
    ...
    // audio_control_interface_ac;
    {
        ...
        {(sizeof(USB_Audio_Descriptor_Interface_AC_t) +                                                       /*9*/
          sizeof(USB_Audio_Descriptor_InputTerminal_t) +                                                      /*12*/
          sizeof(USB_Audio_Descriptor_OutputTerminal_t) + /*9*/ sizeof(USB_Audio_StdDescriptor_FeatureUnit_t) /*10*/
          ),
         0},
...
static unsigned short usb_desc_str[32 + 1];
 ...
 */
static unsigned short *usbdesc_get_str(const char *src)
{
    unsigned int char_count;

    char_count             = strlen(src);
    unsigned int max_count = sizeof(usb_desc_str) / sizeof(usb_desc_str[0]) - 1; // -1 for string type
    ...
    for (unsigned int i = 0; i < char_count; i++) {
        usb_desc_str[1 + i] = src[i];
    }

    /* first byte is length (including header), second byte is string type. */
    usb_desc_str[0] = (unsigned short)((DTYPE_String << 8) | (sizeof(USB_Descriptor_Hdr_t) + char_count * 2));
...
{
    return (unsigned char *)usbdesc_get_str(STRING_VENDOR);
}

unsigned char *usbdesc_get_product(void)
{
    return (unsigned char *)usbdesc_get_str(STRING_PRODUCT);
}

#if (MS_OS_DESCRIPTOR_ENABLE)

unsigned char *usbdesc_get_OS_descriptor(void)
{
    return (unsigned char *)usbdesc_get_str(STRING_MSFT);
}

unsigned char *usbdesc_get_OSFeature(int *length)
{
    *length = sizeof(OSFeatureDescriptor);
    return (unsigned char *)(&OSFeatureDescriptor);
}

unsigned char *usbdesc_get_compatID(int *length)
{
    *length = OSFeatureDescriptor_compatID.compatID_Header.dwLength;

    return (unsigned char *)(&OSFeatureDescriptor_compatID);
}
...
{
    return (unsigned char *)usbdesc_get_str(STRING_SERIAL);
}
...
{
    return (unsigned char *)(&configuration_desc.audio_descriptor);
}
#endif
...
{
    return (unsigned char *)(&configuration_desc.somatic_descriptor);
}
#endif
