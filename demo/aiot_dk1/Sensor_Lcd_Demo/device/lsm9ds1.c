...
{
    _i2c_master_write(LSM9DS1_I2C_AG_ADDR, &reg_addr, 1);
    return _i2c_master_read(LSM9DS1_I2C_AG_ADDR, pData, Size) ? 0 : ENXIO;
}

static uint8_t lsm9ds1_mag_iic_read(uint8_t reg_addr, uint8_t *pData, uint32_t Size)
{
    _i2c_master_write(LSM9DS1_I2C_MAG_ADDR, &reg_addr, 1);
    return _i2c_master_read(LSM9DS1_I2C_MAG_ADDR, pData, Size) ? 0 : ENXIO;
}
...
__attribute__((unused)) static uint8_t lsm9ds1_ag_iic_write(uint8_t reg_addr, uint8_t pData)
{
    ...
    return _i2c_master_write(LSM9DS1_I2C_AG_ADDR, write_buff, 2) ? 0 : ENXIO;
}

__attribute__((unused)) static uint8_t lsm9ds1_mag_iic_write(uint8_t reg_addr, uint8_t pData)
{
    uint8_t write_buff[2] = {reg_addr, pData};
    return _i2c_master_write(LSM9DS1_I2C_MAG_ADDR, write_buff, 2) ? 0 : ENXIO;
}
