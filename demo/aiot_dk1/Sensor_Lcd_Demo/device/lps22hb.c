...
{
    _i2c_master_write(LPS22HB_I2C_ADDR, &reg_addr, 1);
    return _i2c_master_read(LPS22HB_I2C_ADDR, pData, Size) ? 0 : ENXIO;
}

__attribute__((unused)) static uint8_t lps22hb_iic_write(uint8_t reg_addr, uint8_t pData)
{
    uint8_t write_buff[2] = {reg_addr, pData};
    return _i2c_master_write(LPS22HB_I2C_ADDR, write_buff, 2) ? 0 : ENXIO;
}
