 ...
 *  Attention
 *  ==============
  -# When three cores write to the interrupt mask register or other shared registers simultaneously, there is a risk of errors, \n
 ...
 *              so only one core should operate on these registers at a time, or a mutex variable should be set for exclusivity.
 */
static _always_inline void stimer_set_irq_mask(stimer_irq_e mask)
{
 ...
 *              so only one core should operate on these registers at a time, or a mutex variable should be set for exclusivity.
 */
static _always_inline void stimer_clr_irq_mask(stimer_irq_e mask)
{
    reg_system_irq_mask &= (~mask);
}
 ...
 * @return      none.
 */
static _always_inline void stimer_clr_irq_status(stimer_irq_e status)
{
