...
/**
 * @brief      This function enable adc source clock: xtal_48M
 * @return     none
 */
...
/**
 * @brief      This function disable adc source clock: xtal 48M
 * @return     none
 */
...
{
    /*
     * When only enabling the FLD_M_CH_TRANS_EN bit in areg_adc_data_transfer_control and not enabling the FLD_AUTO_NOT_EN bit in reg_adc_config0, 
     * the data from the M channel won't be transferred to the FIFO, but the LR channel data will still be transferred. 
     * This design was made with the assumption that only the LR channel would be used. However, in practice, even when using only the LR channel, M channel data is still being scanned. 
     * This logic is currently only implemented in Panther and Onca chips. After discussing with the digital team, this logic will be removed in future chip versions.
     * (updated by bolong.zhang, confirmed by shiyi.wu at 202503031)
     */
    reg_adc_config0 |= FLD_M_CH_TRANS_EN; //Used to transfer data from the adc's M channel to the sar adc rxfifo.
    analog_write_reg8(areg_adc_data_transfer_control, analog_read_reg8(areg_adc_data_transfer_control) & (~FLD_AUTO_NOT_EN)); //Used to enable the transfer of data from the adc's M-channel, L-channel, and R-channel to the sar adc rxfifo.
}
 ...
 * @return      none
 * @note        Enabling this function on TL751X chip will affect adc performance. This function cannot be used
 */
 ...
 * @return   none.
 * @note     -# User need to wait >200us after adc_power_on() for ADC to be stable.
 *           -# If you calling adc_power_off(), because all analog circuits of ADC are turned off after adc_power_off(),
 *            it is necessary to wait > 200us after re-adc_power_on() for ADC to be stable.
 */
