...
{
    /*lspi and gspi with txfifo depth = 32 bytes.*/
    return (32 - (reg_spi_txfifo_status(spi_sel) & FLD_SPI_TXF_ENTRIES) < 4);
}
...
void gspi_set_pin(gspi_pin_config_t *spi_pin_config)
{
    ...
    gspi_set_pin_mux(spi_pin_config->spi_io4_pin, GSPI_IO4_IO);
    gspi_set_pin_mux(spi_pin_config->spi_io5_pin, GSPI_IO5_IO);
    gspi_set_pin_mux(spi_pin_config->spi_io6_pin, GSPI_IO6_IO);
    gspi_set_pin_mux(spi_pin_config->spi_io7_pin, GSPI_IO7_IO);
    gspi_set_pin_mux(spi_pin_config->spi_dm_pin, GSPI_DM_IO);
}
...
{
    lspi_set_pin_mux(spi_pin_config->spi_clk_pin, LSPI_CK_IO);
    lspi_set_pin_mux(spi_pin_config->spi_csn_pin, LSPI_CN_IO);
    lspi_set_pin_mux(spi_pin_config->spi_mosi_io0_pin, LSPI_MOSI_IO);
    lspi_set_pin_mux(spi_pin_config->spi_miso_io1_pin, LSPI_MISO_IO);
    lspi_set_pin_mux(spi_pin_config->spi_io2_pin, LSPI_IO2_IO);
    lspi_set_pin_mux(spi_pin_config->spi_io3_pin, LSPI_IO3_IO);
    lspi_set_pin_mux(spi_pin_config->spi_io4_pin, LSPI_IO4_IO);
    lspi_set_pin_mux(spi_pin_config->spi_io5_pin, LSPI_IO5_IO);
    lspi_set_pin_mux(spi_pin_config->spi_io6_pin, LSPI_IO6_IO);
    lspi_set_pin_mux(spi_pin_config->spi_io7_pin, LSPI_IO7_IO);
    lspi_set_pin_mux(spi_pin_config->spi_dm_pin, LSPI_DM_IO);
}
...
void spi_slave_set_pin(sspi_pin_config_t *spi_pin_config)
{
    ...
    if ((spi_pin_config->spi_csn_pin != GPIO_FC_PA4)) {
        if ((reg_gpio_func_mux(GPIO_FC_PA4) == SSPI_CN_I) && ((reg_gpio_func(GPIO_FC_PA4) & GPIO_PA4) == 0)) {
            gpio_function_en(GPIO_PA4);
        }
    }
    if ((spi_pin_config->spi_clk_pin != GPIO_FC_PA5)) {
        if ((reg_gpio_func_mux(GPIO_FC_PA5) == SSPI_CK_I) && ((reg_gpio_func(GPIO_FC_PA5) & GPIO_PA5) == 0)) {
            gpio_function_en(GPIO_PA5);
        }
    }
    if ((spi_pin_config->spi_miso_io1_pin != GPIO_FC_PA6)) {
        if ((reg_gpio_func_mux(GPIO_FC_PA6) == SSPI_SO_IO) && ((reg_gpio_func(GPIO_FC_PA6) & GPIO_PA6) == 0)) {
            gpio_function_en(GPIO_PA6);
        }
    }
    if ((spi_pin_config->spi_mosi_io0_pin != GPIO_FC_PA3)) {
        if ((reg_gpio_func_mux(GPIO_FC_PA3) == SSPI_SI_IO) && ((reg_gpio_func(GPIO_FC_PA3) & GPIO_PA3) == 0)) {
...
/**
 * @brief      This function serves to set the spi clk.
 * @param[in]   spi_sel     - the spi module.
 * @param[in]   src_clk     - Select spi clk source.
 * @param[in]   clk_div   - the division factor for SPI module.
 *              spi_clock = src_clk / clk_div
 * @return      DRV_API_SUCCESS: operation successful.
 *              DRV_API_INVALID_PARAM: clk_divc out of register bit width.
 ...
 *             - The maximum value of spi_clk that can be output by the master device is 48M, but it depends on whether the slave device can support it.
 *             - The master connects to the telink's slave, since the maximum clock of the slave spi module is 48M, according to the formula(spi_clock_in(output from master clk_pin )  <= slave module clk/4), the maximum value of spi_clk on the io is 12M.
 *             - for the master,due to the divider register bit width limitation,the limits between src_clk and spi_clock are as follows.
 *               LSPI(spi_clk minimum=src_clk/15),GSPI(spi_clk minimum=src_clk/255 ).
 */
drv_api_status_e spi_clock_clk_config(spi_sel_e spi_sel, sys_src_clk_e src_clk, unsigned short clk_div)
{
    switch (spi_sel) {
    case LSPI_MODULE:
        reg_rst0 |= FLD_RST0_LSPI;
        reg_clk_en0 |= FLD_CLK0_LSPI_EN;
        if (clk_div & 0xfff0) //clk_div  exceeds the register bit-width
        {
            return DRV_API_INVALID_PARAM;
        } else {
            reg_lspi_clk_set = clk_div | ((src_clk & 0x3) << 4); //default  FLD_LSPI_CLK_MOD =1
        }
        break;
    ...
    case GSPI_MODULE:
        ...
        if (clk_div & 0xff00) //clk_div exceeds the register bit-width
        {
            return DRV_API_INVALID_PARAM;
        } else {
            reg_gspi_clk_set = clk_div | ((src_clk & 0x3) << 8); //default  FLD_GSPI_CLK_MOD =1
        }
        break;
...
/**
 * @brief      This function serves to set the spi slave module clk.
 * @param[in]   spi_sel     - the spi module.
 * @param[in]   src_clk     - Select spi clk source.
 * @param[in]   clk_div   - the division factor for SPI module.
 *              spi_clock = src_clk / clk_div
 * @return     none
 * @note       - The master connects to the telink's slave, since the maximum clock of the slave spi module is 48M, according to the formula(spi_clock(output from master clk_pin )  <=  slave module clk/4),
 *               The limits between src_clk and spi_clock:LSPI/GPSI(spi_clk maximum)=src_clk/4.

 */
void spi_slave_clock_clk_config(spi_sel_e spi_sel, sys_src_clk_e src_clk)
{
    switch (src_clk) {
    case SRC_CLK_RC_24M:
        spi_clock_clk_config(spi_sel, SRC_CLK_RC_24M, 1);            //slave module clk 24M
        break;
    case SRC_CLK_XTAL_48M:
        spi_clock_clk_config(spi_sel, SRC_CLK_XTAL_48M, 1);          //slave module clk 48M
        break;
    case SRC_CLK_BASEBAND_PLL_192M:
        spi_clock_clk_config(spi_sel, SRC_CLK_BASEBAND_PLL_192M, 4); //slave module clk 48M
        break;
    case SRC_CLK_BASEBAND_PLL_264M:
        spi_clock_clk_config(spi_sel, SRC_CLK_BASEBAND_PLL_264M, 6); //slave module clk 44M
        break;
    case SRC_CLK_BASEBAND_PLL_300M:
        spi_clock_clk_config(spi_sel, SRC_CLK_BASEBAND_PLL_300M, 7); //slave module clk 42.857M
        break;
    case SRC_CLK_AUDIO_PLL_33P8688M:                                 //slave module clk 33.8688M
    case SRC_CLK_AUDIO_PLL_36P864M:                                  //slave module clk 36.864M
        spi_clock_clk_config(spi_sel, SRC_CLK_AUDIO_PLL_33P8688M, 1);
        break;
    case SRC_CLK_AUDIO_PLL_147P456M:                                 //slave module clk 36.864M
    case SRC_CLK_AUDIO_PLL_169P344M:                                 //slave module clk 42.336M
    case SRC_CLK_AUDIO_PLL_184P32M:                                  //slave module clk 46.08M
        spi_clock_clk_config(spi_sel, SRC_CLK_AUDIO_PLL_147P456M, 4);
        break;
    default:
        break;
    }
 ...
 * @param[in]   spi_sel     - the spi module.
 * @param[in]   src_clk     - select spi clk source,if you select SRC_CLK_BASEBAND_PLL and SRC_CLK_AUDIO_PLL,the frequency should be consistent with the clk initialization function.
 * @param[in]   clk_div     - the division factor for SPI module.
 *              spi_clock = src_clk / clk_div
 ...
 */
drv_api_status_e spi_master_init(spi_sel_e spi_sel, sys_src_clk_e src_clk, unsigned short clk_div, spi_mode_type_e mode)
{
    drv_api_status_e ret = spi_clock_clk_config(spi_sel, src_clk, clk_div);
    ...
    spi_rx_irq_trig_cnt(spi_sel, 4);                                                          //default gspi and lspi burst1(N=1) ,rx_fifo trig_cnt=4*N=4.
    spi_xip_dis(spi_sel);
 ...
 */
void spi_slave_init(spi_sel_e spi_sel, sys_src_clk_e src_clk, spi_mode_type_e mode)
{
    spi_slave_clock_clk_config(spi_sel, src_clk);
    reg_spi_ctrl3(spi_sel) &= (~FLD_SPI_MASTER_MODE);                                         //slave.
    reg_spi_ctrl3(spi_sel) |= FLD_SPI_AUTO_HREADY_EN;                                         //slave.
    reg_spi_ctrl3(spi_sel) = ((reg_spi_ctrl3(spi_sel) & (~FLD_SPI_WORK_MODE)) | (mode << 2)); // select SPI mode, support four modes.
    if (spi_sel == GSPI_MODULE) {
        spi_tx_irq_trig_cnt(spi_sel, 4);                                                      //default lspi and gspi burst1(N=1) ,tx_fifo trig_cnt=tx fifo depth-4*N=8-4=4.
    } else if (spi_sel == LSPI_MODULE) {
        spi_tx_irq_trig_cnt(spi_sel, 16);                                                     //default lspi and lspi burst1(N=1) ,tx_fifo trig_cnt=tx fifo depth-4*N=20-4=16.
    }
    spi_rx_irq_trig_cnt(spi_sel, 4);                                                          //default lspi and lspi burst1(N=1) ,rx_fifo trig_cnt=4*N=4.
    spi_xip_dis(spi_sel);
}
 ...
 * @param[in]   spi_sel - the spi module.
 * @param[in]   mode    - single/dual/quad/octal /3line.
 * @return      none.
  */
 ...
 */
drv_api_status_e spi_master_send_cmd(spi_sel_e spi_sel, unsigned short cmd)
{
...
/**
 * @brief       This function serves to single/dual/quad/octal write to the SPI slave.
 * @param[in]   spi_sel     - the spi module.
 ...
 *              DRV_API_TIMEOUT:timeout exit(solution refer to the note for spi_set_error_timeout).
 */
drv_api_status_e spi_master_write_plus(spi_sel_e spi_sel, unsigned short cmd, unsigned int addr, unsigned char *data, unsigned int data_len, spi_wr_tans_mode_e wr_mode)
{
    spi_tx_dma_dis(spi_sel);
    spi_tx_fifo_clr(spi_sel);
...
/**
 * @brief       This function serves to single/dual/quad/octal write data to the SPI slave repeatedly.
 * @param[in]   spi_sel     - the spi module.
 * @param[in]   cmd         - cmd one byte will first write.
 * @param[in]   addr        - the address of slave.
 * @param[in]   data        - pointer to the data need to write.
 ...
 */
drv_api_status_e spi_master_write_repeat_plus(spi_sel_e spi_sel, unsigned short cmd, unsigned int addr, unsigned char *data, unsigned int data_len, unsigned int repeat_time, spi_wr_tans_mode_e wr_mode)
{
    unsigned int i = 0, j = 0, k = 0;
    spi_tx_dma_dis(spi_sel);
    spi_tx_fifo_clr(spi_sel);
...
/**
 * @brief       This function serves to single/dual/quad/octal  read from the SPI slave.
 * @param[in]   spi_sel     - the spi module.
 * @param[in]   cmd         - cmd one byte will first write.
 * @param[in]   addr        - the address of slave.
 ...
 *              DRV_API_TIMEOUT:timeout exit(solution refer to the note for spi_set_error_timeout).
 */
drv_api_status_e spi_master_read_plus(spi_sel_e spi_sel, unsigned short cmd, unsigned int addr, unsigned char *data, unsigned int data_len, spi_rd_tans_mode_e rd_mode)
{
    spi_rx_dma_dis(spi_sel);
    spi_rx_fifo_clr(spi_sel);
    spi_set_address(spi_sel, addr);
 ...
 *              DRV_API_TIMEOUT:timeout exit(solution refer to the note for spi_set_error_timeout).
 */
drv_api_status_e spi_master_write_read_plus(spi_sel_e spi_sel, unsigned short cmd, unsigned char *addrs, unsigned int addr_len, unsigned char *data, unsigned int data_len, spi_rd_tans_mode_e wr_mode)
{
    spi_tx_dma_dis(spi_sel);
    spi_rx_dma_dis(spi_sel);
    spi_tx_fifo_clr(spi_sel);
...
/**
 * @brief       This function serves to single/dual/quad/octal  write to the SPI slave by dma.
 * @param[in]   spi_sel     - the spi module.
 * @param[in]   cmd         - cmd one byte will first write.
 * @param[in]   addr        - the address of slave.
 * @param[in]   data        - pointer to the data need to write.
...
/**
 * @brief       This function serves to single/dual/quad/octal  read from the SPI slave by dma.
 * @param[in]   spi_sel     - the spi module.
 * @param[in]   cmd         - cmd one byte will first write.
 * @param[in]   addr        - the pointer to the cmd and address for write.
...
/**
 * @brief       This function serves to single/dual/quad/octal write address and read from the SPI slave by dma.
 * This interface could be used for full duplex.
 * When this interface is used for full-duplex communication, it can only be used on the master side.
...
{
    unsigned int tx_fifo_depth = 0x04;
