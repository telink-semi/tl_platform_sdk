...
/**
 * @brief               from memory to memory via dma(memory: ram/flash/register)
 * @param[in] chn       - dma channel.
 * @param[in] src_buff  - src address,four-byte alignment is not required.
 * @param[in] dst_buff  - dst address,four-byte alignment is not required.
 * @param[in] len       - the maximum transmission length of DMA is 0xFFFFFC bytes
 * @return    none
 * @note                both the source and destination addresses are auto-increment.
 */
static inline void dma_from_memory_to_memory(dma_chn_e chn, unsigned char *src_buff, unsigned char *dst_buff, unsigned int len)
{
    unsigned int dma_cfg = (unsigned int)M2M_DMA_CFG;
    dma_config(chn, (dma_config_t *)&dma_cfg);
    dma_set_size(chn, len, DMA_BYTE_WIDTH);
    dma_set_address(chn, (unsigned int)(src_buff), (unsigned int)(dst_buff));
    dma_chn_en(chn);
}
...
/**
 * @brief     This function servers to disable DMA llp.
 * @param[in] chn       - dma channel.
 * @return    none.
 */
static inline void dma_llp_dis(dma_chn_e chn)
{
    reg_dma_llp(chn) = 0;
}
