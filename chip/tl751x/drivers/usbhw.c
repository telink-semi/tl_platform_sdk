...
{
    reg_gpio_func_mux(GPIO_PF2) = reg_gpio_func_mux(GPIO_PF2) & (~BIT_RNG(0, 5));
    gpio_function_dis(GPIO_PF2);
    reg_gpio_func_mux(GPIO_PF3) = reg_gpio_func_mux(GPIO_PF3) & (~BIT_RNG(0, 5));
    gpio_function_dis(GPIO_PF3);
    gpio_input_en((gpio_pin_e)(GPIO_PF2 | GPIO_PF3)); // DP/DM must set input enable
    usb_dp_pullup_en(1);
    /*                                      Note
