...
#define reg_system_irq_mask_d25f  REG_ADDR8(STIMER_BASE_ADDR + 0x8)
...
typedef enum
{
    ...
    FLD_SYSTEM_OVERFLOW_IRQ_MASK = BIT(2), /**<
                                            ...
                                            If you do not trigger an interrupt and only polling can detect the state, this mask also needs to be turned on.
                                          */
    FLD_SYSTEM_IRQ_MASK_D25F     = BIT(3), /**<
                                            When the current tick value of the system timer reaches the preset tick value,the IRQ_SYSTIMER interrupt is generated,
                                            This mask must be turned on if you want to trigger an interrupt.
                                            If you do not trigger an interrupt and only polling can detect the state, this mask also needs to be turned on.
                                          */
} stimer_irq_mask_d25f_e;

#define reg_system_cal_irq_d25f REG_ADDR8(STIMER_BASE_ADDR + 0x9)

typedef enum
{
    FLD_SYSTEM_IRQ_D25F     = BIT(0), /**<
                                            Get interrupt status:stimer_get_irq_status_d25f(),clear interrupt status:stimer_clr_irq_status_d25f().
    ...
    FLD_SYSTEM_IRQ_OVERFLOW = BIT(3), /**<
                                         ...
                                         <p>
                                            Note: The GPIO interrupt must be cleared at the same time -gpio_clr_irq_status(FLD_GPIO_IRQ_GPIO2RISC0_CLR),
                                            and capture interrupt -stimer_clr_irq_status_d25f(FLD_SYSTEM_IRQ_CAPT).
                                          */
} stimer_irq_status_e;
...
enum
{
    FLD_SYSTEM_IRQ_WAIT_D25F     = BIT(0),
    FLD_SYSTEM_TRIG_PAST_EN_D25F = BIT(4),
};

#define reg_system_timer_capt    REG_ADDR32(STIMER_BASE_ADDR + 0x1c)

#define reg_system_irq_level_n22 REG_ADDR32(STIMER_BASE_ADDR + 0x20)

#define reg_system_irq_mask_n22  REG_ADDR8(STIMER_BASE_ADDR + 0x28)

typedef enum
{
    FLD_SYSTEM_IRQ_MASK_N22     = BIT(0), /**<
                                            When the current tick value of the system timer reaches the preset tick value,the IRQ_SYSTIMER interrupt is generated,
                                            This mask must be turned on if you want to trigger an interrupt.
                                            If you do not trigger an interrupt and only polling can detect the state, this mask also needs to be turned on.
                                          */
    FLD_SYSTEM_IRQ_WAIT_N22     = BIT(1),
    FLD_SYSTEM_TRIG_PAST_EN_N22 = BIT(2),
} stimer_irq_mask_n22_e;

#define reg_system_irq_n22 REG_ADDR8(STIMER_BASE_ADDR + 0x29)

enum
{
    FLD_SYSTEM_IRQ_N22 = BIT(0), /**<
                                            Get interrupt status:stimer_get_irq_status_d25f(),clear interrupt status:stimer_clr_irq_status_d25f().
                                            The interrupt flag bit needs to be manually cleared.
                                            */
