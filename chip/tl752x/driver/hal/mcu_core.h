...
#if !defined(MCU_CORE_TL752X_N22) /* In CLIC mode, the MIE register is hardwired to zero. */
typedef enum
{
...
#else
/**
 * @brief This function serves to enable n22 interrupts globally in the system, MEI(machine external interrupt) will also enable.
 * @return  none
 */
static _always_inline void core_interrupt_enable(void)
{
 ...
 */
static _always_inline unsigned long long rdmcycle(void)
{
#if __riscv_xlen == 32
    do {
