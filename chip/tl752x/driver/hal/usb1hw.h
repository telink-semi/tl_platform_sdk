...
#include "reg/usb1_reg.h"
#include "hal/hal_systimer.h"

typedef enum
{
    /* EDP7 and EDP8 share 1024 bytes of space
     * EDP1,EDP2,and EDP3 share 64 bytes of space
     * EDP4,EDP5,and EDP6 share 64 bytes of space
     * */
...
typedef enum
{
    ...
    USB1_IRQ_SETUP_MASK     = BIT(4),
    USB1_IRQ_DATA_MASK      = BIT(5),
    USB1_IRQ_STATUS_MASK    = BIT(6),
    USB1_IRQ_SETINTF_MASK   = BIT(7),
} usb1_irq_mask_e;
...
/**
 * @brief     This function servers to set usb set address irq mask.
 * @param[in]  none.
 * @return    none.
 */
static inline void usb1hw_set_addr_irq_mask(void)
{
    reg_usb1_edp0_extn |= FLD_USB1_SET_ADDRESS_MASK;
}
...
static inline void usb1hw_set_printer_threshold(unsigned short th)
{
    ...
    reg_usb1_ep8_send_h_thres = (reg_usb1_ep8_send_h_thres & (~FLD_USB1_EDP8_MIN1)) | MASK_VAL(FLD_USB1_EDP8_MIN1, ((th >> 8) & FLD_USB1_EDP8_MIN1));
}
 ...
 * @return    none
 * @note
 *            - For tl752x the data address must be word aligned.
 */
