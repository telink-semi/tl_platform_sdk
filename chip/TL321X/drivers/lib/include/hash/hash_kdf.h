 ...
 * @return      0:success     other:error
 */
unsigned int pbkdf2_hmac(HASH_ALG hash_alg, unsigned char *pwd, unsigned short sp_key_idx, unsigned int pwd_bytes,
         unsigned char *salt, unsigned int salt_bytes, unsigned int iter, unsigned char *out, unsigned int out_bytes);
#endif


#ifdef SUPPORT_ANSI_X9_63_KDF
#ifdef SUPPORT_HASH_NODE
unsigned int ansi_x9_63_kdf_node_with_xor_in(HASH_ALG hash_alg, HASH_NODE *node, unsigned int node_num,
        unsigned char *counter,  unsigned char *in, unsigned char *out, unsigned int out_bytes, unsigned int check_whether_zero);

unsigned int ansi_x9_63_kdf_internal(HASH_ALG hash_alg, HASH_NODE *hash_node, unsigned int node_num, 
        unsigned char *counter, unsigned char *key, unsigned int key_bytes);

unsigned int ansi_x9_63_kdf_node(HASH_ALG hash_alg, HASH_NODE *hash_node, unsigned int node_num, unsigned char *counter,
        unsigned char *k1, unsigned int k1_bytes, unsigned char *k2, unsigned int k2_bytes);

unsigned int ansi_x9_63_kdf(HASH_ALG hash_alg,  unsigned char *Z, unsigned int Z_bytes,  unsigned char *shared_info,
